...
**Scope (owner-directed):** the flock's Drive tree holds spoken recordings of the letters — a "Read by Benjamin" folder that must SUPERSEDE all other renditions (no duplicates), and per-collection folders for everything else. Insert a play button near the top of the reading hero (letter title / WTLB title), plus intuitive whole-volume playback and WTLB "part 1, section 1" playback. Tracks STREAM (owner call — offline play attempts toast "requires internet"); Google Drive is the stream source (verified HTTP 206 range support on `drive.usercontent.google.com`; Bandcamp has no stable direct-mp3 URLs — its per-letter external links stay; a GitHub-Releases mirror stays a clean later retarget since the player just takes URLs). Per-track AND queued-album playback both work, matching the Bandcamp capability the owner named as the bar.

**Manifest pipeline.** `tools/fetch-drive-audio.py` (gdown, same technique as FlockSync) enumerates the public tree → `tools/_audio-drive-listing.json` (gitignored, regenerable); `tools/gen-audio-manifest.mjs` loads the corpus via vm (validate-schemas technique), parses `V{n}.{num}_{Title} (read by …)` names across prefixes V1–V7 / TLR / OFV(flock) / LFT(timothy) / WTLB / WTLB2 / TB2(blessed) / Holy-Days `NN_src_Title`, and emits `src/data/audio-manifest.js`: `AUDIO_MANIFEST` ("volKey:letterId" → `[[driveId, readerCode, partLabel?]…]`) + `AUDIO_SECTIONS` (wtlb1/wtlb2 → 7 range-compilation tracks each, "Part 3 · 40–59"). **Coverage: 729/729 — every letter, every preface, all 16 Holy Days entries.** Matching is staged (prefix+num verified by normalized title → title-index rescue for misnumbered uploads → "(Full Letter)" strip → "- Section N_…" strip → parenthetical strip → token-subset for dropped mid-title words), with: Benjamin supersedes wherever his file sits (reader rank B > Timothy > TTS > AI-with-music resolves every duplicate), full-letter tracks beat their per-section splits, "Addendum to X" files append as ordered parts (Christmas, Obey God), "0. ALL LETTERS" fills gaps only. Excluded by design: AI Songs (3,376), Gospel-of-John movie audio, Bible-Letter Studies tracks (span multiple study chapters), TSOT Matthew chapters, and 4 bonus tracks with no corpus letter (Amplified, Handmaidens of The Lord, Poured Out, My Anger Runs Deep) + the Garden intro reading — all reported by the generator, none shipped. Manifest rides **bundle-a-vot** (c19→c20; new build.py A_VOT member) so any screen that can show a play button has it loaded by definition; `CORPUS_CONTENT_VERSION` + search cache bumped in step.
