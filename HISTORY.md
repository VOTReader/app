...
- **P5 docs (this entry + CLAUDE.md current-state + BACKUP-STREAMING-PLAN.txt + AUDIT-PLAN.txt S4/S5 closure).** The only remaining cleanup (tracked, low-priority): fold the shared store-read between `applyV3` and `applyImportPayload` into one helper. The v2 `buildExportPayload`/`applyImportPayload` stay exported (rollback + legacy-import path) but the active export wiring no longer calls v2.
