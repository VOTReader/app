...
- **P1 manifest builder (landed):** `buildV3Manifest` (in `backup.js`) builds the v3 manifest (stores + media *metadata*, no base64) + the ordered blob list for the codec to stream; +4 tests including a builder→`writeContainer`→`readContainer` byte-exact round-trip. Self-contained — the v2 `buildExportPayload` is untouched; folding the shared store-read into one helper is a tracked P5 cleanup. vitest 1758/71 `test:coverage` exit 0.
- **Next:** `applyV3` (import applier) → P2 web I/O (FS Access API + `File.stream`) → P3 Android native framing + chunked bridge (emulator-verified) → P4 back-compat + cap removal → P5 docs.
