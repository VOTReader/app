...
### 2026-08-09 s8 — The Lamb of God illustrated (c27) + the system media card

**The seven illustrations enter the study (owner call — reverses the no-figurative-art default for this one study).** From the restored `THELAMBOFGODstudy.pdf` (see s7's addendum): seven `study-image` blocks in lamb-of-god ch3/4/6/7/8/11/12, placed exactly where the PDF places them — chapter-top for Supper/Gethsemane/Thorns/Burial/Resurrection/Emmaus, mid-chapter before the closing "I am The Passover" excerpt for the Crucifixion. No renderer change (LetterView's existing study-image block renders anywhere in blocks[]); no captions (the PDF has none — and the word-count baseline stays untouched). Assets `study-lamb-*.jpg` ×7 (long side ≤1400px, q82, 987 KB total) at the assets root, SW-precached; corpus c26→c27 (+ the `CORPUS_CONTENT_VERSION` twin in search/cache.js + lock). **Live-verified in the preview**: ch3/ch4/ch6/ch7/ch8 render + load with correct dimensions, and the ch7 mid-chapter image sits immediately before the excerpt in DOM order; all 7 srcs cross-check 1:1 against files.

**The system media card (owner request, from his GrapheneOS phone).** The player's web MediaSession is INERT inside a WebView — it never surfaces the QS/lock-screen card — so the same metadata now crosses the bridge natively. `AudioKeepAliveService` owns a `MediaSessionCompat` + MediaStyle notification (title/artist/artwork/seekbar, prev/play-pause/next), fed by `AppInterface.setAudioNowPlaying` (a 6-arg direct-call bridge method, contract-tested) from `_syncNative()` in audio-player.js — mirrored at the SAME sync points as the web session (track start, state edges, seek, rate) but deliberately NOT the 1 Hz timeupdate (the card interpolates position from rate; per-second Intents are churn). Transport flows back: session callbacks + notification-action PendingIntents → the `commandSink` static (MainActivity registers, onDestroy clears — the releaseTree pattern) → `JsEvent.MediaCommand` → `window.__votMediaCommand` → toggle/next/prev/seek. **Pause now keeps the card alive (YouTube semantics)** — the keep-alive edge releases only on `idle`: the paused card's Play button needs the WebView alive, so `_setStatus('paused')` no longer drops the anchor; natively the notification DETACHES from the FGS while paused (swipeable; the swipe's deleteIntent stops the service; the `running` gate stops a trailing update from resurrecting it). **POST_NOTIFICATIONS declared** (owner call, reversing the s-era no-nag omission for the media use-case only): asked contextually on the first playback of a session, never at boot; denied = the exact old behavior. Dependency: androidx.media (MediaSessionCompat + MediaStyle) — still no media3. The old class-KDoc "no MediaSession" rationale is rewritten in place. Gates: 3,633 vitest / 202 files, 250 Kotlin, lintDebug, eslint 0/0, tsc. **Owed: on-device verification** (card appearance, background playback, permission flow) — the phone dropped off wireless adb mid-session; the APK is built and pending install.

### 2026-08-09 s7 — External audit (Luna) round 2 adjudicated: Android API-floor crash, export fail-clean hole, lint into CI, backup rules

An external read-only audit ("Luna") returned a release-readiness verdict; every finding was re-verified from source before acting. **8 acted on, 3 adjudicated no-action with reasons.**

**The one real crash: `MainActivity`'s frame-rate vote read `display` on every boot.** `Context.getDisplay()` is API 30; on Android 8–10 (API 26–29, inside minSdk 26) resolving it throws `NoSuchMethodError` — an `Error` the surrounding `catch (Exception)` does NOT stop — during activity setup. Fixed: API-30 gate with the pre-30 `windowManager.defaultDisplay` fallback, so an old high-refresh panel still gets the window-level vote. Found by `lintDebug`, which had never been run as a gate — **6 NewApi errors total**, the other three adjudicated as lint-blind-spot, not behavior: `VOTReaderApp.setDataDirectorySuffix` (API 28, same uncatchable-Error class but only reachable from a sub-process that doesn't exist — gated anyway), `AppInterface`'s lazy `VibrationAttributes` (runtime-safe behind the API-33 dispatch check; lint can't see `@get:RequiresApi` through a lazy initializer lambda — restructured to a cached field + `@RequiresApi` function, behavior identical), and `AudioKeepAliveService.usesTypedForeground` (runtime-safe; annotated `@ChecksSdkIntAtLeast(api = Q)` so lint understands the extracted helper). **`lintDebug` now passes and runs in CI's kotlin job** — it ran tests but never lint, which is how 6 errors sat invisible.

**Export fail-clean had one genuine hole; the contract itself was defended.** Luna: "a failed export can delete a pre-existing user document." Adjudicated PARTIAL — once `openOutputStream(uri, "wt")` succeeds, truncation has already destroyed the old bytes, so the fail-clean delete destroys nothing extra (and SAF offers no reliable atomic-rename alternative; a temp-copy design doubles I/O on GB-scale exports). But if the open itself **throws** (revoked permission, dead provider) — as opposed to returning null, which was already handled — the old single try/catch deleted a document that was never touched. `writeTextToUri` now opens OUTSIDE the delete-on-failure region; new test RED-proven (fails on the old code, delete fires; 247 Kotlin tests green with the fix). `finishV3Export`'s deletes are all post-write and stay as designed; `beginV3Export` deliberately does NOT delete on failure (default "w" mode may not have truncated — deleting could destroy an intact document).

**Backup config now says on Android 12+ what it always meant.** `allowBackup="false"` no longer blocks device-to-device transfer for targetSdk 31+ — the new-phone migration flow would have copied the entire data dir (all IndexedDB user data). New `data_extraction_rules.xml` + `full_backup_content.xml` exclude every domain from cloud backup AND device transfer, wired in the manifest. **Owner-visible consequence: a new-phone migration will NOT carry app data — Settings → Your Data export/import is the way, which is the stated design.**

**Also landed:** dev-dep patch bumps via `npm audit fix` (js-yaml 4.3.1, nanoid 3.3.18 — two high advisories, dev-only, 0 vulnerabilities after; 3,628 vitest + eslint 0 + tsc green on the patched toolchain); the four OCR-era audit scripts (`misattribution_check` / `excerpt_audit` / `ocr_gap_check` / `aggregate_ocr`) repointed from the dead OneDrive checkout to script-relative paths + the current `assets/src/data/` layout (verified running); deploy-web's publish gate grew `check_balance.py` + `smoke-lite` (BLD3 — the en-dash/smart-quote data classes aren't JS syntax errors, so no deploy gate saw them; smoke:ci/coverage/app-size/type-scale stay CI-only by design); stray `kmath.js` (a Khan Academy khan-exercises utility misdirected into the repo root, untracked, unreferenced) deleted at owner request.

**Adjudicated no-action:** (1) *v3 import has no aggregate media cap* — by design: the manifest frame is size-bounded, blobs stream one at a time, and a hard aggregate cap would reject a legitimate large backup (user-data-paramount / salvage 99%>0); quota pressure surfaces at IDB write time. (2) *No encryption at rest* — app-layer encryption needs a passphrase, which the NO-credentials policy forbids; Android file-based encryption already covers the sandbox at the device level. (3) *Maintainability (cross-bundle globals, `any`, large orchestration files)* — known and tracked (Q4 ambient-globals design, @ts-nocheck lift queue); not a this-batch item.

**Gates:** lintDebug PASS (was 6 errors), 247 Kotlin tests, 3,628 vitest / 202 files, eslint 0/0, tsc, npm audit 0.

**Same session, later — the suite runs SILENT (NOISE-1), and the Lamb of God source PDF is back.** The 9 jsdom "Not implemented" lines in every full test run were root-caused, not muffled: (1) 7 canvas notices — platform-bridge's downscale/uniformity-probe canvases hitting jsdom's canvas-less stubs; vitest.setup.js now shims `HTMLCanvasElement.prototype.getContext/toDataURL` to return the SAME values jsdom returned (null/undefined) minus the log, so behavior is byte-identical (vitest's `onConsoleLog` was tried first and PROVEN blind to these — they're jsdomErrors piped straight to stderr; the config now documents that dead end). (2) A download-anchor notice — webSaveToFile/openExportSink click a real `<a download>`; a capture-phase listener preventDefaults download-attributed anchors only, so a real accidental navigation still logs. (3) The last notice took a stack-trace listener on `window._virtualConsole` (the Error carries the trigger stack vitest never prints — tip recorded in the setup file) and turned out to be a REAL test-hygiene bug, not ambiance: SettingsScreen's import-success `setTimeout(reload, 600)` outlives its test, and any restore of the real `window.location` (per-test afterEach or file-end afterAll) let the straggler timer fire jsdom's actual navigate() during a later test or env teardown. Fix: SettingsScreen.test.jsx stubs location per-test at FILE level and deliberately never restores (the env is discarded; the stub is the only state where a straggler is provably silent); ErrorBoundary.test.jsx got the same stub and its Reset-to-Home test now asserts the reload actually fires. Full suite: ZERO stray lines between RUN and summary, 3,628/202 green, coverage thresholds hold, eslint 0/0, tsc. Luna's other claimed noise (JSX-transform warnings, NaN CSS, IndexedDB reports) does not reproduce on the current checkout in either plain or coverage mode — older-snapshot artifacts. Separately, owner-requested: **`THELAMBOFGODstudy.pdf` restored** from trumpetcallofgod.com (linked on thevolumesoftruth.com) to `source-pdfs/` — 25,744,445 bytes, sha256 `06055e1a…` **byte-identical to the row the 2026-08-05 MANIFEST.json already carried** for the then-missing file; the 5 illustrations (Last Supper verified on render) are now extractable, and whether they enter the app remains the held owner decision (BACKLOG/memory).

### 2026-08-09 s6 — The Word of Promise: a second Bible edition, per-chapter tracks, two release tags

*(Backfilled — `712c997` landed without its detailed entry; written from the commit and the source it left behind.)*

**A second recorded Bible.** The Word of Promise — Thomas Nelson's dramatized NKJV, a 600+ voice cast — joins BRM's KJV: 1,189 per-chapter MP3s, ~5.5 GB, mirrored by a new `tools/mirror-wop-release.py`. It does not fit the shape the s3/s5 work assumed. BRM is ONE file per book; WOP is one file per CHAPTER, and 1,189 assets do not fit in a GitHub release, which caps at 1,000. So the edition spans **two tags** — `audio-wop-v1` (Old Testament, 929) and `audio-wop-v2` (New Testament, 260) — and the testament digit already present in the asset name (`wop1_` / `wop2_`) is what picks the tag inside `bibleAudioAssetUrl`. No lookup table, no per-book mapping: the name carries its own routing.

**The trust boundary stopped being a pair.** `audio-track.js` had grown from one release prefix to two by special case (letters, then `audio-bible-v1`); a third and fourth would have been a fourth special case. It is now a single frozen **`RELEASE_PREFIXES`** list, and `isVotAudioUrl()` finds the matching prefix then shape-checks the remainder against `ASSET_NAME` — the list IS the boundary, and adding a future voice is one line in it rather than a new branch in the validator. This matters more than it looks: the boundary is what keeps an imported `.votbak` from persisting an arbitrary URL into the saved-recordings shelf.

**One manifest part per chapter, without 1,189 written rows.** A per-chapter edition would have needed a manifest row per chapter — the generated file would have grown by three orders of magnitude on the critical path (`bible-audio-manifest.js` rides bundle-a). Instead the generator emits a ~1 KB **loop** that expands book + chapter-count into the parts array at load, `partLabel` "Chapter N". The manifest stays small; the runtime shape is identical to a hand-written one, which is what lets the player stay shape-driven below.

**`playBibleBook()` branches on SHAPE, not on edition id.** `parts.length > 1` means per-chapter, and "play chapter N" becomes a queue POSITION — `playCollection({ startPartIndex })`; otherwise it is whole-book and stays the s5 `BIBLE_AUDIO_CHAPTERS` seek. Nothing in the player names an edition, so a third edition of either shape needs no player change. `startPartIndex` advances the **forward-only horizon INTO the start item's parts** — the same rule s4 established for letters, now chapter-grained: it walks the run of queue entries sharing the start key and slices within it, clamped at `run - 1` so a chapter number past the end of a book lands on that book's last chapter rather than slicing into the next book. Settings needs no edit at all — it lists editions from `BIBLE_AUDIO_EDITIONS`, so the new voice appeared in the picker by registration. Prefetch DOES warm WOP chapter files (they are small, and warming them is the point) while still never warming a whole-book track.

**Corpus source verified before mirroring:** `D:/BibleAudio/wop-nkjv` — 66 books, 1,189 chapters, zip integrity OK, and the collected asset names checked 1:1 against the manifest's expansion, so a naming drift between the generator and the upload could not ship silently. Corpus c25→c26.

**Verified:** the gate contract ran with the commit — `dist/` bundles (a, b, d, e), `service-worker.js`, `search/cache.js`, and `tools/corpus-version.lock` all move with the source in the same commit, which is exactly what the build + corpus-version gates produce and what a split commit would have failed. New tests landed with the code: audio-player (edition-shape branch, startPartIndex horizon, clamp) and audio-track (the four-prefix boundary, both WOP tags, rejection of everything else).

### 2026-08-09 s5 — Bible chapter-level listen: a 1,189-chapter seek index into the whole-book tracks

*(Backfilled — `3a4247c` landed without its detailed entry; written from the commit and the source it left behind.)*

**The recording's shape decided the design.** BRM ships one MP3 per book, so "listen to this chapter" cannot be a track — there is no chapter asset to play. It is a **seek** into the book track. `BIBLE_AUDIO_CHAPTERS` (`"volKey:bookId"` → `[sec, …]`) joins the generated manifest in bundle-a (~14 KB minified — the whole index, on the critical path, because the pill must work before the 5 MB Bible corpus loads); `playBibleBook()` gained `chapterNum` and applies the offset on `loadedmetadata`, the same timing contract the boot-restore resume seek already uses (a `currentTime` written before that point is discarded), wrapped so an unseekable element falls back to the book start rather than throwing. `bibleChapterStart()` returns 0 — i.e. the top of the book — for chapter 1, an unknown book, or a chapter with no scanned row, so a gap in the index degrades to the old behavior instead of a wrong offset. BibleChapterView's hero Listen pill now starts at ITS chapter. Corpus c24→c25.

**The timestamps are the actual work, and they were not trusted.** Every chapter announcement was located by a faster-whisper scan (base.en, transcripts cached so re-runs were cheap) followed by gap repair — and then **every boundary was verified by at least one independent belt**: the announcement re-heard at the stamp, or the chapter's KJV opening verse matched there, or a human YouTube-comment timestamp (Jeremiah checked 46/46, all within 3.3s). 205 first-pass probe failures were arbitrated one by one rather than waved through: **166 were transcription false negatives** (the probe was wrong, the stamp was right) and **39 were real errors, fixed** — the worst class being a text-realign that matched a *neighbor chapter's tail*, which is precisely the failure a single-belt pipeline would have shipped as a confident wrong number. Final index: 1,189/1,189 rows, strictly monotonic within every book, all 66 books OK.

**Housekeeping caught in passing:** `audio-player.test.js` was rebuilt on main's s4 version plus only the new chapter-seek describes — a stale copy had briefly resurrected tests that s4 deliberately retired, which would have pinned behavior the session had just removed.

**Verified:** the gate contract ran with the commit — `dist/` bundles (a, d, e), `service-worker.js`, `search/cache.js`, `tools/corpus-version.lock`, and the generated eslint globals + `globals.generated.d.ts` all move with the source in the same commit (build, corpus-version, and lint:globals all leave exactly those tracks). New audio-player tests cover the seek path and `bibleChapterStart`'s three zero cases.

### 2026-08-09 s4 — The Listening Library campaign: reader choice, forward-only queues, home card, browse-to-recordings

Owner brief (ultracode session): make the Listening Library a real library — home-screen card instead of the Library/Volumes tiles, browse should reach RECORDINGS not text indexes, a letter with several readings should offer the choice ("if Volume 3 Letter 5 has a recording from benjamin, timothy, and text to speech… choose which one"), queues should never reach backward past the chosen start, saved recordings become a sub-menu, recently-played collapses, back pills everywhere — and "if you know a better way… do it. Quality comes first." Five commits landed, all pushed + deployed (`0f329c4`, `528e2c9`, `ad7ec21`, `8dfa015` + the session-opening rescue).

**Rescued the interrupted batch first.** The tree held ~630 uncommitted lines from a cut-off session (nav-origin `returnOrigin` chaining, study-history `st:` dedup keys + import normalization, hl-key-keyed annotation containers, AnnotationDomSync placeKey, the shelf-language AudioLibraryScreen visual pass, MainActivity `stopStreamingAudio()` through renderer crashes, achievements public-collection filter, Library milestone chip self-compute). Verified green across every gate (3,511 vitest, Kotlin, smoke) and landed as its own commit before anything else — no unreviewed work was built upon.

**Forward-only queues (owner directive).** `playCollection`'s startId now SLICES the queue at the chosen letter instead of positioning qi mid-list — the queue holds the letter and what follows, prev() clamps at the start; `playSection` and (via delegation) `playLetter`/`playBibleBook` inherit it. The persisted snapshot records the horizon (`startKey`/`startIndex`) so a reboot rebuilds the same forward-only queue; legacy snapshots keep the full rebuild once. RED-proven restore tests.

**Reader alternates end to end.** The manifest generator kept only the rank winner (B>T>V>M) per slot — every other reading was discarded at generation. It now composes per-reader renditions from the full candidate pool (same precedence rules per reader: full > sections > parts, addendum last; incomplete fragments skipped; same-reader duplicate uploads collapse) and emits **AUDIO_ALTERNATES**: 45 letters with a second complete reading, 48 assets (V×32, M×15, and one Benjamin reading that had lost to a Timothy full-letter under the full-beats-sections rule). All 48 mirrored to audio-v1 (**793/1000** — comfortably inside the cap, no new tag needed; the mirror's id-regex covers the new block unchanged). Corpus c23→c24 (+ the SRCH1 search-cache signature — the gate caught the split-commit attempt AND the forgotten sync, working as designed). Player: `renditionsFor()` (primary-first), `playCollection`/`playLetter` take `startReader`/`reader` (the chosen voice replaces the start letter; following letters keep primaries), persistence carries `startReader`, and the restore rebuild splices the alternate back in by url so resume stays on the recording actually being heard.

**Honest play counting (found by recon, verified in code).** `_start()` fed `AudioLibraryStore.recordPlayed` on EVERY track start — auto-advance, next/prev, playAt, restore — so the Milestones "recordings played" tier inflated by whole queue lengths on a single Play All. `plays++` moved to a new `countPlay()` called once per user-initiated entry point; the recent shelf still records every start. RED-proven both directions.

**The IA itself.** HOME gained the seventh card (eyebrow "Audio Readings" / "Listening Library" / "The Letters & Scriptures, read aloud" — `hni-*` anatomy, zero new CSS) routed through an origin-capturing `onOpenAudio` like settings/history; the LibraryScreen tile and VolumesHome tile retired. BOTH order stores were first made merge-tolerant in both directions (growth appends, removal drops in place) because the strict schema checks would have wiped every saved arrangement — the migration risk was closed before the schema moved. The hub reworked: now-playing deck kept, saved recordings became a doorway row into a new **audio-library-saved** screen (full shelf + the search field moved there), recently-played is collapsible (choice in localStorage, default open, >8 rows fold behind Show all), and Browse lists sources in two groups — the 14 VOT collections and the Bible editions registry (never gated on the lazy VOT corpus). A new **audio-library-collection** screen shows one source's every recording in reading order: Play all, WTLB compilation chips, numbered rows with parts/reader credits, per-letter "N voices ▾" disclosure playing the chosen rendition into the forward-only source queue, VOT-only text taps, an honest "K letters await recording" footer, defensive empty state on a stale volKey. Both new screens chain navOrigin through the hub (back unwinds level by level; hub labels stay origin-true: Home/Library/Volumes), Android-back branches added, describeTab entries, `audioColKey` tabField, text taps pushFromLetter naming their actual source screen. `hasTextDestination` now also requires the collection's `letterScreen` (recon found the icon could render while `onOpenTrack` silently no-opped).

**Type-scale gate grew a reference pass.** An undefined `var(--fs-N)` silently unsets the declaration — six phantom tokens (--fs-9/17/19/22/27/34, 12 references) had shipped inert in the audio-library + milestones CSS the day before. The gate now validates every token reference against the declared ladder (RED-proven on the 12, then all snapped to real steps). The two overlapping `.audio-library-*` CSS blocks (the scout's "dead block" was actually a load-bearing base layer under the redesign's overrides) were consolidated into ONE coherent block; dead selectors (`-now-mark`, `-collections`, the 460px media query, the VolumesHome tile variant) deleted.

**Bar/sheet coverage + three real defects.** AudioPlayerBar and AudioManagerSheet had zero component tests; a dedicated agent wrote 28 (driving the REAL player via FakeAudio; 5-mutation RED proof) and surfaced: (1) the bar advertised "Play" while buffering though toggle() pauses a loading element — the sheet said "Pause" for the same state; the bar now agrees; (2) unknown duration painted the disabled seek 100% full and the clock read "2:03 / 0:00" — now paints empty and omits the fake total; (3) the sheet claimed the modal registry + `__closeSheet` before deciding to render null — registry now gates on `open && hasCurrent`.

**Process notes.** Two Sonnet recon workflow fan-outs (6 scouts) mapped the architecture first; their reports were verified before acting — two claimed-unfixed defects (DST day-skip, autoscroll dwell floor) were confirmed ALREADY FIXED in code (adjudicated 08-04) and dropped, and the alternates count was re-derived by the generator itself rather than trusted. The big screens Opus agent stalled twice mid-implementation; its completed work (both screens, AudioShelf extraction, all nav wiring) was reviewed line-by-line and the remainder (CSS consolidation, tests, home card) finished inline. Smoke walk updated: the Library step now clicks by its "Personal Study" eyebrow (a bare /Library/ matched the new card) and a Listening Library step was added.

**Verified:** 3,574 vitest / 202 files (net +82 this session), lint 0 --max-warnings 0, tsc, type-scale, smoke:ci PASS (0 unreached incl. the new step), Kotlin 246 + assembleDebug, live Puppeteer walk screenshotted on every new surface with real corpus data (home card, hub, collection with the voices disclosure open, Bible edition, saved shelf), CI + Pages deploys green, APK rebuilt + installed to the test device.

**Same-day follow-up (owner):** the fourteen collections must not splay across the hub — the Volumes enter like the Scriptures do. The hub's Browse became one doorway per source family (The Volumes of Truth · N collections, plus each Bible edition); a new **audio-library-volumes** screen holds the collection list (availability sub-lines, corpus-warm), chaining navOrigin so home → hub → The Volumes → collection unwinds level by level (the collection pill says "The Volumes" when entered there). Full nav checklist applied (route, Android back, describeTab, UX3 array, four-deep chain test), the hub test pins the no-splay rule, smoke grew a "Listening → The Volumes" step. 3,579 vitest / 200 files, smoke PASS, screenshots verified, APK reinstalled.

### 2026-08-09 s2 — Whole-book Bible audiobooks: BRM KJV edition, Listen pill on Bible surfaces, Bible Audio setting

Owner ask: stream the BRMinistries audio Bible in-app like the letter tracks, plus a Settings toggle sized for future recorded editions. Research fan-out first (two workflows + agents): BRMinistries = Biblical Restoration Ministries (Sioux City, IA) — complete 66-book KJV, ONE whole-book MP3 per book (no per-chapter files), narrator uncredited; the corpus lives behind their site's Subsplash embeds, and every media page exposes a direct CDN MP3 (`cdn.subsplash.com/audios/JJW95G/{uuid}/audio.mp3`; media pages need a browser UA, the CDN doesn't). Downloaded all 66 to `D:\BibleAudio\brministries-kjv\` (~4.3 GB; 2 Kings is video-only on Subsplash — pulled the MP4, ffmpeg-extracted to MP3 @128k). A hosting investigation (GH Releases vs R2 vs B2 vs archive.org) came back **KEEP GitHub Releases** — documented no-limit release size/bandwidth, 2 GiB/asset (largest book 260 MB), proven Range/seek; the one live constraint is the now-enforced **1,000 assets per release**, so the Bible books ship on their own tag **`audio-bible-v1`** (audio-v1 already holds ~729 letter assets). R2 is the written-down contingency (~$0.08/mo + account) if GH ever throttles.

Implementation (isolated worktree, rebased onto the seven-front batch mid-session): **trust boundary** — `audio-track.js` now accepts exactly TWO release prefixes (letters + `AUDIO_BIBLE_RELEASE_PREFIX`); `bibleAudioAssetUrl()`, editions registry `BIBLE_AUDIO_EDITIONS` (`brm-kjv` → volKey `bible-brm-kjv`, label, translation) + `bibleAudioEdition()` resolver (own-property check — settings values are import-restorable), registry also published as a global for the classic-globals SettingsScreen. **Player** — `bible-*` volKeys route `_mapFor`/`_assetUrlFor` to `BIBLE_AUDIO_MANIFEST`/the bible release across hasAudio/prewarm/firstReaderCode/collectionHasAudio/_tracksFor; `playBibleBook()` queues the WHOLE edition positioned at the tapped book (the letters' album behavior); boot-restore rebuilds bible collections from `BIBLE_AUDIO_BOOKS` with zero corpus load; the gentle prefetch **never warms whole-book tracks** (a warm = a full 30–260 MB download). **Data** — `src/data/bible-audio-manifest.js` (generated, 66 rows + canonical book order w/ titles) rides **bundle-a** (~4 KB minified, deliberate: pill + resume must work on matthew-idx and Settings without the 5 MB a-bible load; Matthew is its own corpus and its ChapterIndex gets the pill via the same prop). **UI** — hero Listen pill on ChapterIndex (bible-idx + matthew-idx) and BibleChapterView's hero (plays the BOOK — no per-chapter splits exist); Settings → Reading → **Bible Audio** SelectField (editions from the registry + Off; default `brm-kjv`; unknown persisted values act as Off). **Pipeline** — `tools/gen-bible-audio-manifest.mjs` (validates 66/66 inventory + book ids against books.js) and `tools/mirror-bible-audio-release.py` (explicit audio/mpeg, idempotent; 66/66 uploaded). Corpus c22→c23.

Verified: 3,492 vitest / 194 files (new: release-URL policy pins, bible routing, playBibleBook queue/position, prefetch-skip, persisted-collection reboot restore), lint 0, tsc, smoke:ci both viewports, CI + Pages deploy green — then **proven on the LIVE PWA**: Jeremiah pill → bar playing, clock advancing (0:30→0:36 of 223:34 — exactly the source recording's 3:43:34), sub "KJV · Biblical Restoration Ministries". Note for future editions: CDN serves release assets as octet-stream for BOTH audio tags (the stored asset type is audio/mpeg); playback is unaffected — the letters release behaves identically and has streamed on-device since 08-06. License: recording rights holder unknown (ministry says "given the permissions to share"; their archive.org upload is CC0-marked) — owner accepts for personal use.

### 2026-08-09 — Seven-front owner batch: bookmarks, discoverability, milestones, listens, background audio, study annotation, study history

Seven owner-directed workstreams, one session; two Sonnet recon agents (study-surface annotatability maps) + one Opus implementor (native service); everything else inline. Every fix live-verified in the preview before landing.

**1. Chapter bookmark flooded every verse — fixed at the store.** `BookmarkStore.getForKeyPrefix`'s reverse clause (`prefix.indexOf(k + ':') === 0`) made one chapter-level bookmark (`bible:john:3`) match EVERY verse container beneath it — a flag icon per verse. Audit of all callers proved the clause served nobody (dom-bookmarks filters ancestors back out itself; journal uses descendants; ChapterBookmarkBtn uses exact getForKey), so it's gone; the pin test inverted with the rationale. Live: John 3 with a chapter bookmark renders **0** per-verse icons (was 36), header icon still highlights. Same fix retroactively covers entry-level bookmarks flooding WTLB paragraphs. **Plus the owner's ask: the chapter INDEX now shows a small gold flag** on any chapter holding a bookmark (chapter-level or any verse within — `bookmarkKeyFor` prop on ChapterIndex, wired for bible-idx + matthew-idx). Verified: exactly one flag, on the bookmarked chapter.

**2. Listening Library + Milestones now under Library.** The owner couldn't find saved recordings (they lived only behind the VolumesHome tile). LibraryScreen grew from 6 to 8 drag-reorderable tiles: **Listening Library** (live saved-recording count, bundle-b store via the runtime bridge) and **Milestones** ("N of 84 reached"). `DEFAULT_LIBRARY_ORDER` grew — the store's growth-tolerant get() appends new tiles to existing saved arrangements (pinned by updated tests incl. the two-generation migration). describeTab entries for both screens (+ the enumeration test rows).

**3. Milestones fleshed out: 84 achievements, 17 categories.** New `utils/achievements.js` — a PURE builder (`buildAchievements`) over a guarded live-store snapshot (`collectAchievementSnapshot`): words (10k→1M), readings finished, DISTINCT Scripture chapters (1→1,189 "Every chapter, Genesis to Revelation"), distinct letters/entries (→729 "The whole library"), re-readings, reading streak (2→365), lifetime active days, best single-day words, best rolling-7-day words (computed from the wordsByDay ledger), highlights, notes, bookmarks, links, journal entries, journal streak, recordings played, recordings saved. Chapter-vs-letter split rides the readItems key shape (numeric cid = chapter, slug = letter). Nothing persists — earned-ness is recomputed from data the app already accrues; the classic 10-def store strip keeps its unlock toasts and My Progress gains a "View all milestones ›" link into the new screen. `AudioLibraryStore` gained a bounded lifetime `plays` counter (normalized on import) to feed the listening tier. New **MilestonesScreen** (Library → Milestones): overall bar + per-category ledgers with progress bars on unearned rows. 12 new module tests.

**4. Listens increment read counts.** Owner: "marked as read should increment as many times as [one] reads/listens." Detector re-reads already incremented (count-valued readItems, 08-03); now a letter played TO ITS END does too: the player's 'ended' fires `__votAudioListened(volKey, letterId)` (last part only on multi-part letters; section compilations with key null never fire), and `useReadProgress` maps volKey→readKey via COL_BY_KEY, increments the same count the reading detector uses, and records a streak day. Respects the markAsRead gate; no stats-ledger record (that ledger holds MEASURED reading — words + visibility-honest time — and a listen is neither). 3 player tests + 4 hook tests; bridge live-verified (0→1 on the exact key).

**5. Background audio: native foreground media service** (Opus 5 implementor, adjudicated). `AudioKeepAliveService` — plain started service, `foregroundServiceType="mediaPlayback"`, minimal LOW-importance notification, no new dependencies, no POST_NOTIFICATIONS (no-nag policy; FGS runs regardless, notification may hide on 13+ until granted elsewhere). Wiring: the existing `setAudioActive` JS edge → AppInterface → new `BridgeHost.setAudioKeepAlive` verb (clearGardenCache pattern) → MainActivity → `ContextCompat.startForegroundService` / `stopService` (stopService, not ACTION_STOP — background `startService` throws on 26+ exactly on the screen-off pause path that matters). Extra stop owners: MainActivity.onDestroy + Service.onTaskRemoved so a swipe-away can't strand an undismissable notification. API branches: typed startForeground ≥29, FLAG_IMMUTABLE, int-overload stopForeground. The JS false-edge is confirmed complete: pause/stop/queue-end/sleep-timer all route through `_setStatus('paused'|'idle')` which fires it. Gates: 246 Kotlin tests 0✗ (7 new incl. Robolectric service wiring), assembleDebug ✓, merged manifest verified. **On-device checks owed** (no device attached): screen-off 10+ min under memory pressure, swipe-away cleanliness, 33+ notification behavior, repeat play/pause notification count, recording-vs-playback focus, API 34 typed-FGS path.

**6. Every study surface annotatable** (two Sonnet recon maps, then inline fixes). The owner's MTAM report: text in collapsible boxes selects natively but never raises the highlight toolbar. Recon proved the boxes are **ProphecyCard** (letter-format studies only; 132 groups, ~129k words) whose para/poetry branches lacked the imperative-path trio — and that the omission was documented only as a WORD-COUNT decision, never an annotation one. Fix: `data-hl-key={letterHlKey(stateKey, bi)}` + `data-hl-dom` + StaticSubtree on both branches (keys `letter:<chapterId>:<groupIdx>:<ot|nt|vot|intro>:<blockIdx>`), headings stay chrome, `data-read-seg` stays absent — annotation on, measurement unchanged. Live: 109 annotatable containers across 21 cards in MTAM ch. 1. Matthew Study Bible: verses were already annotatable (React sweep-line); the gaps were StudyPanels/InlineNotes commentary — non-interactive prose rows (commentary cites, un-tappable letter notes) now carry the same trio (`study:<book>-<ch>:panel-sN` / `study:<book>-<ch>:<verse>-sN|-vN`, suffixes keeping clear of the verse containers' bare keys); tappable rows remain navigation chrome (their excerpts are annotatable at the letters they open). Bundled adjacent fixes from recon: ChapterView's hardcoded `'study:matthew-'` bookmark key now derives from book.id; study-image captions annotatable (they're prose); BibleStudyIndex accordion gained `aria-expanded`; studyHlKey's misleading JSDoc corrected (it serves the Matthew reader; letter-format studies key via letterHlKey).

**7. Study chapters never recorded in History — root-caused after "several attempts."** The prior fixes all chased lazy-corpus timing; the actual hole was BRANCH ORDER in `useNavHistoryTracking`: `else if (letterId)` preceded the study branch, and letterId is never cleared on study navigation — so after any letter visit, every study visit fell into the letter branch (`COL_BY_LETTER_SC.get('bible-study-chapter')` misses → nothing recorded, study branch unreachable). RED-proven with a stale-letterId test, fixed by gating the letter branch on its real condition (`COL_BY_LETTER_SC.get(screen)`), plus a symmetric stale-studyId guard test. Live: the MTAM Preface visit sits in History as `study-chapter` directly above the stale letter entries that used to swallow it.

**Gates:** full vitest green after expectation updates (3,48x/194 files incl. ~30 new), lint 0 (500 globals), tsc, build + SW re-hash, smoke:ci, Kotlin 246 ✓, assembleDebug ✓. Owner heads-up honored: Bible-chapter audio may land from another session — the listen bridge no-ops on unknown volKeys rather than miscounting.

### 2026-08-08 — Listening Library + expanded listening desk (external contribution, reviewed + fixed)

**Provenance:** the feature body was authored by an external agent (ChatGPT) working uncommitted in the tree; this session reviewed every line against the repo's contracts, fixed four defects, verified the whole surface live in the preview, and landed it.

**What shipped (theirs, verified good):** the media player grew from a mini-player into a managed listening system, all on the ONE AudioPlayer singleton — no second engine, no parallel queue state in React. (1) **`utils/audio-track.js`** — trust-boundary primitives: `AUDIO_RELEASE_PREFIX` URL policy (`isVotAudioUrl` accepts exactly `<release>/<asset>.mp3`), `normalizeAudioTrack()` bounding every persisted/imported display string, `AUDIO_PLAYBACK_RATES` presets + `normalizeAudioRate()` float-tolerant snap. An imported backup cannot turn the app into a generic remote-audio loader. (2) **`stores/audio-library-store.js`** (bundle-b, IDB v8→v9 `vot-audio-library`, `{v:1,saved,recent,rate}`) — saved recordings (cap 100), recent history (cap 30), speed preference; URL-identity dedupe keeping the newest event; sort-before-dedupe on import; full backup export/import with nested re-normalization (`import-validators` + Settings mapping via the runtime bridge — bundle-d reaches the bundle-b store through `globalThis.AudioLibraryStore` at call time, never by import, which would fork the singleton). (3) **Player additions** — `rate` + `sleepEndsAt` in state; `skip(±s)`, `setPlaybackRate` (persisted via the library, playback never depends on it), session-only sleep timer (pauses, never stops — resume snapshot intact), queue editing (`playAt`/`moveUpcoming`/`removeUpcoming`/`clearUpcoming`, current track protected by design), `playTrack` for library rows (normalized-only), persist schema v2 with `mode:'custom'` carrying an edited queue verbatim across restarts (v1 records still readable); Media Session `setPositionState` + `playbackState` sync; a capture-phase document `play` arbiter so journal memos and VOT streams can never sound simultaneously; loading-state pause hardening (`_markPaused` — toggle/pauseIfPlaying no longer wedge a settling WebView play); Save-Data-aware prewarm. (4) **UI** — mini-player title area became a disclosure button opening **`AudioManagerSheet`** (portal bottom sheet, SheetHandle + focus trap + modal registry + `__closeSheet`, z 200/201 under toasts): save/star, seek slider with `aria-valuetext`, ∓15s, prev/play/next, speed radiogroup, 15/30/60 sleep chips, editable Up-next ledger. **`AudioLibraryScreen`** (`audio-library` route, tile on VolumesHome): now-playing strip, Saved, Recent (relative timestamps, Clear history), collection shelf into the existing Volumes hierarchy. Reduced-motion, hover-gated hover states, `--fs-*` ladder, 44px targets throughout. Docs in `docs/AUDIO-MANAGER.md`; ~70 new vitest cases across player/store/idb.

**Four defects found in review, fixed here:**
1. **Speed preset was silently lost on every new track on real devices.** `_start()` applied `el.playbackRate` BEFORE assigning `el.src` — the HTML media load algorithm resets `playbackRate` to `defaultPlaybackRate` on load, so 1.5× snapped back to 1× at each track boundary. Tests passed because the mock didn't emulate the reset. Fix: apply `defaultPlaybackRate` + `playbackRate` AFTER the src assignment (both in `_start()` and `setPlaybackRate()`); FakeAudio now emulates the spec reset so the regression is pinned; **proven live in the preview — effective rate ≈1.5 measured across a fresh track load** (media-clock delta vs wall clock).
2. **Library "Text" opens skipped reading-state wiring.** The `audio-library` route's `onOpenTrack` did `setLetterId`+`setScreen` only; every other letter nav also runs `setActiveReadKey('vol:'+volKey, …)`+`setLastReadForVol`. Reading dot / last-read / resume state would silently ignore Library-originated reads. Fix mirrors `colIdxProps`' nav; verified live (`activeReadKey='vol:one'`, lastRead updated).
3. **`describeTab` had no `audio-library` case** — tab cards, History, and the window title would all label the screen "Home" (the exact defect class the P1-8 comment in tabs.js warns about, and the enumeration test exists to force — the route just wasn't added to it). Fix: describeTab entry ("Listening Library · Saved & recent recordings") + the route added to the corpus-independent enumeration.
4. **Sleep countdown froze while paused** — the sheet's readout only re-rendered on playback ticks. One coarse 1s reducer tick while armed+open keeps it honest.

**Live verification (preview, clean SW/caches):** Volumes tile → Listening Library (empty states correct, title "Listening Library — VOTReader", screen restores from tab state); Play All Volume One → 31-track queue streaming; manager sheet: focus lands in-dialog, speed 1.5× applied + persisted, queue move/remove persist as `mode:'custom'` (customQueue intact in `vot-audio-pos`), +15s seek, sleep 15m armed → countdown live, Save → `is-saved` + store row; recent history newest-first; auto-advance on natural `ended` confirmed (a same-instant manual `next()` during the preface's final second explained an apparent double-advance — app behavior correct); mobile 375px: sheet full-width, tools stack 1-col, zero horizontal overflow.

**Gates:** 3,455+ vitest (191 files) all green, lint 0 (499 globals), tsc, build + SW re-hash, smoke:ci. Stray `kmath.js` (Khan Academy vendor file) found untracked at repo root — unrelated to this work, left untracked.

**2026-08-09, owner-directed (gentle queue prefetch):** with a volume queued, the player now quietly warms the **next two** queued tracks once the CURRENT track is fully buffered (`el.buffered` covers duration) on a healthy connection — a second, detached, never-playing `<audio preload='auto'>` element pulls each into the HTTP cache, chained one at a time, sliding-window as playback advances (`PREFETCH_AHEAD=2`). Measured live: a warmed release asset re-serves in ~65–103ms vs ~700–820ms cold (redirect + object both cached), so track boundaries start near-instantly and survive a brief network blip. Guards: Save-Data / 2g-class (`navigator.connection`) and offline skip entirely; a failed warm stays retryable (next healthy tick); 45s backstop timer; `stop()` releases the warmer; the one-audio arbiter never sees it (detached = no document events). `fetch()` prefetch was measured and REJECTED — `connect-src` deliberately excludes the asset hosts (CSP), and media-src is the honest lane anyway. Triggers piggyback existing listeners: the 1 Hz timeupdate tick + `progress` (covers warming-while-paused). 4 new vitest cases (warm chain + window slide, no-warm-while-buffering, Save-Data, retry + stop-release). Still streaming-only: no download manager, the HTTP cache owns eviction.

**Same day (2026-08-08), owner-directed follow-up (album behavior + disclosure cue):** (1) a hero **Listen on any letter now queues its WHOLE collection positioned at that letter** — `playLetter` resolves the registry globals (`COL_BY_KEY`/`colPreface`/`colLetterArr`) and delegates to `playCollection({startId})` (which already supported it), so the bar's prev/next appear immediately, walk neighboring letters, playback continues past the letter's end, and the persisted source becomes `mode:'collection'` (restart rebuilds the same queue at the same letter). Registry absent (tests, stripped harnesses) or letter not in the registry → single-letter queue exactly as before; the prev() restart-after-3s convention untouched. Pinned by a new vitest case (full-queue order, qi at the letter, prev into the neighbor, persisted mode). (2) The mini-player's title area — the bar's one non-obvious tap target — now carries a **gold-dim up-chevron disclosure cue** (`.audio-bar-more`, hover-brightens with the title; summary became flex row + text column, ellipsis intact). Verified live on the owner's exact scenario (Volume Two → "The Seventh Day"): 29-track queue at qi=1, prev restarts >3s in, next×2 lands on letter 4, persist `mode:'collection'` `key:'two:i-am-the-passover'`, chevron tap opens the desk at 375px with zero horizontal overflow.

### 2026-08-05 — Streaming audio letters (play buttons + queue player, Drive-backed)

**Scope (owner-directed):** the flock's Drive tree holds spoken recordings of the letters — a "Read by Benjamin" folder that must SUPERSEDE all other renditions (no duplicates), and per-collection folders for everything else. Insert a play button near the top of the reading hero (letter title / WTLB title), plus intuitive whole-volume playback and WTLB "part 1, section 1" playback. Tracks STREAM (owner call — offline play attempts toast "requires internet"). Per-track AND queued-album playback both work, matching the Bandcamp capability the owner named as the bar (Bandcamp itself has no stable direct-mp3 URLs — its per-letter external links stay).

**Stream host — the Drive dead end, then the GitHub mirror.** Drive direct streaming LOOKED viable (curl: HTTP 206, audio/mpeg, range support on `drive.usercontent.google.com`) and shipped first — then failed identically in headless Chrome, real Chrome, AND the on-device WebView: request completes, **zero bytes**, `MEDIA_ERR_SRC_NOT_SUPPORTED`. Root cause isolated with header bisection: **Drive 403s any request carrying `Sec-Fetch-Site: cross-site`** — hard anti-hotlinking (curl passes only because it sends no sec-fetch headers; `crossOrigin='anonymous'` doesn't help — the ACAO:* headers are a decoy over an unconditional 403). So the tracks are MIRRORED to a GitHub release — `votreader-assets` tag `audio-v1`, the Garden-images host family — via `tools/mirror-audio-release.py` (gdown download → uploads-API upload with an EXPLICIT `audio/mpeg` content type, because the release CDN sends `X-Content-Type-Options: nosniff` and `gh release upload` would store octet-stream; additive/idempotent, re-run after new flock uploads). Assets are named `<driveFileId>.mp3`, so the manifest needs no URL rewrites ever — `trackUrl()` is the single URL builder. Verified end-to-end: cross-site sec-fetch 206 ✓, headless playback ✓, and **on-device playback on the installed APK ✓** ("My Redeemer Lives", status playing, clock advancing, 540s duration). One observed WebView quirk — the session's very FIRST request can hang with zero progress while an immediate retry streams instantly — is covered by a one-shot 20s cold-start watchdog in the store (re-arms src once, then defers to the normal error path).

**Manifest pipeline.** `tools/fetch-drive-audio.py` (gdown, same technique as FlockSync) enumerates the public tree → `tools/_audio-drive-listing.json` (gitignored, regenerable); `tools/gen-audio-manifest.mjs` loads the corpus via vm (validate-schemas technique), parses `V{n}.{num}_{Title} (read by …)` names across prefixes V1–V7 / TLR / OFV(flock) / LFT(timothy) / WTLB / WTLB2 / TB2(blessed) / Holy-Days `NN_src_Title`, and emits `src/data/audio-manifest.js`: `AUDIO_MANIFEST` ("volKey:letterId" → `[[driveId, readerCode, partLabel?]…]`) + `AUDIO_SECTIONS` (wtlb1/wtlb2 → 7 range-compilation tracks each, "Part 3 · 40–59"). Drive ids double as mirror asset names (see stream-host block above). **Coverage: 729/729 — every letter, every preface, all 16 Holy Days entries.** Matching is staged (prefix+num verified by normalized title → title-index rescue for misnumbered uploads → "(Full Letter)" strip → "- Section N_…" strip → parenthetical strip → token-subset for dropped mid-title words), with: Benjamin supersedes wherever his file sits (reader rank B > Timothy > TTS > AI-with-music resolves every duplicate), full-letter tracks beat their per-section splits, "Addendum to X" files append as ordered parts (Christmas, Obey God), "0. ALL LETTERS" fills gaps only. Excluded by design: AI Songs (3,376), Gospel-of-John movie audio, Bible-Letter Studies tracks (span multiple study chapters), TSOT Matthew chapters, and 4 bonus tracks with no corpus letter (Amplified, Handmaidens of The Lord, Poured Out, My Anger Runs Deep) + the Garden intro reading — all reported by the generator, none shipped. Manifest rides **bundle-a-vot** (c19→c20; new build.py A_VOT member) so any screen that can show a play button has it loaded by definition; `CORPUS_CONTENT_VERSION` + search cache bumped in step.

**Player.** `src/utils/audio-player.js` — singleton store (subscribe/getVersion/getState), ONE lazy `<audio>` (`preload='none'`; `stop()` clears src + load() so the Drive connection actually drops), flat track queue with parts expanded, auto-advance on `ended`, prev() restart-after-3s convention, whole-second timeupdate notify guard, offline guard on every entry point + `error` (toast via the existing `.vot-toast`, `vot-toast-audio` id; mid-play failure keeps queue+position and `toggle()` re-arms src and seeks back), Media Session metadata + transport handlers (guarded/try-caught per Rule 6), and the Android keep-alive bridge. 36 vitest cases (FakeAudio + module-reset pattern).

**UI.** `AudioPlayerBar` (AppShellOverlays, self-subscribed, null when idle): fixed bottom mini-player — play/pause with reduced-motion-aware loading pulse, title + collection · reader + m:ss/m:ss, native thin-gold range seek (max floored at 1 — the aria-valuemax=0 class), prev/next only when queue > 1, close; `body.audio-bar-open` reserves `.screen-scroll` bottom room (inset-aware) and lifts the `.ascroll-pill`. `AudioPlayButton` (listen / playall / chip variants; reader tag only for the human readers) + `AudioSectionChips`. Hero pill sits under the ornament in LetterView + WtlbEntryView (inert swipe-peeks KEEP it — pixel parity, can't fire); "Play All" + WTLB section chips render from `colIdxProps` on all 14 index screens (Holy Days keeps its Bandcamp/YouTube playlist header alongside). Pinned-chrome type uses `--fsc-*`, pills `--fs-*`; z-index 150 (under sheets 200); no backdrop-filter.

**Native.** `AppInterface.setAudioActive` (@JavascriptInterface, binder-thread inline write) → `MainViewModel.streamAudioActive` (@Volatile) → `MainActivity.onPause()` skips `webView.onPause()` while playing, so screen-off/backgrounded listening survives (WebView.onPause halts HTML5 media). BridgeContractTest map + AppInterfaceTest updated; audio-player calls the bridge DIRECTLY (guarded) rather than via PlatformBridge — PlatformBridge's stateful module must not be duplicated into bundle-d (comment at all three sites). Voice-memo recording now calls `AudioPlayer.pauseIfPlaying()` (guarded window global — the sheet is Cluster B) before taking the mic.

**Plumbing.** CSP `media-src` = blob: + the three GitHub asset hosts (github.com + release-assets/objects.githubusercontent.com — mirrors img-src's Garden list). SW untouched: cross-origin requests already pass straight to network, so range requests stream unproxied. `smoke.js` WTLB probe regex tightened to `/Introduction/i` — the new "Part 1 · Intro–19" chip matched `/Intro\b/` first and stole the click (caught by the smoke gate FAILING, then re-proven green).

**Also this session (owner report, screenshot):** the ✦ ornament-divider now renders DIRECTLY under a standalone closing ("Says The Lord.") when that closing is the letter's final block — instead of below the footnote list; letters whose text ends mid-paragraph keep the old placement (LetterView `closingEndsBody`).

**Durable resume (owner directive, same session):** playback position survives app close AND phone restart. A throttled ~300-byte localStorage snapshot (`vot-audio-pos`) records the queue's source descriptor + the current track's display fields + the clock (written every ~5s of playback, on pause, on track advance, on seek). On boot the bar returns PAUSED at the saved spot with zero network/corpus cost (display fields render it before the lazy VOT bundle exists); the first transport tap awaits `__loadVotCorpus` if needed, rebuilds the real queue, lands on the saved track/part, seeks on `loadedmetadata`, plays. ✕ and queue-completion clear the snapshot. 4 vitest cases.

**Mirror operations note:** Drive rate-limits anonymous bulk downloads after a few dozen files (per-IP cooldown, gdown "Failed to retrieve file url"). `tools/mirror-audio-release.py --until-done` loops passes 20 min apart until all 745 assets exist — the detached worker outlasts the quota windows. Progress check: `gh release view audio-v1 --repo VOTReader/votreader-assets --json assets -q '.assets | length'` (target 745); log at `tools/_mirror-run.log`. Letters whose asset hasn't landed yet toast "Couldn't load this track" and heal automatically — no app update involved.

**Gates:** 3,433 vitest / 190 files, lint 0 (globals regenerated: 495), tsc, type-scale, check_balance ALL OK, validate-schemas 3,120 items 0 errors, corpus lock c20, smoke:ci PASS (desktop + 360×800), Kotlin testDebugUnitTest BUILD SUCCESSFUL.

**Orchestration note:** built with two Opus 5 subagents (store+tests, bar/buttons+CSS) against a frozen API contract, coordinator-verified line-by-line — both landed on-contract (agent deviations: globalThis manifest reads for typecheck-order immunity; label-in-name aria fix on the chips; both kept).
...
### 2026-08-04 (session 6) - The Matthew Study Bible was unsearchable + backlog [23]/[30]

**The P1 nobody was looking for.** Recon for backlog [29b] (search-hit depth) turned up something worth more than the item itself: `search/index-builder.js` looped over `mCh.sections` while `matthew.js` chapters expose `verses` **directly** - the file contains no `sections` key at all. So `mCh.sections || []` was always empty and **the Matthew Study Bible contributed ZERO documents to the search index**. A headline feature of the app had been entirely unsearchable.

Verified against the shipped corpus before touching anything: **28 chapters, 0 sections, 1,071 verses.** The reason a fully green suite never caught it is the sharpest part - the unit fixture carried a `sections` wrapper that production never had, so the test was written to the code instead of the data. The fixture was corrected first (RED), then the builder: verses-first, with the nested-sections branch kept as a fallback for any future Format-C shape. `MS_INDEX_VERSION` bumped `m1`->`m2` so every already-cached index is discarded rather than reused - without that, existing installs would keep a Matthew-less index forever.

**Live proof:** searching "YAHUSHUA" now returns `Matthew 1:1 - The Genealogy of YahuShua - "The book of the genealogy of YahuShua The Messiah..."` - 526 of the result cards reference Matthew. Before the fix: none.

**[30] STORAGE GROWTH - shipped.** Settings -> Your Data -> "Growth". The sentence is the feature ("Your data has grown by 412 KB across 7 measurements since 2026-07-28 - now 2.3 MB"); the bars are aria-hidden decoration, because a sparkline scaled to its own max flattens exactly the case that matters and reads as nothing to a screen reader. Samples ride the already-registered `meta` IDB store - no new CachedStore, so no `STORE_NAMES` entry and no `DB_VERSION` bump (the trap that hydrates `degraded` and queues writes forever behind a green suite). The load-bearing detail: `meta` is deliberately absent from `USER_DATA_STORES`, so the series can never inflate the number it trends - a new user-data store would have measured itself. Sampling rides the Settings mount effect where `measureUserData` already runs: one sample per visit, same-day overwrite, capped at 60, never a timer and never at boot.

**[23] READING MILESTONES - shipped (milestones half).** Ten defs over data the store already accrues (completions, words, rereads), thresholds set against the corpus's real scale (~1.09M words, so "100k words" is a genuine fraction of the library). `recordCompletion` now checks them and **returns** the newly-unlocked defs; `useMarkAsRead` toasts the first through the existing `jrnShowMilestoneToast`, so reading and journal milestones share one treatment rather than growing a second. My Progress gained a Milestones section that shows locked rows too, dimmed. Fire-once is the tested property: the persisted key list is the guard, so re-crossing never re-toasts and a `.votbak` restore does not re-fire - and `milestonesUnlocked` is listed explicitly in `replaceAll`, which rebuilds `_cache` from a field list where an omitted field is a field a restore silently wipes. **Not** in this session: the full month streak calendar; the 14-day strip still ships, and the backlog says so.

**Deferred with evidence, not skipped:**
- **[24] Year in Review - do not build yet.** `wordsByDay` only exists from **2026-08-03** (the store is IDB v8, shipped that day), so a screen headed "2026 in Review" would today describe about two days of reading. The backlog itself says "Seasonal; build near year-end," and that is the right call - the panels are otherwise fully derivable, so this is a timing decision, not a feasibility one.
- **[29b] search-hit depth - dropped, with reason.** 31,102 of 31,751 index docs are single-verse docs where depth is always 1, group-level counts already ship, and positional depth is unmappable because the builder flattens a letter to one string with no block offsets.
- **[29a] Matthew commentary-weight badge - still open**, now with real numbers: verse text 23,221 words vs votNotes 49,752 words = 2.14x commentary, ranging 0.00x (ch 2) to 8.43x (ch 24).

**Gates:** 3,397 vitest / 189 files, lint 0, tsc, type-scale, schema 3,120 items / 0 errors, build, smoke:ci.
