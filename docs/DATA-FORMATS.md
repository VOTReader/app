...
> Moved verbatim from CLAUDE.md on 2026-09-22, so the briefing that loads into every agent session stays short.
> The permanent data rules (ASCII hyphen in verse ranges, ASCII delimiters, decimal verse markers, `check_balance.py`
> after every batch edit) stay in CLAUDE.md, "Permanent rules". `npm run validate:data`
> (`tools/validate-schemas.js --strict`) is the executable form of these shapes.
