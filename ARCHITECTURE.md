...
**Copy / Share text (cp1, `utils/passage-copy.js`).** `passageCopy(range)` is the one serializer: a verse per line with its gutter number and a space ("37 On the last day"; a single verse is numbered only when its number was selected), a letter's paragraphs a blank line apart, a poem's lines (`<div>` or WTLB `<br>`) a line apart, `ANNOTATION_CHROME` and headings left out, then the reference line (`passageReference`: "John 7:37-39 (NKJV)" with the translation Settings shows, Matthew's `matthew-plain` included; "Matthew 5:3-5 (Study Bible)" for the study's own corrected text, which Settings does not change; "The Wide Path (Volume Two)"; Hidden Manna by title alone; the journal none, and its copy is left to the browser). It is ALWAYS there for reading text: a footnote / scripture sheet, a letter's footnote list and the Answers Ten Commandments sheet declare theirs (`data-copy-ref`, "(NKJV)" unless the reference names its own), and a reading page declares its own name for words outside its verse blocks (`data-copy-key` on the chapter body / letter page: a heading copies as "John 7 (NKJV)"). Share uses it with `{ numbers: false }` and puts the reference and link under the words. Every multi-block action (highlight, note, remove, copy) reaches only `readingBlocksIn(range)`: never an annotation icon's key and never the inert `.pager-peek` clones a select-all also spans.
...
While a recorded track is playing, the clause being read gets a soft gold wash and the page follows it. ONE component serves three surfaces (letters, WTLB-shaped entries, Bible chapters) off three generated corpora, behind two settings. Module header is the deep doc — this is the map.

**Data.** `src/data/audio-sync.js`, generated by `tools/batch-align.py` (DO NOT EDIT the output). Its OWN lazy file since **c41** (2026-09-01): it left bundle-a-vot because every reader who opened any letter was parsing ~400 KB of timings whether or not they ever pressed Play, and the bundle sat 140 KB from its ceiling. Fetched by `utils/sync-loaders.js` (`loadAudioSync()`, on index.html's `__makeLazyLoader`) the first time a letter recording is LOADED with the wash on; the module's `audioSyncStore` version is how the component learns it landed (the arrival must be a render input, or the first letter a reader listens to never paints). Classic-script `var AUDIO_SYNC`, so once loaded it is a window global like `AUDIO_MANIFEST`. **The path literal lives in that module and nowhere else:** the deploy (`tools/list-runtime-src-assets.js`) and the APK gate (`tools/check-apk-assets.js`) derive what ships by scanning `src/` — never index.html — for `src/data/` literals and `__makeLazyLoader(…)` calls, and `tools/check-corpus-version.js` unions that same scan into the corpus fingerprint, so a regenerated file always forces a CORPUS_VERSION bump. Served from the STABLE corpus cache (the SW's `isCorpusData` route, cache-on-use), deliberately not precached: audio streams online-only, so read-along cannot run offline.

```
AUDIO_SYNC["volKey:letterId"] = [[startSec, blockIndex, charStart, charEnd, partIndex], …]
AUDIO_SYNC_ALT[assetId]       = same shape, one timeline per alternate rendition
```
Sorted by `startSec`. `AUDIO_SYNC_ALT` is keyed by release ASSET because a letter can have several complete readings with different pacing; an alternate with no timeline of its own paints **nothing** rather than borrowing the primary's clock.

**THE OFFSET DOMAIN IS THE WHOLE GAME.** `charStart`/`charEnd` index the block's rendered `textContent`, and that string is defined in exactly one place — `utils/segment-dom-text.js`, which `ui/components/Segments.jsx` renders through and `tools/audio-fragments-lib.mjs` measures through. When those two disagreed by one character per segment boundary, the wash slid into the previous clause and resnapped at each block; 292 blocks shipped that way for months because **every gate checked that the row's BLOCK existed and none checked what the offsets selected inside it** (owner report 2026-08-12, closed 2026-08-26). Three consumers, one definition: the extractor that feeds the aligner, `tools/check-audio-sync.js`, and the QA renderer `tools/hone-sample.mjs` — which was itself rendering the drift until it was pointed at the shared library.

Poetry blocks need `lineBounds`: consecutive lines are separate `<div>`s whose `textContent` concatenates with **no separator**, so a legal span boundary there joins two word characters. Any boundary check that does not know this false-positives on every poetry line in the corpus.

**Format B** (WTLB One/Two, The Blessed, paragraph-shaped Holy Days entries) stores offsets in the **CORPUS** domain instead, because its rendered text is not its corpus text and the rendered one moves: emphasis markers are stripped, every soft line break renders as a `<br/>` contributing nothing (67.7% of paragraphs), a reference becomes either a footnote number or a parenthesised cite, and a nav link renders the book's TITLE — "Songofsolomon" until the lazy Bible corpus lands, "Song of Solomon" after. `utils/format-b-dom-text.js` projects corpus offsets onto whatever is rendered, once, at paint time, via the component's `offsetMapFn` prop. `charStart === charEnd === -1` remains as a legacy whole-paragraph sentinel.

**Bible** read-along is per-edition and verse-level: `src/data/bible-sync-<edition>.js` = `BIBLE_SYNC_<EDITION>[bookId][chapter]` — a positional array of integer CENTISECONDS, one slot per verse, `0` meaning "not proven, do not paint". `<edition>`/`<EDITION>` is the `BIBLE_AUDIO_EDITIONS` registry id (the aligner's own spelling, e.g. `web-ebible`) — **never** the `volKey` (`bible-web`): the two coincide for `brm-kjv`/`wop-nkjv` and diverge for WEB, which is why the filename and global are resolved through `bibleSyncEditionFor`/`bibleSyncGlobalFor` (`audio-track.js`) rather than sliced off the volKey (read-along-2, 2026-09-03 — a WEB file requested as `bible-sync-web.js` would have 404'd silently). ~184 KB per edition for all 31,102 verses, fetched by `utils/sync-loaders.js` (`loadBibleSync(volKey)`, which performs that same translation before building the path; also published as `window.__loadBibleSync` for the harnesses; it lived in index.html until c41 and shipped only by accident of translations.js's `bible-` prefix) only while a Bible recording is actually playing with the wash on. The unit is the WHOLE VERSE, and that is what makes it translation-proof: the app's own default pairs `brm-kjv` audio with NKJV text, and verse 5 is verse 5 in either — where character offsets would have to chase the restored-name editions' +6 characters per Name. `BibleChapterView` mounts the component with the verse NUMBER in the block-index column (stable across translations where a positional index is not) and requires the chapter on screen to be the chapter playing, since a book queues its whole remaining run.

**Unproven units still paint** (owner directive 2026-08-26). A clause the belt could not prove used to be dropped, so the wash skipped a sentence and resumed — which reads as the feature being broken. `_alignlib._interpolate_runs` now spreads a RUN of them across the gap between proven neighbours, weighted by token count; the previous per-row midpoint gave k consecutive misses the same timestamp, leaving k−1 unreachable. UNSPOKEN units — words the recording verifiably does not contain — are still never painted and consume none of the gap.

**Coverage** drifts every alignment run; ask the gate (`npm run check:audio-sync`) rather than trusting a number written here. Everything unaligned plays with no wash: the component finds no rows for the key and paints nothing. `tools/audio-sync-stale.allow` is the register of items still carrying pre-fix offsets — it may only shrink, and the gate FAILS if it names a key that has since started passing.

**The component** — `ui/components/ReadAlongHighlight.jsx` (bundle-d), mounted by `LetterView`, `WtlbEntryView` and `BibleChapterView` on the LIVE pane only (`!inert`: a swipe peek must not fight over the one global `::highlight` registration, nor write the live container's scrollTop). Renders `null`; it is an effect bundle. It subscribes to the `AudioPlayer` store, binary-searches the fragment for the current clock (with a 0.15 s lead — the eye should beat the ear), builds a `Range` over the block's text nodes with a TreeWalker, and registers it as `::highlight(vot-reading)` through the **CSS Custom Highlight API** (Chrome 105+; the floor is 108). **No DOM mutation** — annotations, selection and the read detector are untouched. Every give-up path CLEARS the wash: a row whose block is missing or whose offsets no longer resolve leaves the reader with nothing rather than the previous clause still lit while the voice moves on.

**Tap a clause to hear it.** The wash is a decoration with no box and no events, so a tap resolves from the POINT: caret → block → `textContent` offset → the fragment whose span holds it → `AudioPlayer.seek`. The exclusion selector must list the inline icons by class (`.verse-link-icon`, `.inline-bookmark-icon`, `.hl-note-icon`) as well as links and buttons: those are bare `<span onClick>` with no role, and their `stopPropagation` runs on the React synthetic event — dispatched from React's root long after the native click has already reached this component's native listener.
...
**Settings** (`hooks/use-settings.js`, both default **ON**, in Settings → **Reading**): `readAlongHighlight` gates the paint — and with it everything, since there is nothing to follow — and `readAlongFollow` gates only the scroll, so a reader can keep the wash without the motion. They reach the two reading views through `sharedViewProps` (`readAlongOn` / `readAlongFollow`), the way every other settings-driven reading behaviour does.

**How this is defended, in four layers** — each catches a class the others structurally cannot:

1. **`ReadAlongHighlight.test.jsx`** drives the real `AudioPlayer` through a fake media element with a hand-drained frame source, so the glide's timestamps are exact. The lease rules, both settings gates, the binary search at its boundaries, the mapper's silent `null`, the clear-on-unresolvable paths, the Bible key path and the Format B projection are each RED-proven. What it CANNOT see: it uses a hand-written fixture and a clock the test sets, so a timeline uniformly offset by three seconds passes every assertion in the file.
2. **`tools/check-audio-sync.js`** (pre-commit + CI, ~150 ms) asserts every shipped span starts and ends on a word boundary in the CURRENT domain. Deliberately not membership in a regenerated fragment set: correct-but-coarser rows from an older extractor would fail that at 66% and demand a GPU re-align of data that is already right, and a gate asking for that gets bypassed. What it CANNOT see: anything about time.
3. **`tools/e2e-readalong.mjs`** plays real audio in a real browser and asserts the comparison whose absence let the 2026-08-12 drift ship — `blockEl.textContent === segmentsDomText(...)`, the offline domain against the browser's. It serves cached bytes under the real release URL (CSP judges the URL, so it must not change) honouring Range requests, seeks row-by-row while paused rather than playing twenty minutes, and proves the wash RENDERS by reading back its own screenshot — every other layer passes in a world where `::highlight(vot-reading)` was deleted from the stylesheet.
4. **`tools/smoke.js`'s Read-along wiring** step checks the contract in the real app without playing anything, and now checks the offsets too rather than only that the block exists.

`format-b-dom-text.test.js` is the Format B analogue of layer 2 and 3 combined: it renders every Format B paragraph in the corpus through a real `WtlbEntryView` in both footnote modes and demands the projection match exactly, then proves every extractor fragment, projected onto that render, selects a whole-word run of its own words.
...
**Registry** — `BIBLE_AUDIO_EDITIONS` (`audio-track.js:61`) maps a `settings.bibleAudio` value to `{ label, short, translation, volKey }`; `bibleAudioEdition()` (`:121`) resolves it with a `hasOwnProperty` guard (settings values are import-restorable, so `'toString'` must not resolve). It lives in `audio-track.js`, not the lazy manifest, so Settings can list editions before any corpus lands, and is published on `globalThis` for the classic-globals Settings screen (`:116`). Every `volKey` starts `bible-`, which is the single prefix test the player branches on. The registry KEY — never `volKey` — is what names the shipped sync file and its global (§22 above): `bibleSyncEditionFor(volKey)` / `bibleSyncGlobalFor(volKey)` are the only sanctioned translation between the two, and `audio-track.editions.test.js` gates the id set and each `volKey` against `tools/batch-align-bible.py`'s own `EDITIONS` so the two lists cannot drift apart again.

### 23.3 playBibleBook — branching on SHAPE, never on edition id

`playBibleBook()` (`audio-player/queue.js`) takes `{ volKey, bookId, label, chapterNum, noResume }` and does two things:
...
`playCollection()` (`audio-player/queue.js`) is the album engine both corpora share, and its contract is **forward-only** (owner directive 2026-08-09): `startId` slices the built queue at the chosen item, so nothing behind it is ever queued and `prev()` simply clamps at the start. A reader stepping backward past where they began is disorienting; the horizon persists, so it survives a reboot too.
...
**Write rules** (`audio-player/persist.js` `_rememberPosition`): a ≥1/s throttle with a `force` bypass at deliberate boundaries, and a **uniform 30 s floor** (`AUDIO_RESUME_MIN_SEC`) applied on the WRITE path since 2026-08-10 — including the forced stop/pause writes. A position under 30 s can never resume, so storing one files a row that means exactly what no row means while spending an LRU slot; skipping chapters through a book used to file a dead row per chapter and evict the real places the listener left. The same test covers a zero clock, which matters because `_start()` sets `_state.time = 0` before metadata lands.
...
- `countCompletion()` (`:264`) — recordings heard to their END, fired from `_countCompletion()` (`audio-player/credit.js`) at the same moment the read credit is granted, so it counts WHOLE recordings (a multi-part letter counts once, when its last part ends).
...
A recording played to its end counts like a read (owner directive 2026-08-09). `_notifyListened()` (`audio-player/credit.js`) fires from `'ended'` BEFORE `next()` advances; range-compilation sections carry `key: null` and never notify. **Two completion grains, because the corpus has two:**
...
**Out** — `_syncNative()` (`audio-player/media-session.js`) pushes `(title, artist, isPlaying, position, duration, rate)` through `AndroidBridge.setAudioNowPlaying` (`AppInterface.kt:150`) to `AudioKeepAliveService`'s `MediaSessionCompat` + MediaStyle notification. It is **edge-driven**: called from `_mediaSession`, `_syncMediaSessionState` and the transport edges, and deliberately NOT from `_syncMediaSessionPosition`, which runs at 1 Hz off `timeupdate` (the call site says so). The card's second line is `_cardArtist()`: a Bible chapter names its EDITION, because those tracks carry no reader and all three editions would otherwise read "The Volumes of Truth" alike.

**Back** — the service's `commandSink` (`AudioKeepAliveService.kt:360`, wired in `MainActivity.kt:506`) forwards `(cmd, posMs)` over `JsBridge` as `window.__votMediaCommand` (`JsEvent.kt:56`), received by `_installNativeTransport()` (`audio-player/media-session.js`). `pause` goes through `pauseIfPlaying()` and `play` resumes only when paused; only the notification's own button sends `toggle` (v02-audio-02, 2026-09-24: the platform MediaSession sends onPause for a media key, the Assistant, Wear or a car head unit whatever the state, so the old everything-through-`toggle()` started playback on a Pause while paused). A bad command can never crash the player.
