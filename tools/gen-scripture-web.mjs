...
// The count the tour says (w-tour-copy, 2026-09-11): the connections shown at the default density,
// by the SAME law the screen's counter runs (ScriptureWebScreen graphStats — Famous is the sum of
// every bucket's off10). Written here beside the data so the sentence can never be typed and can
// never drift from the picture: one generator run writes both. An ES module under utils/ (not
// src/data/, which the corpus gates and the lazy loaders own) so bundle-b can import it.
const famousCount = layout.buckets.reduce((n, b) => n + b.off10, 0);
const countPath = resolve(DATA, '..', 'utils', 'scripture-web', 'famous-count.js');
writeFileSync(countPath, `/* famous-count.js — GENERATED by tools/gen-scripture-web.mjs. DO NOT EDIT.
   ...
   node tools/gen-scripture-web.mjs */
export const SCRIPTURE_WEB_FAMOUS_COUNT = ${famousCount};
`);
console.log(`wrote ${countPath} (famous count ${famousCount})`);
