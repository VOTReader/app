...
import { readerFromFilename } from './audio-renditions-lib.mjs';
...
const NL = String.fromCharCode(10);   // a literal newline, safe inside the emitted template
const BOOKS66 = [
...
const EDITION_ROWS = Object.entries(BIBLE_AUDIO_EDITIONS).filter(([, e]) => !e.driveFolder).map(([id, e]) => {
  if (!e.assetPrefix) throw new Error(`edition ${id} declares neither assetPrefix nor driveFolder`);
...
/* DRIVE-SOURCED editions. An edition with a `driveFolder` takes its asset ids
   from the archive listing rather than from a <prefix><testament>_<book>_<NNN>
   naming rule, and declares its release tag instead of encoding it in the name
   (Architect section 11). Rows are written out explicitly — there is no rule to
   expand — so this is the one part of the manifest that is data, not a loop. */
const DRIVE_ROWS = [];
for (const [id, e] of Object.entries(BIBLE_AUDIO_EDITIONS)) {
  if (!e.driveFolder) continue;
  if (!Array.isArray(e.books) || e.books.length !== 1) {
    throw new Error(`edition ${id}: a driveFolder edition must declare exactly one book`);
  }
  if (!e.releaseTag) throw new Error(`edition ${id}: a driveFolder edition must declare its releaseTag`);
  const listing = JSON.parse(fs.readFileSync(path.join(HERE, '_audio-drive-listing.json'), 'utf8'));
  const bookId = e.books[0];
  const count = CHAPTER_COUNTS[BOOKS66.findIndex(([b]) => b === bookId)];
  // Case-INSENSITIVE on the extension: exactly one file in 5,151 is ".MP3"
  // (TSOT_Matthew-Chapter-001.MP3), and a case-sensitive match drops chapter 1
  // and reads like a gap in the archive.
  const re = new RegExp('Chapter-(\\d{3})\\.mp3$', 'i');
  const found = new Map();
  for (const f of listing) {
    if (!f.path.startsWith(e.driveFolder)) continue;
    const m = re.exec(f.path.split('/').pop());
    if (!m) continue;                       // a non-chapter file in the folder
    const n = parseInt(m[1], 10);
    if (found.has(n)) throw new Error(`edition ${id}: two files claim ${bookId} chapter ${n}`);
    // The reader is in the FOLDER, not the filename — pass the full path.
    // readerFromFilename('TSOT_Matthew-Chapter-001.MP3') returns V, which is
    // simply wrong; the full path finds "(read by ...Bejamin...)" and returns B.
    found.set(n, [f.id, readerFromFilename(f.path), 'Chapter ' + n]);
  }
  const missing = [];
  for (let c = 1; c <= count; c++) if (!found.has(c)) missing.push(c);
  if (missing.length) {
    throw new Error(`edition ${id}: no audio for ${bookId} chapter(s) ${missing.join(', ')} `
      + `— ${found.size} of ${count} found under ${e.driveFolder}`);
  }
  const rows = [];
  for (let c = 1; c <= count; c++) rows.push(found.get(c));
  DRIVE_ROWS.push([`${e.volKey}:${bookId}`, rows]);
  console.log(`  ${e.volKey}: ${rows.length}/${count} chapters of ${bookId} from the Drive listing `
    + `(readers ${[...new Set(rows.map((r) => r[1]))].join('')})`);
}
...
/* DRIVE-SOURCED editions: written out, because their asset ids are the
   archive's Drive ids and there is no naming rule to expand. The release tag
   comes from the edition declaration (BIBLE_AUDIO_EDITIONS[...].releaseTag),
   not from the asset name. */
${DRIVE_ROWS.map((r) => 'BIBLE_AUDIO_MANIFEST[' + JSON.stringify(r[0]) + '] = ' + JSON.stringify(r[1]) + ';').join(NL)}
`;
