...
# Per AUDIO_SYNC key, the asset ids its rows were aligned against (the belt's
# `parts`). Committed, because the belts are not: check-audio-sync.js compares
# it to AUDIO_MANIFEST so a manifest run that changes a letter's PRIMARY
# recording goes red instead of playing the new voice under the old timings.
PROVENANCE = os.path.join(BASE, "audio-sync-provenance.json")
...
def provenance_for(sync):
    """key -> the asset ids the shipped primary rows were aligned against.

    Read from each key's belt (`parts[].asset`, one per part index the tuples
    carry). A key whose belt is gone -- carried forward after a crash or a
    memory kill -- keeps what the previous sidecar said; a key nothing can vouch
    for is None, and check-audio-sync.js fails on it rather than guessing.
    one:and-he-shall-be-called-i-am shipped a month of Benjamin's voice under
    text-to-speech timings because nothing recorded which recording a
    letter-keyed timeline belonged to; this is that record.
    """
    prev = {}
    if os.path.exists(PROVENANCE):
        try:
            prev = json.load(open(PROVENANCE, encoding="utf-8")).get("primary", {})
        except ValueError:
            prev = {}
    out = {}
    for key in sorted(sync):
        ...
        if os.path.exists(path):
            try:
                parts = json.load(open(path, encoding="utf-8")).get("parts") or []
                assets = [p.get("asset") for p in parts if p.get("asset")] or None
            except ValueError:
                assets = None
        out[key] = assets if assets else prev.get(key)
    return out


def write_provenance(sync):
    prov = provenance_for(sync)
    body = {
        "note": ("Auto-generated by tools/batch-align.py ship(). DO NOT EDIT. primary[key] = the "
                 "asset ids (one per part) the AUDIO_SYNC rows for that key were aligned against, "
                 "read from the belt; null = no belt vouches for the rows. check-audio-sync.js "
                 "fails a key whose entry is null or differs from AUDIO_MANIFEST[key]."),
        "primary": prov,
    }
    with open(PROVENANCE, "w", encoding="utf-8", newline="\n") as f:
        json.dump(body, f, indent=0, sort_keys=True)
        f.write("\n")
    unknown = sum(1 for v in prov.values() if not v)
    print(f"audio-sync-provenance.json: {len(prov)} keys, {unknown} with no belt to vouch for them")
    return prov
