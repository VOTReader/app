...
>
> **KNOWN LATENT BUG (surfaced during P5c verification, NOT caused by P5c):** `WtlbEntryView.js:33` throws "Cannot read properties of undefined (reading 'useMemo')" 7 times on initial mount, even when the active screen is a LetterView (not WtlbEntryView). Reproduces on the pristine pre-P5c state too. `React` is defined globally, `React.useMemo` is a function, and no local shadowing exists in the file — root cause not yet diagnosed. App still renders correctly; ErrorBoundary swallows the throws. Worth a dedicated investigation when P6 begins.
>
