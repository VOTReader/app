...
**Latest landed (2026-08-04):** the `63562d0..4a6a7f3` handoff received an adversarial whole-app review and nine corrective commits (`e4c9f2a..b750a33`). Backup restore now has symmetric 16 MiB manifest limits, token-owned restore guards, and a non-destructive fallback when legacy journal media is absent. Reading frontiers, dwell, scroll restoration, word weighting, manual marks, compact progress metadata, and real-browser resume were reconciled. Keyboard/focus/dialog coverage now reaches the remaining action sheets, sidebars, popovers, Safari notices, journal interactions, and app confirms; the wide companion rail remains correctly non-modal. Mobile navigation no longer clips at 360px, and `smoke:ci` now locks compact-toolbar geometry as well as desktop overflow. Coverage exposed and pinned a delayed index-scroll callback surviving navigation. Live visual QA covered core flows at 360×800 and 1920×1080. Gates: 3,356 vitest / 188 files + JS coverage, 237 Kotlin / 12 files + JaCoCo, lint 0, tsc, smoke:ci, e2e:read, schema/corpus/assets. Full detail in HISTORY.md.
