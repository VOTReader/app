...
**Open — TWO THINGS STILL NEED THE OWNER'S CALL (raised 2026-08-27, not yet decided); the first of the original three landed as c41:**
1. ~~Lazy-load the letter timings~~ **Landed 2026-09-01 (c41).** `audio-sync.js` is its own lazy `src/data/` file, fetched by `utils/sync-loaders.js` the first time a letter recording plays with the wash on (the Bible loader moved into the same module). bundle-a-vot 2.66 → 2.26 MB and re-baselined; the deploy, APK and corpus-version gates all derive the file from the loader module, so it cannot silently stop shipping or be pinned stale (ARCHITECTURE.md § 22).
