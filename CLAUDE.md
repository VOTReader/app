...
- **Polish pass — 2026-05-28 (NEARLY DONE; HANDOFF).** All committed + pushed except where noted. Landed: **#5** `.gitattributes` (LF-normalize `* text=auto eol=lf` + binary protection incl. gradle-wrapper.jar; `c70ecfd`) · **#4** SW same-origin fetch passthrough + **#3** deploy-web `paths:` filter (`4158583`) · **W4.6** `useDocumentTitle` (reuses `describeTab`; `7912bc6`) · **W4.2** `useDesktopKeyboard` (`/`+Ctrl/Cmd+F → `window.__goSearch`, Left/Right click `.chapter-nav-sticky-arrow`; web-only; `1886c13`) · **W4.3** inline-ref `:hover` for `.inline-scrip-ref`/`.letter-link-ref` (`8fc2291`). **Counts now: 30 App() hooks, 1260 tests/52 files, app.jsx 774/800.** New hooks (`useDomAnnotationSync`, `useKeyboardInset`, `useDocumentTitle`, `useDesktopKeyboard`) are wired in `_entry-b.js` (import + `Object.assign(window,…)`) and globalized for app.jsx; each has a test.
  - **REMAINING (the only open work): W4.4 right-click context menu + W4.5 mouse-drag selection.** KEY FINDING — likely **verify-only, no new code**: `src/ui/sheets/SelectionToolbar.jsx` (~line 228) already registers `selectionchange` + `pointerdown` + `pointerup` (unified mouse+touch, so mouse-drag selection should already raise the toolbar) + `touchend` + **`contextmenu` (onContextMenu)**. Next session: read `onPointerUp` + `onContextMenu` in that file, then VERIFY in preview (desktop viewport: drag-select text → toolbar appears = W4.5; right-click selected text → inspect onContextMenu = W4.4). Only add code if a real gap exists. Then mark W4.4/W4.5 + close W4 in PLAN.txt §W4 exit criteria, and update this bullet.
  - **Verify cadence for new hooks:** preview clean-slate is required to load fresh bundles (the SW caches them) — `(async()=>{for(const r of await navigator.serviceWorker.getRegistrations())await r.unregister();for(const k of await caches.keys())await caches.delete(k);location.reload();})()`. Watch CI via `& "C:\Program Files\GitHub CLI\gh.exe" run watch <id> --exit-status` (gh installed+authed as VOTReader). Coverage floor 55/46/60/60 — new hooks in `hooks/` need a test or they erode the branches margin.
