...
> - **NOW**: **P6 — `App()` hook extraction** IN PROGRESS. Landed so far: `useMarkAsRead` (P5d warmup), `useSavedState`+`_validateTabState` (P6a), `useRefMirror` (P6b), `useHistory` (P6c), `useThumbnails` (P6d), `useScrollMemory` (P6e), `useReadingDwell` (P6f), `useSettings` (P6g), `useSheetOrchestration` (P6h), `useFromLetterStack` (P6i), `useNavigateToLink` (P6j), `useTabs` (P6k Commit A), `useTabActions` (P6k Commit B), `usePersistedState` (P6k+1). Remaining per PLAN.txt §P6 revised order: P6l `useAndroidBack` — the last one. Go ONE hook at a time; smoke after each; one hook = one commit.
>   - **POST-P6 HOOK CALL-ORDER DAG (placeholder — refine through P6l):** `useSavedState` → `useTabs` → [`useSheetOrchestration`, `useThumbnails`, `useScrollMemory`, `useReadingDwell`, `useSettings`, `useHistory`, `useFromLetterStack`, `useNavigateToLink`] → `useTabActions` → (`useAndroidBack`, P6l) → `usePersistedState`. The order is load-bearing: `useTabs` must precede everything that reads `tabField`-derived state; `useTabActions` must follow `useReadingDwell`/`useThumbnails` (it needs `cancelDwell`/`setTabThumbnails`); `usePersistedState` is last (it sinks the union). Do NOT reorder hook calls without checking this graph.
> - **`App()` LINE COUNT (tracked per extraction — record the real `wc` number in every P6 commit message)**: post-P6k+1 = **2,241 lines** (`function App()` at index.html:1000). Progression: ~2,815 post-P6d → 2,735 → 2,693 → 2,641 → 2,516 post-P6h → 2,461 post-P6i → 2,398 post-P6j → 2,323 post-P6k-A → 2,246 post-P6k-B → 2,241 post-P6k+1 (all measured). The "~2,500" figure in pre-P6d docs was a drifted estimate — measure, don't guess. **REALISTIC P6-COMPLETE TARGET ≈ 1,400–1,600 lines, NOT ~400.** The "~400" in older docs was always wrong — App()'s render tree alone is ~1,000 lines, plus nav-helper glue (~150) plus hook composition; PLAN.txt §P6 itself says "~1,400 left in App()". After P6l the realistic floor is ~2,150+ minus whatever of `handleAndroidBack` + nav helpers actually moves. NOTE: the vot-state PERSIST sink became `usePersistedState` (P6k+1) — it takes 8 params, of which only 4 are hook returns (tabs/activeTabIdx from useTabs, activeReadKey from useReadingDwell, settings from useSettings); the other 4 (theme, lastReadChapters, lastReadLetterMap, readItems) are still plain App() useState that no cluster owns.
...
> - `App()` — **2,241 lines** (`function App()` at :1000), **the only big inline component left, P6 target for hook extraction** (P6a-k + P6k+1 extracted 14 hooks; only P6l `useAndroidBack` to go)
...
>
> **NEXT: P6l — `useAndroidBack`.** The last P6 hook. P6k landed in two commits — `aaef0f8` (Commit A, `useTabs` core: tab state machine + `tabField` + the cached per-key setter identity / invariant 1 + the stability probe) and `70d646a` (Commit B, `useTabActions`: the open/close/switch operations, a late hook taking `cancelDwell`/`setTabThumbnails` as plain params) — then `4714a92` consolidated the persistence sink into `usePersistedState` (P6k+1). All landed clean: smoke PASS, invariant-1 probe silent, targeted tests PASS.
> P6l (`handleAndroidBack` → `src/hooks/use-android-back.js`, PLAN.txt §P6 Cluster 8) is **NOT a clean extraction**. P6k resolved all of `handleAndroidBack`'s state/setter deps to hook returns — but the handler still couples to ~7 App-local nav-helper functions (`goNavOrigin`, `goHome`, `goSearchOrigin`, `goScripturesHome`, `goVolumesHome`, `goStudiesHome`, `goJournalViewer`) plus `getStudyById`/`journalEntryId` and ~7 `useRefMirror` refs. Expect P6l to be a **partial** extraction (the effect + ref wiring moves; the nav helpers thread in as a wide param object), OR a P6l-prep commit that extracts the nav helpers first. Do the recon before committing to a shape.
>
