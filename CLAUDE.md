...
- **W2 storage hardening — W2.1 → W2.3b LANDED 2026-05-27.** 12 commits, range `16b8fbd..339944f`. Every byte of structured user data now in IndexedDB (database `votreader`); legacy localStorage keys read once via per-store fallback then converge to IDB. **W2.1** (`16b8fbd`) — `src/stores/idb-adapter.js`, generic CRUD wrapper with retry-on-AbortError + QuotaExceededError preservation + versionchange handling + 44 vitest cases (fake-indexeddb). **W2.2** (`bea5877`) — `CachedStore` extended with state machine (`pending`/`loaded`/`degraded`), write-queue REBASE on hydration, `_pendingCache` overlay so reads during pending show user writes immediately, `_shouldDefer` guard pattern for mutation methods, batched single-`_save`+`_bump` after replay (per [[w2-hydration-data-loss]]). **W2.3** (`c49d658` → `90e64b2` → polish `bdc479e`) — all 10 CachedStore-backed stores opted into `idb:true`: recent-nav (Tier 1) + bookmark + notebook + journal × 4 (Tier 2) + annotation + note + link (Tier 3). LinkStore's legacy `{a,b}→{source,target}` migration split into `_normalize()` with post-hydration subscriber. HydrationGate component at `src/components/HydrationGate.jsx` gates `<App>` on `hydrateAllStores()` (Promise.allSettled over the IDB-store registry); shows a centered "VOTReader" splash for the (single-digit-ms) wait. **W2.3b** (`cd88255`/`f8df8bd`/`339944f`) — 6 of 7 hook-owned direct-LS keys migrated: WelcomedFlagStore + AboutSeenFlagStore + GardenWarningFlagStore + ProphecyCardsStore + StateStore (with `lsShim` for boot-script sync read of theme + fontStyle) + HistoryStore (useHistory refactored to `useSyncExternalStore`). 7th key `vot-ann-migrated` stays in LS as a permanent boot-time exception (legacy annotation-migration gate read at module load). **Hydration latency** measured via `performance.mark`/`measure` in HydrationGate: 3–7 ms across all 16 IDB stores on preview machine (target was <200 ms mid-range, <500 ms budget). **Tests**: 628 → 720 (+92). **Bundle delta**: bundle-b 357 → 390 KB (+33 KB for the entire W2 infrastructure). **W2.4 unblocked** (LS-clearing pass; skip list: `vot-state` keeps its reduced shim, `vot-ann-migrated` stays for the boot gate). W2.5 (persist + quota) / W2.6 (export v2 + alert→toast + realistic round-trip + V1↔V2 forward-compat) / W2.7 (StorageHealth dynamic warnings) / W2.8 (App ErrorBoundary) pending. **Legacy-LS migration is end-to-end verified** in preview-eval for all 16 stores: pre-W2.3 LS data is transparently seeded into IDB by `CachedStore._hydrate`'s legacy-LS-fallback path on first boot post-W2 deploy — no separate one-time migration step required.
- **W2.6 scope expansion (carried forward in PLAN.txt)**: the W2.6 agent already touching `exportPersonalData` + `importPersonalData` for the media-blob upgrade MUST also (a) migrate the 4 alert() sites to in-app toast (jrnShowMilestoneToast pattern) — they block the renderer on every desktop platform; (b) seed at least 50 annotations + 10 bookmarks + 5 journal-with-media entries before testing the round-trip (W1.6 verified at toy scale only); (c) make the v2 import handler explicitly skip unknown top-level keys so a v2 file imported into an older v1-speaking client doesn't silently store data into localStorage as wasted quota. All three items live in PLAN.txt W2.6 boxed callouts.
...
- **W0-W8 cross-platform PWA + quality hardening plan** — Phases: W0 prereqs (fonts ✅ + owed NK7 device walk) → **W1 PlatformBridge ✅ CLOSED** → W2 IndexedDB migration (next) → W3 PWA shell → W4 desktop polish → W5 hosting (GitHub Pages) + dual CI → W6 cross-platform verification (Chrome + Edge + Firefox + Android) → W7 quality hardening (IDB schema versioning, raw() freeze, hlTick dead-prop removal, JS DiagnosticLog, 47→5-prop bundle refactor) → W8 type coverage + CSS @layer. Full per-phase exit criteria + 18-critique traceability matrix in PLAN.txt.
...
- **7 cluster bundles** in `app/src/main/assets/dist/`:
  ...
  - `bundle-b.js` 357 KB — stores + components + hooks + journal + scripture-resolution + letter-linking + W1.5 modal registry + history-sync + W1 platform-bridge (esbuild IIFE)
  - `bundle-c.js` 27 KB — renderer (esbuild IIFE, 3 files)
  - `bundle-d.js` 545 KB — screens + sheets + components + utils + late stores + screen-routes + App() + W1.5 AppShell registrations (esbuild IIFE)
- **628 vitest tests** across 25 files (+33 across W1).
