...
- **D5 per-action write-fail toast** — _in progress this session; see HISTORY when landed._
- **D6 degraded-hydration cascade hardening + test** — _in progress this session; see HISTORY when landed._
- **D2 typed navHandoff module** — _in progress this session; see HISTORY when landed._
- **D1 / D8 / D4 / D3 — SKIPPED with reasoning** (each evaluated against source, net-negative or stale): **D1** `content-visibility:auto` is a no-op on the WebView-69 floor (it's a Chromium-85 feature) — zero benefit on the budget Androids that most need it — for non-zero risk to scroll-memory precision on the working annotation/scroll layer (profiling never flagged reading-view render). **D8** dissolve `use-sheet-orchestration` — plan-tagged low-priority refactor of load-bearing, working sheet UI ([[respect-production-code]]). **D4** NavContext to collapse the ROUTES factory — already W7.5-adjudicated net-negative ("the honest receipt of clean extraction, not a debt"; bundling doesn't reduce coupling). **D3** project-wide `strictNullChecks` — W8.3 deliberately left it off; would surface hundreds of nulls across every typed file (tsconfig is `strict:false` throughout) for a single-dev, nearly-done app.
...
- **P6–P11** (05-24/25) — App() 2,815 → 770 lines; 28 hooks; ROUTES factory; AppShellOverlays/Sheets.
