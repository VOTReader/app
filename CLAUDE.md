...
> Last major update: 2026-05-19 (later — MODULARIZATION PUSH P1-P5b, single marathon session).
>
> **index.html: 17,200 lines / 944 KB → 8,244 lines / 419 KB.** ~9,000 lines / ~525 KB extracted into **95+ cohesive modules** under `src/`. All work on branch `refactor/modularize-incremental` (12 commits ahead of `main`, fully pushed to origin as backup).
>
...
> - `ui/sheets/` — `BookmarkCreateSheet`, `JournalRecordingSheet`, `JournalInsertSheet`, `JournalNotebookSheet`, `JournalInboundSheet` (P5c — optional — will add ~12 more: `LinkPicker`, `NoteSheet`, `NotebookPickerSheet`, `AnnotationActionChip`, `SelectionToolbar`, `LinkSidebar`, `MultiNotePopover`, `TabActionSheet`, `TabsOverview`, `VersePickerScreen`, `LetterExcerptPickerScreen`)
> - `components/` — `ExpandableText` (used by journal/bookmarks/notes)
> - `styles/` — `journal-styles`
>
> **WHAT'S LEFT IN `index.html` (~8.2k lines):** `App()` (~2,500 lines — the only big inline component, **P6 target for hook extraction**); ~30 small helpers (`bibleHlKey`/`letterHlKey`/`wtlbHlKey`/`studyHlKey`, `bookCategory`, `buildNavIndex`/`searchNavIndex`, `navItemToEndpoint`/`buildSourceEndpoint`, `parseRefRange`/`splitIntoVerses`, `relativeDate`/`timeAgo`, `highlightExcerptInDom`, `splitWithHighlight`/`normalizeForHighlight`, `renderTextWithScripRefs`/`renderCommentaryCite`, IDB thumbnail helpers, `loadTranslation`/`loadBibleStudies`, `translateVerse`, `linkWtlbEntries`/`linkPreface`, `resolveVotLetter`, `isHiddenManna`, `srchGroupKey`, etc.); ~12 sheet/picker components (P5c target, optional, low risk); inline `ErrorBoundary`, `HL_COLORS`, `useMarkAsRead`, `noteSourceLabel`/`noteSourceNav`; the final `ReactDOM.createRoot` + render.
>
...
>
> **OPTIONAL ALSO P5c** — extract the ~12 remaining sheet/picker components (same mechanical pattern as P5a/P5b, extractor is ready, low risk, further slims index.html). Independent of P6 — can go before or after.
>
