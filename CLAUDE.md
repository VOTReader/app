...
>
> **CURRENT SEQUENCING (user-confirmed, refined through G.2.3)**:
> - **NOW**: **P6 — `App()` hook extraction** (deferred since P5d when `useMarkAsRead` landed as warmup). Extract `useTabField` / settings persistence / scroll handlers / history tracking / thumbnail capture / visualViewport keyboard tracking / etc. into `src/hooks/`. `App()` drops ~2,500 lines → ~400. The riskiest single refactor in the plan — real semantic surgery (closure deps, React hook ordering, state passing between newly-separated hooks). Go one hook at a time; smoke after each. The G.2.x ES module bridge means hooks now live in true modules with explicit imports, not bare global functions — extraction is structurally cleaner.
...
> - **DEFERRED FOLLOW-UP**: clean up the ~13 module-private-state declarations now dead-duplicated in index.html (THUMB_DB, THUMB_STORE, _thumbDbPromise, _translationPromises, _translationLoaded, _bibleStudiesPromise, EXPAND_THRESHOLD, MIN_HIDDEN_WORDS, GARDEN_TOTAL, GARDEN_TIERS, GARDEN_DEFAULT_TIER, GARDEN_PRELOAD_AHEAD, GARDEN_CRAWL_DELAY, gardenImageCache). They're harmless — module-scope bindings win the lookup inside their own bundle — but the index.html copies became dead code with G.2.3 and can be removed when convenient. App() inline references resolve to the bundle-D window globals (set by `_entry-d.js` before ReactDOM.render runs at index.html:4040).
>
