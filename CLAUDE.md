...
**Latest landed (2026-07-28, commits `e9f4f46`/`c5a7cc6`/`283001e`/`ac23e20`):** the responsiveness session — the owner's "taps take a second" input lag root-caused on-device (CDP long-task probes while he read): html2canvas thumbnail captures after every reading pause. Fixed in two stages (interaction calm gate, then the scroll-stop capture path RETIRED outright — do not re-add); measured 15,878ms → 135ms main-thread blockage per 2-min reading window. Also: single-allocation PixelCopy screenshot (crop+scale inside the hardware copy; `MainActivityLogic.screenshotGeometry`), and the unscrollable-when-tall center-modal fix (`.garden-warning-modal`/`.disable-tabs-dialog` max-height+overflow). External native review adjudicated 1-of-4 (cache-clear gating, biometric lock, manifest checksum all discarded with reasons — see HISTORY). Immediately prior: the 07-22 import-hang fix (`229210f`).

**Still owed (owner/device-side; nothing blocking):**
- Owner re-import of his own backup to confirm data lands (his device went offline mid-verify 2026-07-22) + batch-5 visual checks (no over-scroll edge glow, no pre-paint white flash).
- Batch-4 on-device checks: splash→UI with no black flash; recording pauses/resumes other audio (OEM-sensitive); a legacy `.votbak` import.
- Standing manual device walks: `tools/n1-smoke-walk.md` (U1/U7/U9, N2.2–N2.5, Garden zoom).
- Owner decisions pending: the 5 Lamb-of-God PDF illustrations absent from the app (source PDF gone); Wave 0 UX plan decisions D1–D6 (gate Waves 1–2; see HISTORY.md).
- Optional tracks: W10 deep accessibility (TalkBack walk, modal focus traps); restored-names phase 2 (OT YAHUWAH).
