...
**The app is feature-complete and shipping.** One JS codebase runs as the Android APK and as a desktop PWA (live + installable + full-offline at https://votreader.github.io/VOTReader-studio/). Every quality/uplift phase is closed — **Q3–Q8, N1, NK, P6–P11, W1–W9, U0–U22, N2** (one-line index below; full detail in **HISTORY.md**). **1678 vitest** tests / 68 files; CI green across build + lint (`--max-warnings 0`) + typecheck + vitest+coverage(floor) + Kotlin `testDebugUnitTest`+jacoco + headless 12-screen smoke. Pre-commit/CI also gate check_balance, schema-validate, corpus-version, CSP-hash, and the ≤800-line app.jsx canary.
...
**AUDIT-PLAN execution — Waves 1–3 closed (2026-06-02).** A SECOND deep adversarial audit (15 subsystem agents + 15 independent verification agents, every finding re-checked against source) rated the post-UPLIFT app **7.5/10** and produced **`AUDIT-PLAN.txt`** — the canonical tracker (≈108 verified items, P0–P3) for the 7.5→8.5+ remediation. Waves 1–3 are DONE, all gated + pushed (vitest **1666 → 1678 / 68 files**; +1 Kotlin AppInterface test):
...
- **Wave 3 — only-backup integrity + native:** **S2** (`6fb3de5`) `JournalMediaStore` settles on the TRANSACTION (reject on abort/error; put/delete resolve on commit) — fixes an import-hang + makes media puts durable. **S1/S3** (`67ff4c7`) export flushes `whenSaved()` before reading (only-backup can't miss a just-made edit); import no longer auto-reloads on `writeFailures` (would mix imported + old IDB data). **N2** (`29dc4bc`) `startAudioSession` guards a double-start that stranded the device in `MODE_IN_COMMUNICATION` (+Kotlin test; `:app:testDebugUnitTest` green).
  ...
  **OWED / NEXT (full P2/P3 list in `AUDIT-PLAN.txt`):** **Wave 4 (performance) is next** — headline **PF1**: minify the corpus bundles for a measured **~3.3 MB** download/parse cut (the data files have no top-level `this`, so the bundle-a UMD trap doesn't apply; needs a **CORPUS_VERSION bump** + a chrome69-floor cold-boot check). Then PF7 (`app.css` −80 KB minify), PF4 (free + offline-precache the ~32 MB resident alt-translations), PF3 (IntersectionObserver list virtualization). **N1 audio-FOCUS is deliberately HELD for an on-device walk** — the transient-exclusive AudioFocus request (pause other apps during a *voice-memo* mic recording — NOT content audio) + the `onCleared` mode-restore touch the Pixel/Samsung `MODE_IN_COMMUNICATION` tuning (project's N2.2 device-discipline; Kotlin changes don't auto-reach users, so holding it is correct). Trivial leftovers: dead `KIND_BOOST['heading']` / `srchGroupKey 'heading'`; SR4's stemmer/phonetic helpers stay unused (separate from the synonym feature). NOTE: content audio/video (Bandcamp/SoundCloud/YouTube `audioUrl`s) are EXTERNAL LINKS opened out (CSP `media-src blob:` blocks in-app playback) — nothing fetches/embeds them; the only egress is Garden `<img>` from GitHub.
