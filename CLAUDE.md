...
**Latest landed (2026-07-22, commit `229210f`):** the "Importing… please wait" hang — a Wave-0 confirm-sheet regression (the async sheet returned instantly, so `_importV3Android`'s `finally` closed the native stream before the user ever confirmed; the confirm is now fire-AND-AWAIT through a single resolver every dismiss path routes through). Root-caused + fix verified on-device; `v3ImportVerify` also hardened size-bounded. Immediately prior: native batch 5 (`a1b8738`/`9e2c167` — scale-restore, onProgress instance-pin, over-scroll/bg polish, v3 backup CRC-32 manifest integrity: additive 4-byte trailer, warn-and-allow, bridge shape 36→37).
