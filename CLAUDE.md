...
**Visible finger-follow page swipe — ViewPager2-style (2026-06-17, commit `fb94463`).** The reading screens already had a swipe, but it was an INVISIBLE release-only hard-swap. It's now a true finger-follow page turn on the **4 reading screens** (Bible chapters, Matthew study, Volume letters, WTLB entries): the page tracks the finger 1:1 and the prev/next page slides in as **real preloaded content**, release past 35%-width OR a velocity flick commits, a short drag springs back; rubber-bands at the ends, honors `prefers-reduced-motion` (instant, like before). **Canonical code:** `hooks/use-pager-gesture.js` (a pure `createPagerGesture` controller + thin React hook; exported `decideAxis`/`isCommit`/`rubberBand`/`velocityFromSamples`) + `ui/components/pager-preview.jsx` (`PagerPeek` + per-format INERT renderers `PreviewLetterBody`/`PreviewWtlbBody`/`PreviewVerses` + `resolveNeighborLetter`). `ScreenLayout` gained an optional **`pager` prop** → wraps `{children}` in `.pager-track` (transform set/cleared imperatively; none at rest) + renders `.pager-peek` as an overlay sibling of `.screen-scroll`. **`use-swipe-nav.js` RETIRED** (no importers). **THE load-bearing design constraint:** do NOT mount 3 live screens — each fires singleton side effects (the single `__scrollEl`, `window.__closeSheet`, the modal registry, mark-as-read, the **document-wide `[data-hl-dom]` annotation scan**) that triple-mounting corrupts. So the neighbor is an **inert preview**: no effects, no `data-hl-*`, no subscribing components → every annotation pass ignores it; the live center pane is byte-identically untouched. Same-collection neighbor → full content (resolved from the in-memory corpus; verse screens already hold full neighbor objects); a reading-chain **boundary → a `.bottom-nav-card` peek** (e.g. "Previous Book · Genesis 50"). Preserves the NAV4 tappable-element + text-selection guards. **GOTCHA fixed:** `velocityFromSamples` now ignores a <8ms sample window (degenerate/coalesced timestamps manufactured a huge "flick" that committed a tiny drag — caught in preview). Preview-verified on the REAL corpus across all 4 screens (finger-follow transform, commit, spring-back, vertical-scroll never hijacked, boundary cards, **0 `data-hl-*` in every peek**, 0 console errors) + 2328 vitest + coverage + smoke:ci green. **PREVIEW GOTCHA:** a backgrounded preview tab throttles `requestAnimationFrame`/`setTimeout`, so the cosmetic peek-clear + the settle commit can lag when you're not looking at the tab — a test-env artifact, NOT a bug (real swipes happen with the tab focused). `BibleStudyChapterView` has no swipe yet (chain-nav only) — easy follow-on, out of scope. GardenView keeps its own bespoke pan/zoom gesture.
