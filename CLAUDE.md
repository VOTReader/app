...
**Latest landed (2026-07-30, `d43638f..4e87ef2`):** the external-review run — three review batches adjudicated + the last reachable backlog items. Selection edge auto-scroll no longer runs away after a handle is released in the band (re-probed every tick); SW `clients.claim()` added, **then its own regression fixed** — claim fires `controllerchange` on a first-load page too, so sw-register now reloads only a page that HAD a controller (this had broken `smoke:ci` outright); import integrity warns on every non-`ok`/`absent` outcome; journal `{{ref:}}` retry can no longer navigate after you leave the entry. **`allowContentAccess` dropped, proven on-device** (picker still reads back 10,623,375/10,623,375 bytes with it false) — both WebView file/content flags are now off. Actions SHA-pinned + Dependabot; deps patched (esbuild bump left bundles byte-identical). **The long-standing pre-commit vitest flake is identified and fixed** (a passive-effect race in SettingsScreen's modalRegistry assertions, not resource contention). **[17] 200% zoom audit DONE — passes.** Splash→UI black-flash check **verified clean on-device**. Full detail in HISTORY.md.

**Previously (2026-07-28, commits `e9f4f46`/`c5a7cc6`/`283001e`/`ac23e20`):** the responsiveness session — the owner's "taps take a second" input lag root-caused on-device (CDP long-task probes while he read): html2canvas thumbnail captures after every reading pause. Fixed in two stages (interaction calm gate, then the scroll-stop capture path RETIRED outright — do not re-add); measured 15,878ms → 135ms main-thread blockage per 2-min reading window. Also: single-allocation PixelCopy screenshot (crop+scale inside the hardware copy; `MainActivityLogic.screenshotGeometry`), and the unscrollable-when-tall center-modal fix (`.garden-warning-modal`/`.disable-tabs-dialog` max-height+overflow). External native review adjudicated 1-of-4 (cache-clear gating, biometric lock, manifest checksum all discarded with reasons — see HISTORY). Immediately prior: the 07-22 import-hang fix (`229210f`).
...
- ~~Wave 0 UX plan decisions D1–D6~~ — **RETIRED as a dangling reference (2026-07-30).** No such enumerated list exists anywhere in the repo; the only D1–D6 is the UPLIFT D-bucket, which is fully resolved (D2/D5/D6 done, D1/D3/D4/D8 skipped with reasoning). It was reading as a live blocker on "Waves 1–2" that nobody can name. If the owner remembers a real list, re-add it with the items written out.
