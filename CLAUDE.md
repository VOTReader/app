...
- **W1 — Cross-platform PWA platform-bridge — CLOSED 2026-05-27.** 35 commits, range `5688f6e..405b382` (33 code commits through `5f5bcc7` + 2 closure-doc-only commits `952dd9b`/`405b382` for the CLAUDE.md prune + W2.6 plan additions). `git log 5688f6e..405b382 --oneline | wc -l` = 35. Same JS codebase now runs as Android APK (existing) AND desktop PWA. Sub-phases: W0.1 fonts → W1.1 PlatformBridge module → W1.2 (Tier A/B/C) all call-site migration → W1.5 back-button nav (Escape key + popstate + modal registry + root-of-stack double-tap toast) → W1.6 cross-browser smoke walk → 6 BOOKS/MATTHEW bare-ref guards fixed + flake stabilized + dead-code reclaimed. Full per-commit detail in HISTORY.md "W1 — Cross-platform PWA platform-bridge". **Final bundle delta**: bundle-b 351 KB → 357 KB (+6 KB net). **Vitest**: 595 → 628 (+33). **Cross-browser coverage**: Chrome + preview_eval verified; Edge + Firefox + Android regression deferred to W6 (hosting + cross-platform verification phase) — see W6 exit criteria for the full gate. The structural correctness invariants (dispatcher contract, registry semantics, history-sync suppress + clear, popstate flow, activeElement gate, typeof-corpus-global guards) are browser-independent JS state-machine behavior, sufficient for structural W1 closure.
- **W2 prep — `/consolidate-memory` IS THE FIRST ACTION**. NOT a suggestion — a hard prerequisite before any IDB work. ~36 memory entries with near-duplicates around blob/media handling + migration mechanics; W2.2's rebase semantics + IDB schema versioning + platform constraints need clean signal-to-noise to get right. Conflicting memories about merge-vs-rebase would be DANGEROUS for the storage layer. PLAN.txt W2 section opens with a boxed "W2 PREREQUISITE" callout.
