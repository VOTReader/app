...
**Latest landed (2026-08-09 s8):** **the Lamb of God illustrated + the system media card.** (1) The SEVEN narrative illustrations from the restored `THELAMBOFGODstudy.pdf` (OCR had undercounted at 5 — Crucifixion + Burial were never annotated) enter the study as `study-image` blocks in ch3/4/6/7/8/11/12, PDF-faithful placement, `study-lamb-*.jpg` ×7 SW-precached, corpus **c27**, live-verified in preview. Owner call — reverses no-figurative-art for this study only. (2) **Native media card**: the WebView's web MediaSession never surfaces a system card, so `AudioKeepAliveService` now owns a MediaSessionCompat + MediaStyle notification (androidx.media, still no media3) fed by `setAudioNowPlaying` from the player's `_syncNative()` (edge-driven, never the 1 Hz tick); transport returns via `commandSink` → `__votMediaCommand`. **Pause keeps the card alive** (keep-alive releases only on idle; paused notification detaches from the FGS so it's swipeable, swipe stops the service). **POST_NOTIFICATIONS now declared** — asked contextually on first playback, never at boot (owner call). Gates: 3,633 vitest, 250 Kotlin, lintDebug, eslint 0/0, tsc. **Owed: on-device verify** (card, background playback, permission) — phone dropped off adb; APK built + pending install. Detail in HISTORY.md.
