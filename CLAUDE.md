...
> - **NOW**: **P6 — `App()` hook extraction** IN PROGRESS. Landed so far: `useMarkAsRead` (P5d warmup), `useSavedState`+`_validateTabState` (P6a), `useRefMirror` (P6b), `useHistory` (P6c), `useThumbnails` (P6d), `useScrollMemory` (P6e), `useReadingDwell` (P6f), `useSettings` (P6g), `useSheetOrchestration` (P6h), `useFromLetterStack` (P6i). Remaining per PLAN.txt §P6 revised order: P6j `useNavigateToLink` → P6k `useTabs` (load-bearing, last) → P6l `useAndroidBack`. The riskiest single refactor in the plan — real semantic surgery (closure deps, React hook ordering, state passing between newly-separated hooks). Go ONE hook at a time; smoke after each; one hook = one commit.
> - **`App()` LINE COUNT (tracked per extraction — record the real `wc` number in every P6 commit message)**: post-P6i = **2,461 lines** (`function App()` at index.html:1000 → closing `}` at :3460). Progression: ~2,815 post-P6d → 2,735 post-P6e (−80) → 2,693 post-P6f (−42) → 2,641 post-P6g (−52) → 2,516 post-P6h (−125, largest drop — 13 sheet slots moved) → 2,461 post-P6i (measured; the −55 reflects the useFromLetterStack extraction + the separate dead-`popFromLetter` removal — the 2,516 post-P6h figure predated the `c331ad7` BookmarkPopover-fix render growth, so don't subtract naively). The "~2,500" figure in pre-P6d docs was a drifted estimate — measure, don't guess. Target when P6 completes: ~400 lines (hook composition + render tree). Each extraction should visibly move this number down; if a commit doesn't, something didn't actually leave App(). NOTE: the vot-state PERSIST effect deliberately stays in App() (P6g Option C) — it's a composition-level sink; it becomes `usePersistedState` after P6k when all 8 of its deps are hook returns.
...
> - `App()` — **2,461 lines** (`function App()` at :1000 → closing `}` at :3460), **the only big inline component left, P6 target for hook extraction** (P6a-i extracted 10 hooks; 3 to go)
...
>
> **NEXT: P6j — `useNavigateToLink`.** See the CURRENT SEQUENCING block above for the full P6j-l order and the proven 6-step extraction workflow. P6 is the riskiest single refactor in the plan — but the workflow is now proven through P6d-P6i and the smoke harness + mandatory per-extraction targeted test catch regressions cleanly. P6j (PLAN.txt §P6 Cluster 7, HIGH RISK): `_navToLinkRef` + `navigateToLink` (useCallback) + the deferred ref-body assignment — the cross-app deep-linking hub. MUST preserve the "ref assigned later" closure-over-mutable-ref pattern (invariant 4) that lets `navigateToLink` keep `[]` deps while still calling the latest body. P6i (`useFromLetterStack`, `80eed25`) landed clean: pure move, smoke PASS ×2, two-branch back-pill targeted test PASS; its `pushFromLetter` is what the P6j deferred body calls.
>
