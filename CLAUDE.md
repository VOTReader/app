...
- **W2 storage hardening — W2.1 → W2.8 + W2.5 + W2.6 + W2.7a LANDED 2026-05-27. W2.7b–e (StorageHealth UI + write-path wiring + Safari flows) remain.** 25 commits, range `16b8fbd..d7bacc6`. Every byte of structured user data now in IndexedDB (database `votreader`, schema v2 with 19 stores); legacy localStorage keys read once via per-store fallback then cleared by W2.4. **Tests**: 628 → 1059 (+431, with audit polish + new store coverage + StorageHealth). **Bundle delta**: bundle-b 357 → 413 KB; bundle-d 545 → 555.6 KB. **Hydration latency**: 3–8 ms across 17 IDB-backed CachedStores on preview machine (well under the 200 ms mid-range / 500 ms budget targets). Per-sub-phase:
  ...
  - **W2.7 remaining (W2.7b–e)**: StorageHealthBanner component (subscribes via `useSyncExternalStore`, mounts in AppShellOverlays). Write-path wiring: CachedStore._save catch → `StorageHealth.onWriteFailure`, JournalMediaStore.put onerror, JournalRecordingSheet pre-record `checkBeforeWrite`. useStorageInfo rewired to delegate to StorageHealth (eliminates duplicate `estimate()` calls). Settings → Your Data platform row. Safari 7-day modal + iOS PWA welcome card + trigger-site gates in gesture handlers. PLAN.txt has the W2.7b–e breakdown.
...
- **7 cluster bundles** in `app/src/main/assets/dist/`:
  ...
  - `bundle-d.js` 555.6 KB — screens + sheets + components + utils + late stores + screen-routes + App() + W1.5 AppShell registrations (esbuild IIFE)
- **1059 vitest tests** across 38 files.
...
│   │       ├── utils/                 # 12 helper bundles (incl. storage-health.js W2.7a)
