...
> - **NOW**: **P6 — `App()` hook extraction** IN PROGRESS. Landed so far: `useMarkAsRead` (P5d warmup), `useSavedState`+`_validateTabState` (P6a), `useRefMirror` (P6b), `useHistory` (P6c), `useThumbnails` (P6d), `useScrollMemory` (P6e), `useReadingDwell` (P6f), `useSettings` (P6g), `useSheetOrchestration` (P6h), `useFromLetterStack` (P6i), `useNavigateToLink` (P6j). Remaining per PLAN.txt §P6 revised order: P6k `useTabs` (load-bearing, last) → P6l `useAndroidBack`. The riskiest single refactor in the plan — real semantic surgery (closure deps, React hook ordering, state passing between newly-separated hooks). Go ONE hook at a time; smoke after each; one hook = one commit.
> - **`App()` LINE COUNT (tracked per extraction — record the real `wc` number in every P6 commit message)**: post-P6j = **2,398 lines** (`function App()` at index.html:1000 → closing `}` at :3397). Progression: ~2,815 post-P6d → 2,735 post-P6e (−80) → 2,693 post-P6f (−42) → 2,641 post-P6g (−52) → 2,516 post-P6h (−125, largest drop — 13 sheet slots moved) → 2,461 post-P6i → 2,398 post-P6j (−63, the useNavigateToLink deferred-body move; all measured — the 2,516 post-P6h figure predated the `c331ad7` BookmarkPopover-fix render growth, so don't subtract naively across that boundary). The "~2,500" figure in pre-P6d docs was a drifted estimate — measure, don't guess. Target when P6 completes: ~400 lines (hook composition + render tree). Each extraction should visibly move this number down; if a commit doesn't, something didn't actually leave App(). NOTE: the vot-state PERSIST effect deliberately stays in App() (P6g Option C) — it's a composition-level sink; it becomes `usePersistedState` after P6k when all 8 of its deps are hook returns.
...
> - `App()` — **2,398 lines** (`function App()` at :1000 → closing `}` at :3397), **the only big inline component left, P6 target for hook extraction** (P6a-j extracted 11 hooks; 2 to go)
...
>
> **NEXT: P6k — `useTabs`.** See the CURRENT SEQUENCING block above for the full P6k-l order and the proven 6-step extraction workflow. P6k (PLAN.txt §P6 Cluster 2, HIGH RISK — the load-bearing one, ~280 lines): the tab state machine + `tabField` + the cached per-key setter identity (invariant 1) + `updateActiveTab` + multi-tab open/close/switch. It is the keystone — most other hooks receive `tabField` or tab-derived values, so it is deliberately extracted LAST among the state hooks. P6k also carries the deferred obligation to consolidate the inline vot-state PERSIST effect into `usePersistedState` once all 8 of its deps are hook returns (PLAN.txt §P6 Cluster 2 + DIRECTION 4). Then P6l `useAndroidBack` (the most-coupled handler — depends on Clusters 2/5/6/7). P6i (`useFromLetterStack`, `80eed25`) and P6j (`useNavigateToLink`, `9031be9`) both landed clean — pure moves, smoke PASS, targeted tests PASS.
>
