...
> - **NOW**: **P6 — `App()` hook extraction** IN PROGRESS. Landed so far: `useMarkAsRead` (P5d warmup), `useSavedState`+`_validateTabState` (P6a), `useRefMirror` (P6b), `useHistory` (P6c), `useThumbnails` (P6d), `useScrollMemory` (P6e), `useReadingDwell` (P6f), `useSettings` (P6g), `useSheetOrchestration` (P6h). Remaining per PLAN.txt §P6 revised order: P6i `useFromLetterStack` → P6j `useNavigateToLink` → P6k `useTabs` (load-bearing, last) → P6l `useAndroidBack`. The riskiest single refactor in the plan — real semantic surgery (closure deps, React hook ordering, state passing between newly-separated hooks). Go ONE hook at a time; smoke after each; one hook = one commit.
> - **`App()` LINE COUNT (tracked per extraction — record the real `wc` number in every P6 commit message)**: post-P6h = **2,516 lines** (`function App()` at index.html:1000 → closing `}` at :3515). Progression: ~2,815 post-P6d → 2,735 post-P6e (−80) → 2,693 post-P6f (−42) → 2,641 post-P6g (−52) → 2,516 post-P6h (−125, largest drop — 13 sheet slots moved). The "~2,500" figure in pre-P6d docs was a drifted estimate — measure, don't guess. Target when P6 completes: ~400 lines (hook composition + render tree). Each extraction should visibly move this number down; if a commit doesn't, something didn't actually leave App(). NOTE: the vot-state PERSIST effect deliberately stays in App() (P6g Option C) — it's a composition-level sink; it becomes `usePersistedState` after P6k when all 8 of its deps are hook returns.
...
> - `App()` — **2,516 lines** (`function App()` at :1000 → closing `}` at :3515), **the only big inline component left, P6 target for hook extraction** (P6a-h extracted 9 hooks; 4 to go)
...
>
> **NEXT: P6i — `useFromLetterStack`.** See the CURRENT SEQUENCING block above for the full P6i-l order and the proven 6-step extraction workflow. P6 is the riskiest single refactor in the plan — but the workflow is now proven through P6d-P6h and the smoke harness + mandatory per-extraction targeted test catch regressions cleanly. P6i (PLAN.txt §P6 Cluster 6): the back-pill `fromLetterStack` (tabField) + pushFromLetter/popFromLetter/tapThroughBack + the prune effect + the backHint computation — LOW risk, but it interfaces with the `_navToLinkRef` deferred-body pattern (invariant 4).
>
