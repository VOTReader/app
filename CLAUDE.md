...
- **W1 follow-up review — 6 observations addressed.** **(1) Escape-on-plain-input verified in real Chrome**: synthetic `<input>` planted, focused, Escape pressed → `handleAndroidBack` called 0 times, `defaultPrevented=false`, heading unchanged. The activeElement gate from `b6107ca` correctly skips back-nav when a plain input is focused — confirms the path the earlier W1.6 report glossed over. **(2) alert() blocking flagged as W2.6 scope work, not W7 polish**: `exportPersonalData` + `importPersonalData` both call `alert(...)` which blocks the renderer on every desktop platform. The W2.6 agent (already touching export/import for the media-blob upgrade) MUST migrate these to the in-app toast pattern (matches `jrnShowMilestoneToast`); this is not optional polish. **(3) BOOKS bare-ref grep across hooks + ui — 3 additional sites found and fixed** (`<commit>` below). Same class of bug as the W1.6 hardening: optional chaining (`?.`) does NOT save you from undeclared identifier ReferenceError; only `typeof BOOKS !== 'undefined'` guard works. Verified in real Chrome: `delete window.BOOKS` then `BOOKS[x]?.chapters` → ReferenceError; `typeof BOOKS` → 'undefined' safely. Fixed sites: `use-android-back.js:194` (bible-ch back single-chapter-book detection), `use-navigate-to-link.js:131` (bible deep-link destSnapshot), `use-navigate-to-link.js:148` (bible deep-link nav). Other grep hits ruled out as false positives: SettingsScreen's PROGRESS_GROUPS is fully gated by `_BOOKS_READY` + `_VOT_READY` ternaries (lines 86-130); ScripturesHome lines 80/96 are gated by `bibleLoaded` ternary; `BibleChapterView.jsx:139` was `POETIC_BOOKS` (local Set, not BOOKS). End-to-end Chrome verification: with `delete window.BOOKS`, `handleAndroidBack()` from bible-ch returns "true" without throwing. **(4) Export round-trip with seeded data verified — small-export concern resolved**: the original 1620-byte file was small because the app had only 4 vot-* keys populated (welcomed/about-seen flags + ann-migrated + state). Seeded 3 bookmarks + 1 notebook → export payload grew to 3250 bytes, 10 keys captured, all 3 W1.6 follow-up bookmarks present in `vot-bookmarks` (521 bytes for 3 bookmarks — proportional). The export mechanism scales correctly with real data; the small original was a fresh-state artifact, not a truncation bug. **(5) Memory consolidation directive elevated** from "should" to **MUST as FIRST W2 action** (see W2 prep below). **(6) Edge + Firefox + Android deferral documented** as explicit one-liner in both CLAUDE.md and PLAN.txt W1.6 closure: "W1.6 verified Chrome + preview_eval only; Edge + Firefox + Android regression deferred to W6 — see W6 exit criteria for the full cross-platform gate."
- **W2 prep — MEMORY CONSOLIDATION REQUIRED AS FIRST ACTION**. Run `/consolidate-memory` AT THE START of the W2 session — NOT as a suggestion, MUST as the first action before any IDB work begins. Reasoning: ~36 memory entries include near-duplicates around (a) blob/media handling (3 entries: readAsDataURL OOM, MediaStream cleanup, amplitude buffer pre-alloc) and (b) migration mechanics (3 entries: abstraction-before-migration, consolidate-don't-duplicate, guard-removal-includes-fallback). W2.2's rebase semantics + the IDB schema versioning + the platform constraints from ~10 storage-related entries need a clean signal-to-noise to get right; conflicting memories about merge-vs-rebase semantics would be DANGEROUS for the storage layer. The W1.6 smoke walk surfacing 4 additional BOOKS bare-refs proves the value of CLASS-of-bug audits over single-site fixes — `/consolidate-memory` is the equivalent for memory entries.
- **W1.6 — Cross-browser smoke walk — CLOSED.** Dual-track verification: a parallel preview_eval agent ran the iframe-friendly subset (12-screen render walk + globals audit + annotation round-trip + Escape priority chain + popstate flow + responsive resize at 375/768/1440), and Claude in Chrome drove the real-browser-only surfaces (file export download via OS-level Downloads/, file import full round-trip with localStorage replacement + reload, audio recording mic-request path through to the user-gesture boundary, garden-warning modal open/dismiss via real OS Escape key). **votSmoke result PASS**: 142 globals present, 0 missing; 12/12 screens reached, 0 crashed; letterAnn 133 marks + 13 note icons after open + next/prev nav (no crash); wtlbAnn 51 marks + 6 note icons; 0 console.errors; 0 resource 404s. **All 5 Escape priority paths exercised** in real Chrome: isComposing → no-op; fullscreen → no preventDefault; modalRegistry.isAnyOpen → dismiss top (verified `bookmark-popover` dismisses via simulated dispatcher); activeElement INPUT → no nav, browser blurs; handleAndroidBack → Library→home navigated via OS Escape. **Sheet-with-INPUT-focused** path explicitly verified: bookmark-create-sheet open + input focused + Escape → SHEET dismisses (not just input blurs), registry transitions `['bookmark-create-sheet'] → []`, handleAndroidBack NOT called (correct per the registry-before-activeElement priority order). **Full popstate flow** confirmed in real Chrome: history.back from a non-root screen navigated without re-push (suppress consumed); back at root showed toast (id `vot-root-exit-toast`, role `status`, text "Press back again to exit", opacity 1); second back within 2s disarmed (opacity 0, no replacement push); TIMER-CLEAR-ON-FORWARD-NAV invariant verified (armed toast → tap tile → opacity 0). **Export round-trip**: PlatformBridge.saveToDownloads creates a valid `votreader-backup-2026-05-27.json` (exportVersion 1, all vot-* keys preserved, current tab state captured). Chrome's silent multi-download safety queues subsequent downloads as `.tmp` until user confirms — browser-level UX, not our code; first download to a fresh tab lands cleanly to `Downloads/`. **Import round-trip**: `window.__onImportFile(base64)` callback wired; confirm prompt fires with REPLACE warning; old localStorage wiped (3 keys removed); new localStorage installed from backup (synthetic `vot-w16-import-marker` survived); alert "Import complete. The app will now reload." fires; reload triggers; post-reload state confirmed the marker key persisted (`'PRESENT'`). **Audio recording up to the gesture boundary**: Voice Recording sheet mounts, registers `journal-recording-sheet`, requests mic, timeout fallback renders correctly ("Microphone request timed out. If a permission prompt appeared, please try again." + CLOSE button), close path cleanly unregisters. Full positive-path verification (record + playback) needs manual mic-permission grant; out of W1.6 automated scope. **Pre-existing latent bug found + fixed** (`9c35993`) — see W1.6 hardening below. **Cross-browser coverage**: W1.6 verified Chrome + preview_eval only; Edge + Firefox + Android regression deferred to W6 — see W6 exit criteria for the full cross-platform gate. The Chrome-only scope is sufficient to call W1 structurally complete (the structural correctness invariants — dispatcher contract, registry semantics, history-sync suppress + clear, popstate flow, activeElement gate — are browser-independent JS state-machine behavior). **Known findings (documented, not fixed)**: (1) `alert()` in exportPersonalData + importPersonalData blocks the renderer — **upgraded from "polish" to W2.6 scope blocker** (the agent already touching export/import for media-blob upgrade MUST migrate these to in-app toast in the same pass). (2) CDP-controlled tab won't auto-grant mic / fullscreen permissions; manual Chrome flag setup required for full positive-path verification. (3) `resize_window` via CDP doesn't shrink below ~700px in Windows context; agent's preview_eval handled 375/768/1440 via the iframe resize API instead. (4) Pre-existing flake in `platform-bridge.test.js` MediaStream-cleanup tests (still owed per W1.5 stabilization plan).
- **W1.6 hardening** (`9c35993`) — pre-W1.5 latent bug guarded. `useReadingChainNav` had two bare `BOOKS.revelation` references — `goToRevelationLast` (line 171) and `boundaryConfig` (line 232) — that throw ReferenceError if a saved-tab cold-boot lands the user on a Volume One letter before HomeScreen's pre-fire effect resolves `__loadBibleCorpus`. Bug introduced when Q8 lazy-loading (`5605f30`, 2026-05-25) moved BOOKS into the lazy bundle-a-bible; never triggered in normal flow because HomeScreen always mounts first. Surfaced by the W1.6 preview_eval agent during the "Volume letter" step. Fix: `typeof BOOKS !== 'undefined'` guard matching the App.jsx existing pattern (app.jsx:459-463 already does this for MATTHEW + BOOKS). Both helpers now silently no-op when BOOKS isn't loaded; next render after the loader resolves fills in the Revelation prev-boundary card normally. 3 new vitest cases (628 total, was 625). Preview-verified in real Chrome: deleted `window.BOOKS`, navigated Volumes → V1 → "A Word of Warning" → letter rendered cleanly (heading + body present, ZERO console errors, prev-boundary card absent); called `__loadBibleCorpus()` → 66 books loaded → re-render → prev-boundary card appears. Recovery cycle works.
- **W1.5 — Back-button navigation (Escape + browser back) — CLOSED.** Seven commits land the entire desktop-back UX. Final commit range `905b78d..b6107ca`. **W1.5(a.1)** (`905b78d`) — `src/hooks/use-modal-registry.js` (~170 lines) lands as pure addition per [[abstraction-before-migration]]. Module-level `Map<string, () => void>` registry (Maps preserve insertion order → `peek()` returns last-registered = topmost). `useModalRegistry({id, dismiss, active})` hook mirrors `dismiss` via `useRefMirror` so inline-arrow callbacks don't churn insertion order. 25 vitest cases including the explicit shared-IDs FAIL vs unique-IDs FIX comparison. **Dead-code housekeeping** (`fd8aa22`) — DONE; folded into the W1.5 batch per user directive: `NotebookManagerSheet` (126 L) + `JournalNotebookSheet` (142 L) deleted, ~10 KB total bundle savings (bundle-b -5.3 KB, bundle-d -5.4 KB). Both files' imports + window-assignments removed from `_entry-b.js` / `_entry-d.js` (build gate confirms — esbuild would error on a stale import). The "Post-Tier-C" entry below mentions these as an OWED housekeeping item; treat that mention as historical context — the deletion is COMPLETE here, no re-investigation needed. **W1.5(a.2)** (`195cfb3`) — wires 25 consumers (24 from the original inventory + 1 discovered during wiring: `letter-scripture-sheet` — `LetterView.jsx`'s inline `scripRef` state, the per-letter fn-sheet that opens when a footnote's scripture ref is tapped INSIDE a letter view, structurally distinct from `ChapterView`'s ScriptureSheet. The original grep missed it because LetterView already wires `window.__closeSheet` for it via a multi-source closer effect, so it didn't surface in the "uses registry" search — it was found by walking the file during wiring). ConfirmStrip uses `React.useId()` per-instance for unique IDs per the locked contract — shared literals would collapse concurrent instances and break peer registrations on first unmount. Test-fixture wiring: `vitest.setup.js` globalizes `useModalRegistry` + `modalRegistry` so colocated UI tests (ConfirmStrip's suite) resolve them as free variables, matching the production reality where `_entry-b.js` Object.assigns onto window. **W1.5(b)** (`5babe86`) — `src/hooks/use-history-sync.js` (~165 lines) + 16 vitest cases. Watches the 8-field per-active-tab nav-key tuple; pushes empty-state entries (Option B per [[root-of-history-pwa]]). `window.__historyReady` flag set after first-mount-skip for the Firefox `popstate`-on-load guard in (d). `suppressNextHistoryPush()` + `clearSuppressNextHistoryPush()` exported pair: caller sets the flag before back-induced nav; if `handleAndroidBack` returns the STRING `"false"` (no state change → effect never fires → flag would otherwise strand onto the next legitimate user nav), caller calls clear. Android `window.AndroidBridge` guard skips entirely. StrictMode-safe via `lastKeyRef.current === key` comparison + idempotent serialization. **W1.5(c)** (`732bf6b`) + **(c) hardening** (`b6107ca`) — second `[]`-deps effect inside `use-android-back.js` adds the keydown listener as the SOLE Escape dispatcher per the DISPATCHER CONTRACT. Seven gates in priority order: web-only / Escape-key-only / not-composing / not-in-fullscreen (browser handles natively, no `preventDefault`) / registry-isAnyOpen (call `peek().dismiss`, do NOT also fire back-nav, `preventDefault`) / **activeElement-is-INPUT/TEXTAREA/contenteditable (added in `b6107ca` after review) — skip without preventDefault so the browser blurs the field instead of navigating away when the user just wanted to dismiss focus** / else-`handleAndroidBack`-with-suppress+clear. Registry check intentionally precedes the activeElement check so a sheet with an input inside (NoteSheet, BookmarkCreateSheet, LinkPicker) dismisses the SHEET on Escape rather than just blurring the input — verified preview_eval. Modals do NOT add their own keydown listeners; no listener race. **W1.5(d)** (`0073c10`) — `src/utils/root-exit-toast.js` (~150 lines) + 14 vitest cases + a third `[]`-deps effect in `use-android-back`. Toast is a fixed-position div appended directly to `document.body` (independent of the React tree, matching `jrnShowMilestoneToast` convention) with `role="status"` + `aria-live="polite"` + Cinzel typography matching the dark theme. `popstate` listener: web-only + `__historyReady` Firefox guard + suppress+`handleAndroidBack` handshake. At root (`result === "false"`): `clearSuppressNextHistoryPush()`; if armed → second back within 2s → `disarm()` + NO replacement push (popstate already consumed an entry → next gesture exits); else first back at root → `pushState({}, '', '')` replacement + `arm()`. **TIMER-CLEAR-ON-FORWARD-NAV invariant** enforced inside `useHistorySync`'s forward-push branch: `disarm()` called before every legitimate push so an old armed timer from a previous root-back can't bleed into a later root-back and silently skip the toast. **Preview-verified end-to-end against real Chrome**: initial mount doesn't push; nav-key change pushes (`history.length` grows); back from a non-root screen navigates without re-pushing (suppress consumed); back at root shows toast (opacity 1, exact text "Press back again to exit"); second back within window disarms (opacity 0, no replacement push); forward nav while armed disarms (the invariant); Escape with plain input focused (no modal) does NOT fire `handleAndroidBack` (browser blurs); Escape with sheet open + input focused DISMISSES the sheet (registry first). **All 5 pre-commit gates green at every sub-step**; vitest **595 → 625** (+30 across W1.5). **Bundle-b** delta across the batch: 351.0 KB → 356.9 KB (+5.9 KB net for the entire W1.5 + (a.2) consumer wiring + (b)/(d) helpers + (c) hardening). **Pre-existing flake observed under combined load** in `platform-bridge.test.js`: `nativeRecordStop` + `nativeRecordCancel` MediaStream-track-cleanup tests pass standalone but flake occasionally under the 25-file suite (real timing — both await `new Promise(r => setTimeout(r, 20))` then assert sync-fake-MediaRecorder cleanup occurred). **Concrete stabilization plan for the W1.6 / W2 agent**: replace the timing-dependent `setTimeout(r, 20)` with `vi.useFakeTimers()` + `vi.advanceTimersByTime` so the async sequence is deterministic regardless of suite load; alternatively, fully mock MediaRecorder + AnalyserNode + getUserMedia at the test boundary instead of relying on the partial-fake jsdom implementation. Either fix is ~30 minutes; not a W1.5 blocker but will get worse as W2 adds 30-50 IDB tests to the concurrent load. **W1 is now complete except for W1.6** (cross-browser preview-eval smoke walk). **Memory consolidation directive**: run `/consolidate-memory` at the START of the W2 session (NOT before W1.6 — the smoke walk is a manual browser pass that doesn't benefit from clean memory). W2's storage architecture / hydration semantics / platform constraints come from ~36 entries with some near-duplicates around blob handling + migration mechanics; that's where signal-to-noise matters.
- **Post-Tier-C review-loop hardening — W1.5 + W2.2 specs locked + memory landscape refreshed.** Six commits + 14 memory writes refine the design after a multi-round review pass: **(1)** [[w2-hydration-data-loss]] now mandates write-queue REBASE on top of IDB — explicitly NOT a "cache wins" store-level merge. The earlier wording could have been implemented destructively (cache wins → discard IDB → all previous-session data lost). Critical correctness fix; W2.2 agent MUST use rebase terminology. **(2)** [[root-of-history-pwa]] locks three required pieces: pushState Option B (empty state object — matches Android no-forward-button), MANDATORY double-tap toast (not optional — without it the user is trapped or silently exited), and TIMER-CLEAR-ON-FORWARD-NAV (without it, a stale armed timer from earlier root-back skips the toast on a later root-back). **(3)** [[centralized-modal-signal]] is Set\<string\> not counter (immune to double-invoke under StrictMode + debuggable via openIds()). ConfirmStrip REGISTERS (decided, not open) — Escape during inline-confirm cancels the confirm; outside it, navigates back. **(4)** W1.5 modal inventory enumerated inline in PLAN.txt as the wiring checklist: 4 AppShellOverlays + 1 nested + 12 AppShellSheets + 6 screen-local + 1 ConfirmStrip = 24 registered consumers. **(5)** Dead-code finding: `NotebookManagerSheet` + `JournalNotebookSheet` exported via entry bundles but have ZERO `<X>` JSX mount sites anywhere — inert dead code (RESOLVED in `fd8aa22` as part of the W1.5 batch; entry imports cleaned + ~10 KB reclaimed). **(6)** Memory sprawl flagged (~36 entries); run `/consolidate-memory` before W2 starts; natural family-merges: blob/media handling (3 entries) + migration mechanics (3 entries). **W1.5 implementation sequencing locked**: (a) modal registry hook + wire 24 consumers → (b) history-sync layer (HIGHEST RISK — touches navigation; today the app has ZERO browser-history entries because navigation is pure React state) → (c) Escape key listener (needs a) → (d) popstate listener + root-of-stack double-tap (needs b). (a) and (b) are independent; can run in parallel. Out-of-order shipping = later pieces have no infrastructure to consume.
- **W0-W8 — Cross-platform PWA + quality hardening — IN PROGRESS (started 2026-05-26).** Same JS codebase, two shells: existing Android APK + installable desktop PWA. The web stack stops being a constraint and becomes the core asset — `if (window.AndroidBridge)` guards are already pervasive, so the desktop browser mostly works today. Phases: W0 prerequisites (fonts + owed NK7 device walk) → W1 PlatformBridge (decouple from AndroidBridge, 15 methods, 6 call sites) → W2 IndexedDB migration (transactional writes, ~50 MB+ quota, in-memory write-through for sync render paths) → W3 PWA shell (manifest + service worker + offline fallback, ~2.3 MB install cache + ~10.3 MB lazy) → W4 desktop polish (max-width, hover, keyboard shortcuts) → W5 hosting (GitHub Pages) + dual CI → W6 cross-platform verification → W7 quality hardening (IDB schema versioning, raw() freeze, hlTick dead-prop removal across 39 files, JS DiagnosticLog, 47→5-prop bundle refactor) → W8 type coverage (82 UI files in 5 tiers) + CSS @layer (eliminate 24 !important hacks). **W0.1 (`5688f6e`)** — landed 6 OFL fonts as WOFF2 (~140 KB total). Fixed pre-existing bug along the way: the @font-face block referenced `.otf` / `.ttf` filenames that have never existed in the upstream repos — Cinzel + EB Garamond were silently falling back to system serif on every platform since the declarations first landed. **W1.1 (`228be7c`)** — PlatformBridge module landed at `src/utils/platform-bridge.js` (140 lines). 20 @JavascriptInterface methods mirrored from AppInterface.kt (single source of truth for platform detection per [[platformbridge-single-source]]; consumers never check `window.AndroidBridge` directly after W1.2). Android impl is pure passthrough (zero behavior change), web impl is placeholders (no-ops, safe defaults, NYI warnings). 45 new vitest cases bringing total to 536. Pure-addition commit per [[abstraction-before-migration]]. **W1.2 Tier A (`a0546c0`)** — use-thumbnails migrated; html2canvas fallback FOLDED INTO bridge web impl per [[consolidate-dont-duplicate]]; takeScreenshot signature became `() => Promise<string>` uniformly. Three-step pattern proven. Also folded in: `.idea/caches/` gitignored + `deviceStreaming.xml` `git rm --cached`'d. **W1.2 Tier B closed across 3 sub-tiers:** **B.1 (`748ed2d`)** use-settings + WakeLock fire-and-forget impl with same-reason de-dup (caught preview-iframe spam early per [[explicit-async-decision]]); **B.2 (`6ec5ff0`)** SettingsScreen + DOM-input openFilePicker preserving the `window.__onImportFile(base64)` callback contract per [[preserve-callback-contracts]] + Blob/anchor saveToDownloads + diagnostic-log UI guard simplified to state-based; **B.3 (`825d38c`)** GardenView + Fullscreen API (best-effort swallowing user-gesture rejections) + setZoomEnabled/resetZoom as DELIBERATE no-ops per [[verify-inertness-not-equivalence]] (browsers default to zoom-enabled — CSS touch-action hacks would actively break the Garden's natural pinch). Intermediate grep gate after Tier B (per [[intermediate-exit-gates]]): all three gates GREEN across the 4 completed files. **W1.2 Tier C closed across 2 sub-tiers:** **C.1 (`22d1c5b`)** added 7 recording-flow web impls to the bridge (MediaRecorder + AnalyserNode + MediaStream + Blob→base64 + `__onNativeRecordingComplete`) with strict mime negotiation per [[mediarecorder-mime-policy]] + pre-allocated AnalyserNode buffer per [[amplitude-buffer-preallocation]] + 21 new vitest cases; **C.2 (`2538e8f`)** migrated JournalRecordingSheet — 701 → 490 lines (-211, -30%). All 5 detection-variable names eliminated; the web MediaRecorder code path deleted; the native flow becomes the sole path with `PlatformBridge.*` calls. Per [[callback-flow-unification]] the migration was a contract-unification (pre-Tier-C web processed blob inline; post-Tier-C both platforms fire `__onNativeRecordingComplete(b64, durMs, mime)`). **POST-W1.2 EXIT GATES ALL GREEN**: zero `window.AndroidBridge` in live code, zero `error:web-impl-pending`, zero local `isAndroid` in consumers. Bundle tally per [[net-bundle-delta-all-affected]]: ~898.8 KB baseline (b+d) → ~899.6 KB post-W1.2 (+0.8 KB net, essentially flat). 570 vitest tests. **Per [[plan-reduction-as-work-progresses]], the original W1.3 (web file I/O) was SATISFIED by Tier B.2, and W1.4 (web audio recording) was SATISFIED by Tier C** — only W1.5 (Escape key + popstate) and W1.6 (cross-browser smoke walk) remain in W1. Full W0-W8 plan + 18-critique traceability matrix in PLAN.txt; when W8 closes, the list is clean — zero gaps.
