...
> - **P6 — `App()` hook extraction — ✅ COMPLETE.** All 15 hooks extracted: `useMarkAsRead` (P5d warmup), `useSavedState`+`_validateTabState` (P6a), `useRefMirror` (P6b), `useHistory` (P6c), `useThumbnails` (P6d), `useScrollMemory` (P6e), `useReadingDwell` (P6f), `useSettings` (P6g), `useSheetOrchestration` (P6h), `useFromLetterStack` (P6i), `useNavigateToLink` (P6j), `useTabs` (P6k-A), `useTabActions` (P6k-B), `usePersistedState` (P6k+1), `useAndroidBack` (P6l). App() went from ~2,815 (P6d) to **2,191** lines. The 6-step extraction workflow held zero regressions across all of it. **ONE flagged follow-up:** `journalEntryId` is read stale inside `useAndroidBack` (a pre-existing `[]`-deps stale-closure bug the extraction preserved verbatim) — a one-line fix (mirror it via `useRefMirror`); see the use-android-back.js header.
>   - **POST-P6 HOOK CALL-ORDER DAG (final):** `useSavedState` → `useTabs` → [`useSheetOrchestration`, `useThumbnails`, `useScrollMemory`, `useReadingDwell`, `useSettings`, `useHistory`, `useFromLetterStack`, `useNavigateToLink`] → `useTabActions` → `useAndroidBack` → `usePersistedState`. Load-bearing: `useTabs` must precede everything that reads `tabField`-derived state; `useTabActions`/`useAndroidBack` must follow `useReadingDwell`/`useThumbnails` (they need `cancelDwell`/`setTabThumbnails`); `useAndroidBack` must follow every `go*` nav helper it threads (`goStudiesHome` is the last — defined deep in App()); `usePersistedState` is last (it sinks the 8-value union). Do NOT reorder hook calls without checking this graph.
> - **`App()` LINE COUNT (tracked per extraction — record the real `wc` number in every P6 commit message)**: **P6 COMPLETE — final = 2,191 lines** (`function App()` at index.html:1000). Progression: ~2,815 post-P6d → 2,735 → 2,693 → 2,641 → 2,516 post-P6h → 2,461 post-P6i → 2,398 post-P6j → 2,323 post-P6k-A → 2,246 post-P6k-B → 2,241 post-P6k+1 → 2,191 post-P6l (all measured). The "~2,500" figure in pre-P6d docs was a drifted estimate — measure, don't guess. **REALISTIC P6-COMPLETE TARGET ≈ 1,400–1,600 lines, NOT ~400.** The "~400" in older docs was always wrong — App()'s render tree alone is ~1,000 lines, plus nav-helper glue (~150) plus hook composition; PLAN.txt §P6 itself says "~1,400 left in App()". After P6l the realistic floor is ~2,150+ minus whatever of `handleAndroidBack` + nav helpers actually moves. NOTE: the vot-state PERSIST sink became `usePersistedState` (P6k+1) — it takes 8 params, of which only 4 are hook returns (tabs/activeTabIdx from useTabs, activeReadKey from useReadingDwell, settings from useSettings); the other 4 (theme, lastReadChapters, lastReadLetterMap, readItems) are still plain App() useState that no cluster owns.
...
>
> **WHAT'S LEFT IN `index.html` (3,555 lines / ~196 KB as of post-P6h):**
> - `App()` — **2,191 lines** (`function App()` at :1000) — **P6 hook extraction COMPLETE** (all 15 hooks out; App() is now hook composition + the ~1,000-line render tree + nav-helper glue). Next frontier is JSX conversion, not further hook extraction.
...
>
> **NEXT: P6 IS DONE — the next frontier is JSX conversion + a consolidation pass.** All 15 P6 hooks landed (P6a–P6l + P6k+1), App() 2,191 lines, zero regressions. The recommended sequence before JSX:
> 1. **Consolidation chapter** (the user framed this as a clean pre-JSX session): (a) hook-header consistency pass — normalize all 15 `src/hooks/*.js` to the same OWNS/DOES NOT OWN/PARAMS/RETURNS/STORAGE/WINDOW template (the early ones — useMarkAsRead, useRefMirror — predate it); (b) the one-line `journalEntryId` follow-up fix in `useAndroidBack` (mirror it via `useRefMirror` — see the hook header); (c) optional: a `votSmoke({ tabsOn: true })` smoke variant so the tab state machine gets ongoing regression coverage (smoke currently runs tabs-off — P6k was verified by manual targeted tests only).
...
>
> **FIXED — bookmark inline icon missing on Bible/study verse views (`c828982`, 2026-05-21):** a bookmark made on a Bible verse (e.g. `bible:proverbs:2:1:0-69`) saved to `BookmarkStore` and appeared in the Library, but no inline icon rendered in the chapter. Root cause was NOT the suspected `hlTick` / `applyDOMBookmarks` wiring — those are all correct, and the DOM-overlay reading views (LetterView, WtlbEntryView) inject bookmark icons fine. The real gap: Bible verses (`BibleChapterView`) and Matthew study verses (`ChapterView`) render each verse via the React `<HighlightableText>` component, NOT a `[data-hl-dom]` container — and `applyDOMBookmarks()` only walks `[data-hl-key][data-hl-dom]`, so it can never reach them. Those views render a per-verse React `<LinkIcon>` for links; the bookmark equivalent was simply never built. Fix: new `BookmarkIcon` component (`src/ui/components/BookmarkIcon.js`) mirroring `LinkIcon` — `BookmarkStore.getForKeyPrefix(hlKey)` → flag icon → tap routes to `__bookmarkEdit` (single) / `__openBookmarkPopover` (multi), identical to `_buildBookmarkIcon` in dom-bookmarks.js. Rendered after `LinkIcon` at all 3 `HighlightableText` callsites (1 BibleChapterView, 2 ChapterView). Additive only — returns `null` when no bookmarks. Two rendering strategies → two bookmark renderers: `applyDOMBookmarks` for `[data-hl-dom]` views, `BookmarkIcon` for `HighlightableText` views.
>
...
- ☐ F — Code modular (split `index.html` into ~40 modules + `cat-modules.py`, slim `App()` to ~1,400–1,600 lines — the realistic floor; the old "~400" target was wrong, see the App() LINE COUNT note in the header).
