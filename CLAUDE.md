...
**Latest landed (2026-08-26 s14):** **the read-along wash was addressing characters that had moved.** The owner reported the highlight "running ahead 1-5 characters or lagging"; it was deterministic, not occasional. `b06ada7c` fixed the character-offset domain and re-aligned Volume One only — 292 blocks across the other six volumes still shipped offsets measured against text the renderer had stopped producing, drifting up to 13 chars. Nothing could catch it: smoke checked only that a row's BLOCK existed, and **the karaoke QA page was itself rendering the bug** (naive `join('')`), so a correct letter looked broken and a broken one plausible. Instruments first: one text domain (`tools/audio-fragments-lib.mjs`, three consumers), **`check-audio-sync`** (word-boundary invariant, exact on all 18,079 corpus fragments; pre-commit + CI), and **`e2e-readalong`** (real audio in a real browser asserting `blockEl.textContent === segmentsDomText(...)` — the comparison whose absence let the drift ship; proves the wash renders by reading back its own screenshot). Also fixed: two sentence-scanner defects of the reported class (`U.S.A.` shredded, `El!` never highlighted); `_paintAt` freezing a stale wash; tap-to-seek firing on note/link/bookmark icons. **Unproven clauses now paint** (owner directive) with an onset spread across the gap by token weight — UNSPOKEN text still never does. **Bible read-along built** (verse-level, integer centiseconds, ~184 KB/edition, whole-verse so KJV audio over NKJV text just works). **Format B can paint a line at a time** — offsets in the CORPUS domain + a projection onto the render (`utils/format-b-dom-text.js`), gated by a sweep of every paragraph AND every fragment through a real render in both footnote modes; 1,338 whole paragraphs → 4,718 fragments. The overnight batch died at letter 26/172 on `0xC0000142` (desktop-heap exhaustion from ~2,000 ffmpeg spawns) — probes now slice in memory, verified byte-identical. Volume Five re-aligned 29/29 (was 26). Corpus **c36**. Detail in HISTORY.md.

**Open — THREE THINGS NEED THE OWNER'S CALL (raised 2026-08-27, not yet decided):**
1. **Lazy-load the letter timings.** `audio-sync.js` rides bundle-a-vot, so every reader now downloads/parses ~500 KB of read-along timings whenever they open ANY letter, even if they never press Play. bundle-a-vot has ~140 KB of headroom left and the remaining flock/rebuke/holydays batch will eat most of it. The clean fix is the lazy `src/data/` path already built for the Bible timings (`window.__loadBibleSync`). It is a runtime change, so it wants its own commit and its own verification — NOT bolted onto a data ship. The alternative is re-baselining the byte ceiling, which is legitimate but leaves the cost on every reader.
2. **How far to take the Bible.** Six pilot chapters ship today. BRM-KJV (the default voice) is 1,189 chapters, about 25 hours of wall time (measured 2026-09-01: 74.7 s/chapter including overhead across John 2–21; ~15 h if scaled by verse count); Word of Promise and WEB are ~28 each. Owner should watch a pilot chapter before committing a night — see 3.
3. **Whole-verse vs sub-verse Bible highlighting.** Verses paint whole, which is what makes KJV audio work over NKJV text (the app's own default pairing). Verses run 7.7 s mean / 7.25 s median / 11.7 s p90 (969 BRM-KJV verses measured 2026-09-01); letter clauses run 4.0 s mean / 3.4 s median. Sub-verse would need word-index rows, only works when the reader's translation matches the audio's, and roughly triples the data. Decide after watching a chapter, not in the abstract.

Also still open: the letters batch was stopped mid-run at the owner's request — flock 10/62 and rebuke 4/31 belts are banked, so `py -3.13 tools/batch-align.py --volkeys flock,rebuke,holydays,two` resumes where it left off. And the owner tests on the INSTALLED APK: none of c37/c38/c39 reaches his phone without `npm run build` → `:app:assembleDebug` → `adb install -r -d`.

**Prior (2026-08-12 s13):** **Scripture Web UX uplift.** The canonical graph exposes **Essential (votes >=20)** and **Famous (votes >=7, 63,418 links)**; the obsolete 300k "Complete" surface is gone. Dense crossings retain multiple tap candidates and open a chooser; the picker is chunk-culled like the renderer. The location card names the book/chapter and visible verse range, **Go to** accepts references, **Nearby** is a keyboard-friendly connection list, and canonical/personal/corpus-underlay lines all have navigable detail paths. Hover is rAF-throttled, dialogs restore focus, and rotated phone overlays use logical landscape dimensions. Detail in HISTORY.md.
