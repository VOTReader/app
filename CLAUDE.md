...
> - **OPEN FOR P6**: hook extraction is the next major work. See PLAN.txt §P6 for the 12-cluster extraction map. First extraction is `useThumbnails` (smallest blast radius). All future hook extractions should prefer bare `let`/`const` over `export let`/`export const` for truly module-private mutable state — strict-mode is satisfied by the binding, no need to inflate the module's public surface (P6+ hardening principle).
>
