...
**The app is feature-complete and shipping.** One JS codebase runs as the Android APK and as a desktop PWA (live + installable + full-offline at https://votreader.github.io/app/). Every quality/uplift phase and all three deep audits are closed; work is now owner-reported fixes + polish. Gates on every commit (pre-commit + CI): build, lint `--max-warnings 0`, typecheck, ~2,930+ vitest, Kotlin `testDebugUnitTest` + JaCoCo floor, headless smoke:ci, check_balance, schema-validate, corpus-version, CSP-hash, ≤800-line app.jsx canary. Counts drift — verify with the runners.
...
**Latest landed (2026-08-03, session 2):** **The reading-measurement engine.** ONE word-count definition (`utils/word-count.js`, ~6.5M words shipped measured) + a per-item corpus word-count baseline gate in validate-schemas (2,169 items — catches the eaten-paragraph class between audits; `--update-wordcounts` regenerates). The naive mark-as-read trigger (scroll≥90%, no time vector, couldn't fire on fits-viewport WTLB entries, secretly gated on showProgressBar) is REPLACED by `use-read-tracker.js` in ScreenLayout: **geometry-sweep detector** (2 Hz batched gBCR, NOT IntersectionObserver — IO structurally can't credit blocks >2 viewports, i.e. poetry + Text Size 300%), credit state keyed by hl-key STRING (survives the headings-toggle DOM swap), READ ⇔ coverage≥90% by words AND visibility-honest dwell ≥ clamp(words×100ms, 8s, 300s). readItems is COUNT-valued (legacy `true` reads as 1; re-reads increment). `ReadingStatsStore` (`vot-reading-stats`, **IDB v8** — adding a CachedStore REQUIRES IDBAdapter STORE_NAMES + version bump or it hydrates degraded and silently queues forever; found LIVE, tests all green) ledgers words/day/time/wpm (pace sampled at VISIT END, not the biased completion instant) + per-item frontiers → **resume-at-first-unread-paragraph** (jump only when >1 viewport from the restored position). My Progress: Words Read + Reading Pace + 14-day bars + journal words/voice minutes; `~N min` chips on letter/chapter indexes. 17-agent adversarial review: 12 confirmed findings (2 P1 detector, Matthew+Studies unwired, pace bias…) all fixed same-session; E2E-proven in compositing Chromium (jsdom/hidden pages deliver no IO/geometry — live verification is mandatory for this subsystem). Skim indicator HELD per owner (BACKLOG [21] ★ + memory note); TTS/milestones/year-in-review deferred (BACKLOG [22]–[26]). 3,314 vitest / 185 files. Detail in HISTORY.md.

**Before that (2026-08-03):** **Desktop width model — inner caps px→rem** (`5a8381e`). Owner's laptop (≈2048 CSS px, 1600+ tier): "Settings so squeezed; the outside lines aren't that narrow but the text inside feels like it." Root cause = Sol's review point #6: root font tiers step 16→18→20px but inner caps were fixed px, so every surface silently SHRANK in rem terms on desktop (Settings 32.5rem→26rem). 24 caps converted px→rem at /16 (mobile ≤768 byte-identical; reader 45rem keeps a ~constant ~72ch measure — prose NOT stretched); 1600+ shell 980→**1040** (all fixed controls derive from `--col-max`, follow automatically); sheets/dialogs stay px on purpose. smoke-ci now runs 1920×1080 (was Puppeteer's 800×600 default — desktop tiers were gate-blind) + a horizontal-overflow tripwire. Live-verified 2048×1280 (Settings 650, reader 900, overflow 0; 300% scale clamps at shell; tablet unchanged 520/760). Deferred from Sol's list (future strikes, his order): footnote/scripture companion rail ≥1440px, Tabs thumbnail aspect at 4K, Library/Tabs grid columns, search/settings rails, 2560+ tier. Discarded: per-screen layout-role machinery, desktop-mode pref, Home 2-col (reorder is 1-D). Detail in HISTORY.md.

**Before that (2026-08-02, session 3):** **GPT 5.6 Sol uplift batch — adjudicated, corrected, landed** (`63562d0`). Sol's ~1,190-line batch (v3 import staging store + strict manifest validation + cross-tab Web Lock; native recording serialized under one binder-thread lock w/ teardown re-arm; URL credential/query redaction JS+Kotlin; Garden HTTPS-per-redirect; a11y sweep w/ dialog semantics + inert closed sheets) was fully re-verified (all gate claims reproduced; dist proven byte-identical to a fresh build), then corrected: 3 content-masking aria-labels deleted (chapter title/summary/section headings announced ONLY their label — jsdom's accname diverges from Chrome so no test can catch this class); Clear-All finally takes the backup mutex; **v3 restore is SALVAGE-mode by owner call ("99% > 0")** — frames match the manifest BY ID, failures skip+count, perfect stream → atomic REPLACE, anything less → `commitImportMerge` (deletes nothing, valid stores still land, shortfall reported); Export/Verify/Clear now hold the same cross-tab `withBackupLock` (import must not double-wrap — Web Locks aren't reentrant); Kotlin idle teardown no longer writes the device-global audio mode; letter-links Enter-only. One finding withdrawn: recorder cancel on teardown is CORRECT (no reattach flow exists). AGENTS.md → 5-line pointer. Known edge: media-DB v1→v2 + onblocked-reject means one stale pre-update PWA tab blocks the new tab's media DB until closed. **Post-land (`318f75d`): 2 residual-risk hardenings** — `useRestoreGuard` boot toast off a `vot-restore-inflight` LS marker bracketing the import apply (the crash-mid-restore window is no longer silent; full 2PC journal adjudicated-out), and a 12 MiB export tripwire vs Android's 16 MiB manifest import cap (warn at export, not on a wiped phone). 3,248 vitest / 237 Kotlin. Detail in HISTORY.md.

**Before that (2026-08-02, session 2):** **Recon-driven strike run** — an Opus 5 recon session ranked 5 improvement targets; coordinator verified + executed all 5 (recon's [4] placement call and 2 of its 7 dead-CSS candidates were WRONG — verification caught both). Cold boot −27% (`books-restored`+`matthew-plain` → bundle-a-bible, bundle-a 593→240 KB, c18→c19); use-settings (last untested hook, SEC-3 clamp RED-proven, 25 tests) + JournalInsertSheet (21 tests) covered; SW install throttled to 4 concurrent; `.mode-toggle-wrap` column-pin miss fixed (cascade-position trap); App() 800→769 via useJournalMediaSweep extraction; −2 KB proven-dead CSS. 3,209+ vitest. Detail in HISTORY.md.

**Before that (2026-08-02):** **SheetHandle + desktop viewport + Text Size 300%.** The sheet grabber pill is now ONE tappable module (`ui/components/SheetHandle.jsx` — bar + ‹ chevron both close; bar aria-hidden so the chevron is the single AT stop) replacing 9 duplicated cosmetic divs across 3 retired CSS classes (+ the journal insert sheet's fake bar deleted). Desktop: `--col-max` column tiers (768→760px, 1100→860px+18px root, 1600→980px+20px root, composed with `--font-scale`), fn/select sheets + chapter arrows + surprise-fab + link-sidebar pinned to the column (link-sidebar's override lives NEXT TO its base rule — cascade position, not the media block up top). Text Size cap 160→**300%** in all three clamps (SettingsScreen/use-settings/boot script) + the spill sweep that forced: settings-row-head/progress-row wrap, clear-btn/chip max-width, fn-sheet-num-row wrap + capped side padding — zero off-screen elements at scale 3 on 375px, proven live. fn-sheet ‹ › pager 44px. 4 defects found by a 7-agent adversarial review, all fixed (detail in HISTORY.md). 3,163 vitest.

**Before that (2026-07-31):** **Reading Font picker + Settings accordion redesign.** `settings.fontStyle` spans **25 fonts** via the `READING_FONTS` registry (`src/utils/reading-fonts.js`): System Serif (classic) + EB Garamond (modern) built-ins + 23 vendored OFL fonts — **all shipped in the app** (`fonts/reading/`, 1.73 MB/58 woff2, `tools/gen-reading-fonts.mjs`), plain @font-face in app.css (browser fetches only the family in use), SW-served corpusFirst from the STABLE corpus cache (`READING_FONT_PRECACHE`) so every choice is offline without re-download on version bumps. List is scripture-first ordered (owner call); picker is the standard SelectField dropdown sheet, names rendered in their own fonts, instant apply — no download step (the brief CDN download-on-demand design + `vot-fonts-v1` bucket are retired; CSP connect-src back to `'self'`). Body text routes through the `--font-body` CSS var; Cinzel chrome survives every non-classic choice. `reading-fonts.test.js` is a four-way sync gate (registry ↔ disk ↔ app.css ↔ SW precache). Settings is an 8-group collapsed-by-default accordion (`SettingsGroup`, bodies unmount while closed) and dependency-gated rows (search subs, Restored Names, History chip) unmount instead of greying. 3,163 vitest. Before that (07-31, `2b0593d`): About page 2 terse rewrite. Before that (07-30/31, `5f3e8c7`): the six-item owner-reported UX batch. **LibraryNav is now THE top-nav** — 19 hand-rolled implementations → 3 (LibraryNav + documented SearchScreen/GardenView exceptions); the "small back arrow" report was TWO root causes (LetterView's `nav-volume` lost the cascade to `.nav-back-icon` — class deleted with a tombstone in app.css; `_idxNav` never adopted the 07-14 icon back). Drilled notebook header split into two rows (name+count / actions, 44px targets), double-header killed. **Back pill everywhere except History** (owner's exception): `journalEntryId` is the 7th tracked field, journal `destSnapshot` fixed, journal-viewer renders ONE pill by precedence, History pushes `silent:true` (no pill, back still returns → `backHint`/`backActive` split), 5 new pushing link paths, and the previously-untested back-stack machine got `use-from-letter-stack.test.js`. New shared **`splitCompoundRef`** makes compound refs tap through per-part (the-blessed's compound chips were silent dead taps) + note-row per-segment taps with `verseEnd` + FootnoteListSection's missing Go-to button. ScripturesHome gained a layout-cycle button sharing Settings' exact options table and state (cannot desync, zero vertical cost). **Autoscroll honesty**: dwell stepper on the pill with `rearmDwell()` mid-countdown, Settings' ×9 wpm guess deleted, replaced by a MEASURED wpm — the naive words-per-line map assumed inline spans and would have shipped ~2000 wpm on LetterView's block paragraphs. 3,140 vitest (63 new), all gates green. Full detail in HISTORY.md.
...
- Owner decisions pending: the 5 Lamb-of-God PDF illustrations absent from the app (source PDF gone). **[19] golden search suite needs the owner's real search vocabulary** (10–15 queries he actually uses) and **[20] device-walk day needs his time** — those are the only two open FABLE5-BACKLOG items.
- Optional tracks: W10 deep accessibility (TalkBack walk — the CODE half landed 07-28 as focus traps); restored-names phase 2 (OT YAHUWAH).
