...
2. **How far to take the Bible.** Six pilot chapters ship today. BRM-KJV (the default voice) is 1,189 chapters, roughly 20 GPU-hours; Word of Promise and WEB are ~28 each. Owner should watch a pilot chapter before committing a night — see 3.
3. **Whole-verse vs sub-verse Bible highlighting.** Verses paint whole, which is what makes KJV audio work over NKJV text (the app's own default pairing). Mean verse is ~9s. Sub-verse would need word-index rows, only works when the reader's translation matches the audio's, and roughly triples the data. Decide after watching a chapter, not in the abstract.
