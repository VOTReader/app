...
>   - **POST-P6 HOOK CALL-ORDER DAG:** the authoritative call-order graph — 13 App()-level hooks, numbered, each annotated with its hook dependencies — is the comment block at the App() hook-call site in `index.html` (commit `eb2de5d`). **That comment is the single source of truth.** It is deliberately NOT duplicated here: the DAG that previously occupied this spot had drifted out of sync with the code (it mis-ordered `useNavigateToLink` and wrongly called `usePersistedState` last), and a maintained second copy is exactly what drifts. The true tail is `… → useTabActions → usePersistedState → useNavigateToLink → useAndroidBack` (useAndroidBack is HARD-LAST — it must follow every `go*` nav helper, and `goStudiesHome` is defined deep in App()). Do NOT reorder hook calls without consulting the index.html graph.
