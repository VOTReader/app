...
**Blind-audit remediation IN PROGRESS (2026-06-04).** An independent 14-agent blind adversarial audit produced **`BLIND-AUDIT-2026-06-04.txt`** (59 actionable items; Section 1 = priority index, Section 3 = detail). One execution session landed **29 items + 4 adjudicated across 13 commits (`a5f2196`..`28fc2a8`), all pushed + gate-green** — incl. the **Critical SEARCH ranking fix** (verses were buried under letter titles; the audit's "remove the ref field" was insufficient — real fix is index-coverage + a phrase boost + AND-search, proven on the REAL corpus via preview) and the **P0 JRNL-1** journal background-kill durability fix (synchronous localStorage draft + timestamp-guarded reconcile). **READ THE `HANDOFF (READ FIRST)` BLOCK near the top of `BLIND-AUDIT-2026-06-04.txt`** for the done/remaining split, the verify→prove→gate method, and 10 load-bearing GOTCHAS (e.g. to verify search in preview you must ALSO `indexedDB.deleteDatabase('vot-search-cache')`; the audit's own fix recs can be incomplete/wrong — re-derive + prove). Next focused priorities (each its own session): **STORE-1** (last P0, cross-tab store clobber — core `CachedStore._save`, riskiest) and **TEST-1** (Android v3 backup-driver test).
