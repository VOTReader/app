...
**Open:** `tools/audio-sync-stale.allow` lists the letters still on stale timings (a register that may only shrink and fails the gate if it names a key that now passes); the WTLB/flock/rebuke alignment runs; BRM-KJV's remaining chapters.
