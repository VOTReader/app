...
**Where things stand.** Still one JS codebase, shipping as the Android APK and a PWA (live + installable + full-offline at https://votreader.github.io/app/). Latest landed: **main `1faf6d15`** (2026-09-22 00:07) — Scripture Web's **structure law** (`9a30e9bf`, item 9 landing 7): every thread draws as the half-ellipse of its own span at *every* zoom (unbounded — `arcShape`: R = rx, A = rx×squash), so zoom is a magnifying glass on one dome instead of a separate bounded "sky" that has to run level and get sorted into strata. This retires the density law's strata/dome/run/LOD tables that had landed the session before (parts 1-3, 09-21) along with its fly-over stand-ins and "×n" badges, keeping the "every anchored line drawn, always" rule Corbin asked for after seeing lines vanish at rest. Next, in order (`lanes/myweb/out/structure-law.md`): landing 8 deletes the dead law's tests/constants, 9 adds sky navigation (an altitude ruler + elevator), 10 adds THE LENS (centre-chapter full ink past 6×, everything else dimmed to 0.35 — nothing hidden). `check:live` confirms `1faf6d15` deployed, CI + Deploy green through landing 7; a Codex T3 refuter pass and an S22 frame-time check on the landing-7 diff were still running as this was written. Corbin's standing rule (09-21): a green landing ships the moment its gates pass — nothing waits for a batch or the morning. Since 09-20, work coordinates through `D:\Swarm` (a multi-agent protocol: lanes + `BOARD.md`) alongside single-session handoffs; HISTORY.md stays the durable narrative.

Corpus is complete across every announced edition: BRM KJV (1,189/1,189 chapters, since 09-02) and Word of Promise NKJV dramatized (whole Bible as of 09-20's c58; 09-22's c61 name-tolerant witness took the three whole Bibles to BRM 31,080 / WOP 31,045 / WEB 31,062 of 31,102 verses timed, 115 unproven in all) both have full read-along; Matthew ships as a one-book Drive-sourced edition correctly named **"The Scriptures of Truth (read by Benjamin)"**, Corrected Version by Timothy with The Lord (09-21; the agent-invented "Sword of Truth" it shipped under 09-13 is purged repo-wide). WEB has its own read-along sync (c49) but ships no separate audio recording — text only, nothing further to align. Bible/Letter Studies got their own audio namespace, including the first "CUT" study recordings that paint clause-by-clause across many chapters from one file (Purity, Lamb of God). The WTLB compilations closed the last read-along gap 09-20 (14/14) — every shipped chapter with audio now has timings. Since 09-10: 1% playback-speed steps (0.50×-3.00×); "Turn the Page with the Audio" (the reading pane follows playback across unit boundaries); a spent collection/book/study continues into the next in site order instead of idling; an update-toast replaces the old silent-reload; new readers get sane defaults (dice/marker/auto-continue ON); Scripture Web's chrome is trimmed to one hide-all button and is back on Home; the six-stop "Show me around" tour shipped and was fully polished; My Web (personal connections) was rewritten around one canon colour law with GPU ceilings registered against the RTX 5080. Gates unchanged (see below); bundle-f's (Scripture Web) budget ceiling was rebaselined several times this window — re-measure, don't quote a size.

The letters batch resumes where it stopped with `py -3.13 tools/batch-align.py --volkeys flock,rebuke,holydays,two`. The owner tests on the INSTALLED APK — nothing reaches his phone without `npm run build` → `:app:assembleDebug` → `adb install -r -d`. Gates on every commit (pre-commit + CI): build, lint `--max-warnings 0`, typecheck, vitest + coverage floors, Kotlin `testDebugUnitTest` + lintDebug + JaCoCo floor, headless smoke:ci, check_balance, schema-validate, audio-sync, corpus-version, CSP-hash, asset-integrity, bundle budget, ≤800-line app.jsx canary; counts drift — verify with the runners. History lives in `git log`, HISTORY.md (the detailed session log) and the Closed-phases index below.
...
- **The structure law** (09-22, item 9 landing 7) — every Scripture Web thread draws as the half-ellipse of its own span at every zoom instead of a bounded, sortable "sky" stack; zoom becomes a magnifying glass on one dome, altitude becomes span. Retires the density law's strata/dome/run/LOD tables from the session before. Landings 8 (deletions) → 9 (sky nav) → 10 (the lens) queued next. `1faf6d15`.
- **The density law + Matthew renamed** (09-21) — every anchored line draws at every zoom, no exceptions (LOD-per-thread, convergence badges, bundle sheets, one representative per fly-over group, strata bands); a Codex refuter's 3 findings fixed same-day; 1% playback-speed steps (0.50×-3.00×); Matthew's one-book edition corrected to "The Scriptures of Truth (read by Benjamin)", purging the agent-invented "Sword of Truth". `ef3d2e8c`.
- **Corpus completes: Word of Promise whole Bible + WTLB compilations** (09-20) — c58 finishes the dramatized NKJV Old Testament (1,189 chapters/66 books, joining its NT), c59-c60 give the WTLB compilations their read-along (14/14, the last coverage gap); Scripture Web's bent-law tessellation re-cut, a Follow control for off-screen thread ends, search lands on the clause it matched. `0e5749c0`.
- **tsot-matthew ships, the tour closes out, search simplified** (09-12/13) — Matthew's read-along ships as c54 (first one-book edition); Word of Promise gains 8 more OT books (c55: Samuel/Kings/Chronicles/Ezra/Esther); the Tour Reviewer's D1-D6 findings all close; search cut to one result-chip row. `550984a3`.
- **Audio continuity, study read-along, My Web rewrite** (09-11/12) — "Turn the Page with the Audio" + cross-unit queue continuation; Purity (c51) and Lamb of God (c52) ship the first clause-by-clause "CUT" study recordings; Word of Promise NT+Pentateuch (c53); My Web rebuilt on one canon colour law with a GPU-batched renderer, ceilings registered against the RTX 5080. `f2a06aa9`.
- **Scripture Web chrome trim + update-toast + new-reader defaults** (09-10) — controls cut to one hide-all button, landscape-always, one palette; an update-toast ("VOTReader was just updated") replaces the old silent-reload; new profiles default to dice/reading-marker/auto-continue ON; Scripture Web returns to Home from "under construction". `6051004a`.
- **Scripture Web depth law + Bible-audio edition namespace + read-along expansion** (09-05/06) — the depth law (ceiling/alpha/curve/tessellation) and one-camera-two-axes fix; `BIBLE_AUDIO_EDITIONS` becomes the one edition declaration; Volume Seven (c47) and WEB (c49) get read-along, the Gospel of John ships as a listening-only film edition; light-theme WCAG-AA gold-token fixes. `0fe4c3b8`.
- **Closing the August sprint: journal/backup/nav/a11y/service-worker hardening, corpus c43-c44** (09-04) — voice-memo and backup resilience batches (journal-3/5/6, storage-backup 1-5), navigation-tabs 1-5, a11y-ux batch, service-worker hardening 1-5; the six-stop "Show me around" tour ships; the KJV divine name is restored (c43) and every reader rendition ships behind its own gate (c44). `c79ead15`.
- **The whole Bible paints verse by verse** (09-02) — 26 chapters / 995 verses became all 1,189 chapters and 31,061 of 31,102 verses (99.87%) out of one 10 h 27 m local-GPU campaign over 76.1 h of BRM KJV audio; mean proven share 0.9954, zero chapters under the 60% ship gate. It shipped WITH a re-cut of 82 chapter mp3s whose boundaries were wrong by 5-240 s (the differing-files set and the updated-release set proven identical), and with `tools/validate-bible-sync.py`, the per-verse gate the pipeline lacked at c40 — it fails 1,170 ways on the pre-c42 tree, `john_014` and `john_017` included. `tools/audit-bible-belts.py` reads UNSPOKEN runs off the belts instead of trusting the ±60 s transcript audit. c42.
- **The letter timings load only when a recording plays** (09-02) — `src/data/audio-sync.js` (497,951 B) left bundle-a-vot for its own lazy fetch, so nobody parses ~400 KB of timings to open a letter they never play; bundle-a-vot 2,657,855 → 2,256,281 B, and 140 KB of ceiling came back for the remaining alignments. A BUMP and not a rebaseline (membership changed). The gates learned the shape: `check-apk-assets` and `list-runtime-src-assets` derive the timing files from the `__makeLazyLoader` literals instead of a `bible-` prefix accident, `check-bundle-budget` re-baselined DOWN. Network-proven under the SW: zero fetches until the wash is on, then exactly one. c41.
- **The gospel of John paints verse by verse** (09-01) — John 2-21 joined the pilot tranche (21 chapters, 879 verses, weakest chapter 0.974 proven, zero on REVIEW), purely additive with the pilot rows untouched. Validating this ship is what exposed the two align-pipeline holes closed in `1ac1fc09`: a re-cut chapter silently reusing its old transcript and silence gaps, and a shipper that shipped every belt on disk whatever settings or audio it was made from. c40.
