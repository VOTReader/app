...
- **Reading Font picker + Settings accordion** (07-31) — 25-font `READING_FONTS` registry, all vendored in-app (1.73 MB `fonts/reading/`, stable-cache SW route, scripture-first order, SelectField dropdown), `--font-body` var plumbing; Settings → 8 collapsed accordion groups, dependent rows unmount.
  **The `fonts/reading/*.woff2` files are committed artifacts with no regeneration script in the tree** — they came from the fontsource CDN and were vendored wholesale by `605b700b`, which also deleted `tools/gen-font-previews.mjs` and the `fonts/previews/` name-glyph subsets it produced. If they ever need regenerating, re-add a generator script *and* its `subset-font` devDependency together; the dependency was left orphaned for a month after its only caller was deleted, and Dependabot kept bumping it against files nothing could rebuild.
