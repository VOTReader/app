...
- **W2 storage hardening — W2.1 → W2.8 + W2.5 + W2.6 LANDED 2026-05-27. Only W2.7 (StorageHealth dynamic warnings) remains.** 24 commits, range `16b8fbd..d7bacc6`. Every byte of structured user data now in IndexedDB (database `votreader`, schema v2 with 19 stores); legacy localStorage keys read once via per-store fallback then cleared by W2.4. **Tests**: 628 → 749 (+121). **Bundle delta**: bundle-b 357 → 402 KB; bundle-d 545 → 555.6 KB. **Hydration latency**: 3–8 ms across 17 IDB-backed CachedStores on preview machine (well under the 200 ms mid-range / 500 ms budget targets). Per-sub-phase:
