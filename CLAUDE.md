...
>
> **KNOWN PRE-EXISTING BUG found during P6h (not P6h-caused — flagged for separate fix):** the `BookmarkPopover` render at index.html:~3448 passes prop `payload: bookmarkPopoverPayload`, but `BookmarkPopover` (src/ui/screens/BookmarksScreen.js:371) destructures `{ bkmIds, x, y, onClose, onNavigate, onDeleteDone }` — no `payload` prop. So `bkmIds` is `undefined` → the component returns `null` at its line 381 (`if (!bkmIds || !bkmIds.length) return null;`). The inline-bookmark-icon tap popover (Open / Edit Thought / Delete) never displays. Confirmed: zero `payload` references in BookmarksScreen.js. Both lines are untouched by P6h. Fix: either spread `...bookmarkPopoverPayload` at the render OR change the render to pass `bkmIds/x/y` explicitly + rename `onNavigateToSource`→`onNavigate`. git-blame both lines to see which side drifted.
>
