...
- **N2 — second native-review response (2026-06-01): 2 fixes landed, 10 dispositioned.** A follow-up external review of the native Kotlin layer (proguard, audio session, GardenImageCache, JsBridge, bundle sizes) produced ~12 suggestions; each verified against source before acting (several converged with U7/U9/U19; a few rested on stale/incorrect premises). **LANDED:** **(N2.1)** fixed the stale `proguard-rules.pro` keep rule — it named the old inner class `MainActivity$AppInterface`, but `AppInterface` is TOP-LEVEL since the N1 extraction, so an R8/minified release would have stripped the whole `@JavascriptInterface` bridge (import/export/record/screenshot/haptic). Replaced with a correct `AppInterface` keep + a `@android.webkit.JavascriptInterface <methods>` wildcard backstop. **Currently dormant** (`isMinifyEnabled=false` → R8 never runs) so it's latent-footgun removal, not active breakage; becomes load-bearing the instant minify is enabled (tracked N2.1b). **(N2.11)** a specific "file too large" import message — the 50 MB cap (`StorageManager.MAX_IMPORT_SIZE`) previously failed SILENTLY (Android `Failure → __onImportFile(null)`, indistinguishable from cancel). Now Android passes a controlled `too_large` code, the web picker enforces the same 50 MB cap (`WEB_MAX_IMPORT_BYTES`) before touching FileReader, and SettingsScreen's `__onImportFile(b64, errCode)` shows an actionable oversize toast; every other failure stays a bare `null` (byte-identical to cancel → silent), so nothing else changed. +1 vitest (1637→**1638**); ALL gates green — build, tsc, eslint (397 globals), vitest+coverage (above floor), Kotlin `testDebugUnitTest`+jacoco, headless 12-screen smoke PASS. **QUEUED for the owed device walk** (real improvements, but they touch device-tuned/device-verified code → validate on hardware, don't ship blind): **N2.2** transient-exclusive audio focus around recording (pause Spotify instead of bleeding into the memo — touches the Pixel/Samsung `MODE_IN_COMMUNICATION` tuning; route AudioManager calls through new BridgeHost methods to keep AppInterface testable); **N2.3** `volumeControlStream=STREAM_MUSIC` (marginal — Android's adaptive default already targets media during playback; optional); **N2.4** Garden `download()` stream-to-disk vs `readBytes()` heap — **PROFILE-FIRST** (à la U8/W7.6: the cache is device-verified working, the ~40 MB-transient is theoretical, and the naive fix changes the deliberate serve-from-memory-on-write-fail property); **N2.5** haptic API-26-28 fallback → `DEFAULT_AMPLITUDE` (dormant — `haptic()` has no JS caller yet); **N2.1b** enable R8 minify (modest AndroidX shrink; needs a full device bridge walk; low urgency — no Play Store goal). **SKIP-with-reasoning** (full ledger in HISTORY.md): N2.6 screenshot-async = already U9 (binder≠main); N2.7 system-theme-follow (we have an explicit light/dark toggle — a 2nd source of truth would fight it); N2.8 WebView Safe Browsing (Google callback vs the no-egress policy, ~zero protection — no untrusted top-level nav); N2.9 Thread→Coroutine (cosmetic, adds scope plumbing); N2.10 `quote()` (NEL is NOT a JS line terminator — the U+2028/U+2029 escaping is correct + JsBridgeTest-pinned; byte-verified the two when-branches are genuinely `E2 80 A8`/`E2 80 A9`, not a duplicate); N2.12 SQLite/FTS5 (collides with dual-platform single-codebase + diffable/gated corpus + offline PWA; bundle figures were stale — bundle-a is 836 KB, corpora lazy — capture the budget-device win via D1 virtualization instead). The native layer stays the highest-rated subsystem; this round was polish on working, device-tuned code.
- **UPLIFT COMPLETE — every item U0–U22 closed (2026-06-01, Wave 5).** Past the 8/10 exit bar, the full P2 "ceiling" set landed, all CI-green + deployed; tests **1527 → 1637**, coverage floor ratcheted, plus a new headless CI render-walk gate. This batch (commits `c836686..3c1dda9`): **U12** scripture-parser robustness (en/em-dash → ASCII normalize in both parsers so a stray range still renders gold; `findBook` exact-before-prefix so "Jude"≠"Judges"); **U20** platform-detect tightened (require AppleWebKit + exclude CriOS/FxiOS/DuckDuckGo/in-app tokens → no wrong Safari eviction warnings); **U14** (formal) routed the 4 `window.AndroidBridge` bypasses through `PlatformBridge.isAndroid` + restored the single-source invariant; **U21** dropped the external connectivity ping (→ `navigator.onLine`, zero egress) + removed thevolumesoftruth.com from connect-src; **U13** lazy-loaded html2canvas off the boot path (**bundle-a 1.03 MB → 836 KB, −199 KB**; injected on first web screenshot, SW-precached); **U18** PWA hygiene (hourly + on-focus `reg.update()`; pruned 6 redundant CORE_ASSETS — react/react-dom/flexsearch/search/search-data are in bundle-a, data-normalize orphaned — kept html2canvas as the U13 lazy source; offline claim documented honest); **U19** robustness (`clearLegacyLs` now guards on `hasAnyPendingStores()` so a reorder can't wipe un-migrated LS; `base64ToBlob` full-string validation); **U9** moved the screenshot bitmap encode off the main thread to `Dispatchers.Default` (the *real* fix — the runBlocking is on the binder thread, not main, so the ANR premise was overstated) + cached the Vibrator; **U17** `BridgeContractTest` pins the 20-method `window.AndroidBridge` surface (drift fails the build); **U16** `tools/smoke-ci.js` runs the real 12-screen render walk + both annotation round-trips in headless Chrome (puppeteer) as a CI gate — **green in CI**. Deferred-with-reasoning (à la W7.6, all verified against source): the U8/U9 risky rewrites (profiled — overstated premises), the U17 JaCoCo-extension (StorageManager Robolectric artifact + GardenImageCache I/O drops the bundle to 0.59), MainActivity Robolectric tests, and the cosmetic `var useState` sweep (11-file churn, "no regex at file scope"). **Device-verified on the API-28 emulator (WebView 69):** the cumulative build boots + fully renders + 0 CSP/JS/crash logcat. STILL OWED (manual/uiautomator — feature-level UI walks, in `tools/n1-smoke-walk.md`): the U1 import file-walk, U7 Garden Ultra-tier crawl, U9 thumbnail-capture device check. The D-bucket (D1–D8 architectural) remains the only un-started work; it raises the ceiling further but was never required.
- **8/10 EXIT BAR MET — UPLIFT Wave 4 closed (2026-06-01).** The five items between Wave 1 and the 8/10 target all landed, CI-green + deployed; tests **1527 → 1621**, coverage floor ratcheted twice. **(U14 / U1-owed) export→import e2e round-trip** — the backup data plane (payload build + import-apply + the U1 durability barrier) extracted from SettingsScreen into a pure, dependency-injected `src/utils/backup.js`; `backup.test.js` drives a full populate→export→WIPE→import→**RELOAD**→assert round-trip vs the REAL stores + fake IDB (the "reload" drops every in-memory cache + the IDB connection and re-hydrates from disk, proving the import DURABLY landed — all 14 stores + 3 flags + a real media blob, bytes-equal); the real Settings→EXPORT button was preview-verified through the shipped bundle. **(U11) dual-render equivalence** — `annotation-engine.test.jsx` now asserts a per-CHARACTER signature {innermost-mark color, data-hl-id (= the elementFromPoint tap winner), note-in-chain} is byte-equal across the React sweep-line + the imperative DOM path over 5 overlap scenarios — the regression net U8 needed. **(U15) coverage** — `renderer/**` + `data/scripture-resolution.js` joined the vitest coverage gate (+70 tests: a 42-case scripture-resolution engine suite + dom-links/dom-bookmarks/dom-journal-chip); the aggregate ROSE (annotation-engine + resolution are well-covered), so the floor ratcheted UP. **(U8) apply-pass perf** — PROFILED first (sweep over 176 containers = 0.215 ms, sub-ms), so the risky single-walk/scoping refactor of the hot, U11-guarded code was deferred WITH DATA (à la W7.6); landed the one real win — dropped a dead guard, scoping the query to `[data-hl-key][data-hl-dom]` (−30% sweep). **(U10, security) CSP `unsafe-inline` dropped from script-src** — `tools/sync-csp-hashes.js` sha256-hashes the 9 inline `<script>` blocks into the CSP allow-list (nonces are impossible on static hosting), wired into `build:csp` + pre-commit + CI `check:csp`; **verified on BOTH web (0 CSP violations) AND the API-28 emulator / WebView 69 (app fully renders, 0 "Refused to execute inline script" in logcat)**; `style-src` keeps `unsafe-inline` (React dynamic inline styles — no static hash path); img/connect-src host-narrowing deferred to U21. Two non-obvious CSP traps solved: hash over CR-stripped content (HTML parser normalizes CR/CRLF→LF before hashing, so tokens match LF-Pages and CRLF-APK alike) + strip HTML comments before extracting scripts. **Remaining (raise the ceiling past 8, NOT required for it): U9** (async screenshot), **U12** (scripture parser en/em-dash defense), **U13** (html2canvas lazy), **U16** (CI headless smoke), **U17** (Kotlin/contract tests), **U18** (PWA hygiene), **U19** (LOW ledger), **U20** (platform misdetect), **U21** (connectivity-ping egress + the U10 img/connect-src narrowing), plus the formal **U14** (route the 4 `window.AndroidBridge` bypasses through PlatformBridge — distinct from the e2e round-trip the exit bar loosely called "U14").
- **8/10 UPLIFT PLAN created (2026-06-01) — `UPLIFT-PLAN.txt`.** A deep 7-subsystem review (frontend, storage, renderer, native, perf, PWA, testing) rated the app **7/10** and found a cluster of fixable issues concentrated in **data-safety + performance**. The remediation — 21 work items (U0–U21) with severities, file:line evidence, exit criteria, and a wave sequence — lives in **`UPLIFT-PLAN.txt`** (canonical home for this work; PLAN.txt points to it). Headline P0/P1 (all coordinator-verified against source): **(U1)** import — the ONLY backup — has a silent data-loss race (fire-and-forget `_save` at cached-store.js:278 + blind `setTimeout(reload,1500)` at SettingsScreen.jsx:714; compounds with the boot orphan-media sweep); **(U2)** nothing is minified (measured ~44% / 472 KB off the boot path); **(U3)** CORPUS_VERSION has no enforcement gate (silent stale-scripture risk); **(U4)** Search loads ~31 MB of alt-translations on every open even on a warm index (search.js:1076); **(U5)** every annotation tap re-renders all of App() (zero `React.memo` in `ui/`). Security: **(U7)** GardenImageCache has no host allowlist; **(U10)** CSP allows `unsafe-inline`. A **doc truth-pass (U0)** corrected three drifted claims this session: the "minified prod build" that never existed, the "1.03 MB cold-boot" that's really ~2.10 MB (a+b+c+d all load blocking), and the PlatformBridge "ZERO `window.AndroidBridge` matches" invariant that 4 live sites violate. (The review's "`__bumpHlTick` still live" finding was a FALSE ALARM — verified fully removed.) **U1 (import durability) LANDED 2026-06-01**: `CachedStore` now exposes `whenSaved()` (a durability barrier — `_save()` captures the IDB put promise), `_doImport` awaits every store's write before reloading (the blind `setTimeout(1500)` is gone), and the boot orphan-media sweep is gated on `JournalStore.isReady()` so a degraded/mid-import store can't trigger an irreversible blob delete. 5 unit tests; tsc + build + **1527 vitest** + 0 console errors; web-verified the primitive is wired into real stores. Owed: emulator import file-walk; U14 (full `_doImport` extraction + round-trip test). **U2 (minify) LANDED** — b/c/d minified, critical path 2054→1594 KB (−460 KB / −22%), CACHE_VERSION auto-bumped. (bundle-a minify was attempted then **REVERTED**: esbuild rewrote the concatenated classic-script's top-level `this`→`undefined`, breaking the FlexSearch + html2canvas UMD global attachment — search engine + web screenshots dead. Reverting restored both, preview-verified. Lesson: never esbuild-minify a concatenated classic-script bundle of UMD vendors.) **U3 (corpus-version gate) LANDED** — `tools/check-corpus-version.js` + `tools/corpus-version.lock`, wired into pre-commit + CI, fails any corpus-bundle change without a CORPUS_VERSION bump (break-tested). **U4 (search 31 MB load) LANDED** — alt-translations now load only on a cache-miss index build (`loadAllTranslations` moved into `ensureIndex`'s miss branch); a warm search open does zero translation downloads. **U22 (user-flagged) LANDED** — headings / inline topic breaks / chapter titles dropped from the search index (SCHEMA 12→13; verse-text search intact). **U5 LANDED** — the annotation store-subscription moved into an `<AnnotationDomSync/>` leaf so a highlight/note/link/bookmark tap re-renders only that leaf, not all of App() + the ~90-prop ROUTES tree. **U6 LANDED** — export fails loud on a partial read instead of writing an incomplete-but-valid-looking file, + adds a `counts` manifest. **U7 LANDED** — GardenImageCache host allowlist (SSRF guard) + single-flight cap enforcer + .tmp sweep. **All P0 + P1 (U1–U7) + U22 are DONE, CI-green, deployed.** The full **8/10 EXIT BAR is now MET** (Wave 4, see the lead bullet above): U14 (export→import e2e round-trip), U11 (dual-render equivalence), U15 (renderer+resolution coverage), U8 (apply-pass perf), U10 (CSP `unsafe-inline` dropped) all landed + verified. The rest of P2 (U9, U12–U13, U16–U21) further raise individual subsystem scores past 8. See UPLIFT-PLAN.txt for the live status table.
...
- **Bundle-b grew** 302→320 KB from JSDoc comments preserved in the build. (CORRECTION 2026-06-01: there is **no minified prod build** — `npm run build` ships unminified and `build:b/c/d` carry no `--minify`. Adding it is UPLIFT-PLAN **U2**, a measured ~44% / 472 KB cut off the boot path.)
...
Cold-boot bundle-a: **11.7 MB → 1.03 MB (91% reduction)** across
three commits. (2026-06-01: 1.03 MB is bundle-a only; b+c+d also load
blocking → full cold-boot JS path ~2.10 MB. See UPLIFT-PLAN.) Pattern proved on books.js (Q8.1), expanded to all
