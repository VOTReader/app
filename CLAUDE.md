...
**Latest landed (2026-08-05):** **streaming audio letters.** Every letter/preface + WTLB/Blessed/Holy-Days entry (729/729) now has a spoken track streaming from the flock's public Drive (verified 206/range; Benjamin's readings supersede other renditions, reader rank B>T>TTS>AI resolves duplicates, full-letter beats section splits). Hero "Listen" pill (LetterView + WtlbEntryView), "Play All" + WTLB Part/Section compilation chips on all 14 indexes (`colIdxProps`), bottom mini-player bar (`AudioPlayerBar` in AppShellOverlays) with queue prev/next, Media Session, offline toast. Pipeline: `tools/fetch-drive-audio.py` + `tools/gen-audio-manifest.mjs` → `src/data/audio-manifest.js` in bundle-a-vot (**c20**; regen + bump on new flock uploads). Native: `setAudioActive` bridge → `onPause()` keeps WebView alive while playing (screen-off listening); recording pauses playback. CSP media-src + drive.usercontent.google.com. Gates: 3,433 vitest, smoke ×2, Kotlin green. Detail in HISTORY.md.
...
- **Streaming audio letters** (08-05) — 729/729 letters mapped to Drive tracks (Benjamin-supersede + reader ranking), hero Listen pill + Play All + WTLB section chips + mini-player queue bar, Media Session, screen-off keep-alive bridge, offline toast; manifest generator pipeline; corpus c20.
