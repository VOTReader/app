...
> Last major update: 2026-05-19 (later — MODULARIZATION PUSH P1-P5c, single marathon session).
>
> **index.html: 17,200 lines / 944 KB → 6,340 lines / 320 KB.** ~10,900 lines / ~624 KB extracted into **106 cohesive modules** under `src/`. All work on branch `refactor/modularize-incremental` (16 commits ahead of `main`, fully pushed to origin as backup).
>
...
> - `ui/sheets/` — 16 sheets: `BookmarkCreateSheet`, `JournalRecordingSheet`, `JournalInsertSheet`, `JournalNotebookSheet`, `JournalInboundSheet` (P5b earlier), plus P5c: `SelectionToolbar`, `NoteSheet`, `VersePickerScreen`, `LetterExcerptPickerScreen`, `LinkPicker`, `TabsOverview`, `AnnotationActionChip`, `NotebookPickerSheet`, `TabActionSheet`, `LinkSidebar`, `MultiNotePopover`
> - `components/` — `ExpandableText` (used by journal/bookmarks/notes)
> - `styles/` — `journal-styles`
>
> **WHAT'S LEFT IN `index.html` (~6.3k lines):** `App()` (~2,500 lines — the only big inline component, **P6 target for hook extraction**); ~30 small helpers (`bibleHlKey`/`letterHlKey`/`wtlbHlKey`/`studyHlKey`, `bookCategory`, `buildNavIndex`/`searchNavIndex`, `navItemToEndpoint`/`buildSourceEndpoint`, `parseRefRange`/`splitIntoVerses`, `relativeDate`/`timeAgo`, `highlightExcerptInDom`, `splitWithHighlight`/`normalizeForHighlight`, `renderTextWithScripRefs`/`renderCommentaryCite`, IDB thumbnail helpers, `loadTranslation`/`loadBibleStudies`, `translateVerse`, `linkWtlbEntries`/`linkPreface`, `resolveVotLetter`, `isHiddenManna`, `srchGroupKey`, etc.); a handful of small components still inline (`NotebookManagerSheet`, `NoteRow`, `LinkIcon`, `LinkCard`, `SelectField`, `SettingsRow`, `ClearProgressRow`, `HistoryEntryCard`, `VolumeIndex`/`VolumeOneIndex`/`VolumeLetterIndex`, `ScripturesHome`, `ScriptureGenre`, `GardenView`, `ChapterIndex`, `BibleStudyIndex`, `SrchSnippet`/`SrchCard`); inline `ErrorBoundary`, `HL_COLORS`, `useMarkAsRead`, `noteSourceLabel`/`noteSourceNav`; the final `ReactDOM.createRoot` + render.
>
> **NEXT: P6 — `App()` hook extraction.** Extract `useTabField` / settings persistence / scroll handlers / history tracking / thumbnail capture / visualViewport keyboard tracking / etc. into `src/hooks/` so `App()` drops from ~2,500 lines to ~400. **The riskiest single refactor in the whole plan** — real semantic surgery (closure dependencies, React hook ordering rules, state passing between newly-separated hooks). DEFER to a dedicated focused session with full attention.
>
> **Verified clean after P5c (2026-05-19, later):** the "7 WtlbEntryView useMemo errors per mount" I originally flagged was a **phantom** — Chrome devtools' persistent console log buffer retained errors from a one-off detached `ReactDOM.createRoot(div).render(WtlbEntryView)` test I ran during P5c verification (a detached root can't establish a React hook context, so hooks legitimately throw there). A fresh-mount `console.error` interceptor caught zero real errors across LetterView render, theme toggle, WTLB entry render, and Home navigation. There is no latent WtlbEntryView bug.
>
