...
/* `hide` drops individual icons: ['settings'|'history'|'search'|'theme'].
   ...
   visibility toggles select on them (app.css:320-338). */
export function NavButtons({ onSettings, onHistory, onSearch, theme, onThemeChange, reading, chapterBookmark, hide }) {
  ...
  return (
    <>
      ...
      {off('history') ? null : (
      <button className={reading ? "nav-search-btn nav-history-reading" : "nav-search-btn"} onClick={onHistory} title="History">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6">
