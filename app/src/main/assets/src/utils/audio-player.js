...
/* ═══════════════════════════════════════════════════════════════════════
   ...
   through a `_set<Name>` beside it). The modules are ORDERED and none imports one above it (smoke-lite
   forbids import cycles): a call up the order goes through core.js's `late` table. Bottom to top:
     core          the state object, subscribers, shared handles, corpus globals, words, `late`
     catalog       manifest queries, renditions, the preferred reader, the corpus lookups
     offline       what plays with no signal
     prefetch      the gentle warm of what comes next (web engine)
     sections      WTLB compilations: the letter under the clock, and its follower
     credit        listen credit and play counts
     persist       the boot snapshot (LS + IDB) and the per-recording positions
     sleep / loop  the sleep timer and its fade / repeat this passage
     ...
     queue         the play* entry points, the site order, queue edits
