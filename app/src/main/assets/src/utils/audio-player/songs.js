...
import { _songTracks } from './catalog.js';
import {
  _el,
  _isSong,
  late,
...
import { _setSongSkips, _start } from './engine.js';
import { _syncMediaSessionActions } from './media-session.js';
import { _offlineNotice, _offlineRefuses } from './offline.js';
import { _forgetPosition, _persist, _rememberOutgoingPosition, _setLastPersistSec } from './persist.js';
...
export function switchSongVersion(id) {
  ...
  if (_pendingRestore) {
    ...
    void late._rebuildRestoredQueue().then(() => { switchSongVersion(id); });
    return true;
  }
  const src = _source;
...
Object.assign(late, { _repeatMode, _replayCurrent });
