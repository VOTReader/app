...
function _ensureEl() {
  ...
  _setNative(nativeAudioAvailable());
  const el = _native
  ...
  el.addEventListener('ended', () => {
    ...
    try {
      ...
      else if (late._repeatMode() === 'one') late._replayCurrent();
      else late.next();
...
function _nativeUpcoming() {
  ...
  } else if (late._repeatMode() === 'all') t = _state.queue[0];
...
function _songLoadFailed() {
  ...
  if (!(_state.qi + 1 < _state.queue.length || late._repeatMode() === 'all')) return false;
...
export function _start() {
  ...
  late._crossInto(track);
  if (_state.qi === _state.queue.length - 1) late._extendQueue();
  const el = _ensureEl();
  _clearStallWatchdog();
  ...
  if (p && typeof p.then === 'function') p.then(() => { late._setResumeAfterUpdateArmed(false); }, (err) => { late._playRefused(err); });
...
Object.assign(late, { _markPaused });
