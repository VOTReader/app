...
describe('export → wipe → import → reload round-trip (real stores + fake IDB)', () => {
  const ALL_STORES = [
    ...
    ReadingStreakStore, GardenPosStore,
  ...
  it('every store + media survives a full backup round-trip across a simulated reload', async () => {
    ...
    GardenPosStore.replaceAll({ lastPage: 137 });
    ...
    ReadingStreakStore.replaceAll(null); GardenPosStore.replaceAll(null);
    ...
    expect(GardenPosStore.get()).toBe(137);
