...
describe('IDBAdapter — open + schema', () => {
  ...
  it('STORE_NAMES contains the 23 vot-* keys plus meta (v8 vot-reading-stats; v7 vot-ann-hint-dismissed)', () => {
    const expected = new Set([
      ...
      'vot-ann-hint-dismissed', 'meta',
    ]);
    expect(new Set(IDBAdapter.STORE_NAMES)).toEqual(expected);
    expect(IDBAdapter.STORE_NAMES.length).toBe(24);
  });
  ...
  it('DB_VERSION is 8 (v8 vot-reading-stats; v7 vot-ann-hint-dismissed; v6 vot-garden-pos; v5 vot-reading-streak)', () => {
    expect(IDBAdapter.DB_VERSION).toBe(8);
  });
